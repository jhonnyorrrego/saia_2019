...
class bpmn {
	...
	public function habilitar_tarea_siguiente() {
		...
		if (in_array($this -> get_estado_bpmni(), array(
			4,
			5,
			6
		))) {
	...
	public function crear_posible_ruta($iddoc) {
		...
		for ($i = 0; $i < $this -> pasos_ruta["numcampos"]; $i++) {
			array_push($ruta, array(
				"funcionario" => -1,
				"tipo_firma" => 0,
				"paso_actividad" => $this -> pasos_ruta[$i]["idpaso_actividad"]
			));
		}
		...
		for ($i = 0; $i < $tarea_siguiente["numcampos"]; $i++) {
			if ($tarea_siguiente[$i]["tipo_destino"] == "bpmn_condicional") {
				...
				if ($tarea_siguiente_destino["numcampos"]) {
					array_push($this -> tareas_siguientes, array(
						"idpaso" => $tarea_siguiente_destino[0]["idpaso"],
						"idsvg" => $tarea_siguiente_destino[0]["idfigura"]
					));
		...
		foreach ($this->enlaces AS $forma) {
			...
			foreach ($camino AS $punto) {
				array_push($puntos, array(
					"type" => "L",
					"x" => $punto -> getAttribute("x"),
					"y" => $punto -> getAttribute("y")
				));
			}
