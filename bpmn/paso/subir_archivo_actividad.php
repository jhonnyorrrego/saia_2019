  ...
  crear_destino($ruta_db_superior.$ruta);
	...
	foreach ($files->files as $key => $value){
      ...
      $verifica=busca_filtro_tabla("","paso_actividad_anexo","actividad_idactividad=".$_REQUEST["idactividad"]." AND documento_iddocumento=".$_REQUEST["iddocumento"]." AND etiqueta='".$_FILES['files']['name'][0]."'","",$conn);
			if(!$verifica["numcampos"] && rename($ruta_db_superior.$ruta.$_FILES['files']['name'][0],$ruta_db_superior.$ruta.$nombre.".".$type)){
				$sql2 = "INSERT INTO paso_actividad_anexo (actividad_idactividad,documento_iddocumento,etiqueta,ruta,tipo) values(".$_REQUEST["idactividad"].",".$_REQUEST['iddocumento'].",'".$_FILES['files']['name'][0]."','".$ruta.$nombre.'.'.$type."','".$type."')";
				phpmkr_query($sql2);
