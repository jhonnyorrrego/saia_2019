...
if(@$_REQUEST["idformato"]){
  ...
  if($campos["numcampos"]){
    ...
    $html.='<option value="" etiqueta_html="" valor_llenado="">Por favor seleccione</option>';
    for($i=0;$i<$campos["numcampos"];$i++){
      $html.='<option value="'.$campos[$i]["idcampos_formato"].'" etiqueta_html="'.$campos[$i]["etiqueta_html"].'" valor_llenado="'.$campos[$i]["valor"].'">'.$campos[$i]["etiqueta"]." (".$campos[$i]["etiqueta_html"].")".'</option>';
    }
    $html.='</select>';
    $retorno["campos"]=$html;
  }
