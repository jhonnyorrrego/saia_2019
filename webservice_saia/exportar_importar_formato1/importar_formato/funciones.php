...
function generar_importar($datos) {
	...
	//VALIDO FORMATO
	$keys_datos_formato = array_keys($datos['datos_formato']);
	//keys recibidas
	...
	$keys_datos_formato = array_diff($keys_datos_formato, $keys_no_validas);
	//esto no esta en release1
	...
	$keys_formato_insertar = listar_campos_tabla('formato');
	//key a insertar
	...
	if (@$datos['campos_formato']) {
		$keys_campos_formato = array_keys($datos['campos_formato'][0]);
		//keys campos_formato recibidas
		$keys_campos_formato_insertar = listar_campos_tabla('campos_formato');
		//keys a insertar
	...
	if (@$datos['funciones_formato']) {
		$keys_funciones_formato = array_keys($datos['funciones_formato'][0]);
		//keys campos_formato recibidas
		...
		$keys_funciones_formato_insertar = listar_campos_tabla('funciones_formato');
		//keys a insertar
	...
	if (@$datos['funciones_formato']) {
		if (@$datos['funciones_formato'][0]['accion_funcion']) {
			$keys_funciones_formato_accion = array_keys($datos['funciones_formato'][0]['accion_funcion'][0]);
			//keys campos_formato recibidas
			...
			$keys_funciones_formato_accion_insertar = listar_campos_tabla('funciones_formato_accion');
			//keys a insertar
	...
	if ($valida_campos_tabla) {
		...
		if ($existe_formato['numcampos']) {
			...
			unset($datos['datos_formato']['fecha']);
			//FECHA
