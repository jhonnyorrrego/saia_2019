...
if (!$_SESSION["LOGIN" . LLAVE_SAIA]) {
	logear_funcionario_webservice(LOGIN_LOGIN);
}
...
function generar_html_info_qr($datos) {
	...
	if (!$respondidos["numcampos"])
		$respuestas[] = "Sin respuestas realizadas";

	$tipo_documento = busca_filtro_tabla("", "serie A", "A.idserie=" . $documento[0]["serie"], "", $conn);
	if (!$tipo_documento["numcampos"])
		$tipo_documento[0]["nombre"] = "Sin serie asignada";
...
function rastro_documento_info($x_doc, $filtro) {
	...
	if ($filtro)
		$titulo = "SEGUIMIENTO PERSONAL DEL DOCUMENTO";
 	...
 	';
		for ($i = 0; $i < $recorrido["numcampos"]; $i++) {
			...
			if ($recorrido[$i]["nombre"] == "DISTRIBUCION" && strpos($recorrido[$i]["notas"], "enviado por e-mail") === false) {
				...
				elseif ($documento[0]["plantilla"] == "CARTA") {$destinos = busca_filtro_tabla("destinos", "ft_carta", "documento_iddocumento=$x_doc", "", $conn);
				...
				} else {$html_retorno .= ('<td><span class="phpmaker" >');
