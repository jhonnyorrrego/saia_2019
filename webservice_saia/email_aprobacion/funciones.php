...
function ejecuta_funciones($datos) {
	...
	if ($datos['accion'] == "REVISADO") {
		...
		$ch = curl_init();
		$fila = PROTOCOLO_CONEXION . RUTA_PDF_LOCAL . "/class_impresion.php?plantilla=carta&iddoc=" . $datos['iddoc'] . "&conexion_remota=1&LOGIN=" . $_SESSION["LOGIN" . LLAVE_SAIA];
		...
		if ($datos -> login != "" && $datos -> password != "") {
			...
			if (trim($verifica[0]['clave']) == $clave_encriptada) {
				...
				Gracias por su gestión.";
							...
							$_REQUEST['refrescar'] = 1;
							$iddocumento = aprobar2($datos -> iddoc);
							// la funcion tiene comentariado las acciones o funciones "llama_funcion_accion"
