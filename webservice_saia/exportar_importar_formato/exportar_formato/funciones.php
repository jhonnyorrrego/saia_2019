...
function generar_array_keys($consulta, $keys_no = array()) {
	if (count($consulta)) {
		$keys = array_keys($consulta[0]);
		//obtengo keys
		$keys_num = array_filter($keys, "is_numeric");
		//filtro llaves numericas
		$keys = array_diff($keys, $keys_num);
		//retiro las llaves numericas
		if (count($keys_no)) {
			$keys = array_diff($keys, $keys_no);
			//retiro llaves no permitidas
		}
...
function generar_datos_formato($idformato) {
	...
	//campos con validaciones particulares
		...
		if ($formato_orig != $idformato) {// busco el nombre del formato inicial
			...
			if ($dato_formato_orig["numcampos"] && ($dato_formato_orig[0]["nombre"] != $formato[0]["nombre"])) {
				...
				if (is_file($ruta_db_superior . "formatos/" . $dato_formato_orig[0]["nombre"] . "/" . $funciones[$i]["ruta"])) {
					...
					$lfunciones["formatos/" . $dato_formato_orig[0]["nombre"] . "/" . $funciones[$i]["ruta"]][] = array(
						"nombre_funcion" => $funciones[$i]["nombre_funcion"],
						"acciones" => $funciones[$i]["acciones"],
						"ruta_real" => realpath($_SERVER["DOCUMENT_ROOT"] . "/" . RUTA_SCRIPT . "/formatos/" . $dato_formato_orig[0]["nombre"] . "/" . $funciones[$i]["ruta"])
					);
					...
					$lfunciones[$funciones[$i]["ruta"]][] = array(
						"nombre_funcion" => $funciones[$i]["nombre_funcion"],
						"acciones" => $funciones[$i]["acciones"],
						"ruta_real" => realpath($_SERVER["DOCUMENT_ROOT"] . "/" . RUTA_SCRIPT . "/" . $funciones[$i]["ruta"])
					);
				} else {
					array_push($includes, 'Error en la ruta ' . $funciones[$i]["ruta"] . "| id=" . $funciones[$i]["idfunciones_formato"]);
					$lfunciones["error"][] = array(
						"nombre_funcion" => $funciones[$i]["nombre_funcion"],
						"acciones" => $funciones[$i]["acciones"]
					);
				}
			...
			if (is_file($ruta_db_superior . "formatos/" . $formato[0]["nombre"] . "/" . $funciones[$i]["ruta"])) {
				...
				$lfunciones["formatos/" . $formato[0]["nombre"] . "/" . $funciones[$i]["ruta"]][] = array(
					"nombre_funcion" => $funciones[$i]["nombre_funcion"],
					"acciones" => $funciones[$i]["acciones"],
					"ruta_real" => realpath($_SERVER["DOCUMENT_ROOT"] . "/" . RUTA_SCRIPT . "/formatos/" . $formato[0]["nombre"] . "/" . $funciones[$i]["ruta"])
				);
			} elseif (is_file($ruta_db_superior . $funciones[$i]["ruta"])) {
				// si el archivo existe en la ruta especificada partiendo de la raiz
				array_push($includes, $funciones[$i]["ruta"]);
				...
				$lfunciones["error"][] = array(
					"nombre_funcion" => $funciones[$i]["ruta"] . "->" . $funciones[$i]["nombre_funcion"],
					"acciones" => $funciones[$i]["acciones"],
					"ruta_real" => realpath($_SERVER["DOCUMENT_ROOT"] . "/" . RUTA_SCRIPT . $funciones[$i]["ruta"])
				);
			}
