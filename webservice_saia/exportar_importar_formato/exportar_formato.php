...
function validar_respuesta_medio($respuesta_medio,$tipo_conexion=0){
	...
	<script>
		$(document).on("click","#continuar_importar",function(){
			window.open("'.$ruta_db_superior.'webservice_saia/exportar_importar_formato/exportar_formato.php?pre_exportar_formato=1","_self");
		});
	</script>';
	...
	<table class="table table-bordered">
		...
		$(document).ready(function(){
			...
			$("#servidor_destino").focusout(function(){					
				validar_existe_destino( $("#servidor_destino").val()+"/webservice_saia/exportar_importar_formato/existe_carpeta.php");
			});
			/**
