...
$origen = @$ruta_actual[0]["origen1"];  
    ...
    <table border="0" cellspacing="1" cellpadding="4" bgcolor="#CCCCCC">
      ...
      <input type="hidden" name="idruta'.$i.'" value="'.$ruta_actual[$i]["idruta"].'">  
       ...
       echo "<tr><td colspan='4' bgcolor='f5f5f5' align='center'><input type='submit' value='GUARDAR CAMBIOS'></td></tr>";
        if(!isset($unafirma)){
          echo "<tr><td colspan=4 bgcolor=\"#F5F5F5\"><table width=100%><tr><td><a href=formatos/librerias/rutaadd.php?doc=$iddoc&origen=".usuario_actual('funcionario_codigo')."&tipo_origen=1&rehacer=editar_ruta&reset_ruta=true&reset_borrador=1&new_user_borrador=".usuario_actual('funcionario_codigo')."&cargar=1 onclick='javascript:if(confirm(\"Esta seguro de eliminar los responsables y volver a asignarlos?\")) return true; else return false;'>Reasignar Responsables</a></td><td colspan='1' bgcolor=\"#F5F5F5\" aling='right'><a href='formatos/librerias/rutaadd.php?doc=$iddoc&cancelar=ruta&plantilla=".$detalles[0]["plantilla"]."' onclick='javascript:if(confirm(\"Esta seguro que solo usted debe firmar este documento?\")) return true; else return false;'>Firma &uacute;nica de quien prepara</a></td></tr></table></td></tr>"; 
        } 
 ...
 if(isset($unafirma) || !$ruta_actual["numcampos"]){
    ...
    echo "<tr><td colspan='4' bgcolor=\"#F5F5F5\"><a href=formatos/librerias/rutaadd.php?doc=$iddoc&origen=".usuario_actual('funcionario_codigo')."&rehacer=crear_ruta&reset_ruta=true&cargar=1  onclick='javascript:if(confirm(\"Esta seguro de que otra persona debe revisar y firmar el documento.?\")) return true; else return false;'>Reasignar Responsables</a></td><td colspan='1' bgcolor=\"#F5F5F5\" aling='right'><a href='formatos/librerias/rutaadd.php?doc=$iddoc&cancelar=ruta&plantilla=".$detalles[0]["plantilla"]."' onclick='javascript:if(confirm(\"Esta seguro que solo usted debe firmar este documento?\")) return true; else return false;'>Firma &uacute;nica de quien prepara</a></td></tr>";
  } 
      ?>	
    </table>   
    </form>
