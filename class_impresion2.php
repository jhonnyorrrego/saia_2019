...
class Imprime_Pdf {
  ...
  private $margenes = array("superior" => "10", "inferior" => "10", "izquierda" => "5", "derecha" => "10");
  ...
  function imprimir() {    
    ...
    if(@$this->versionamiento){
      ...
      $carpeta = "../" . $this->documento[0]["estado"] . "/" . $fecha[0] . "-" . $fecha[1] . "/" . $this->documento[0]["iddocumento"] . "/pdf";
      ...
      crear_destino($carpeta);
      	...
      	$nombre_pdf = $this->documento[0]["numero"] . "_" . $this->documento[0]["fecha"] . ".pdf";
				if($nombre_pdf=="_.pdf"){
					$nombre_pdf =$ruta_db_superior.'temporal_'.usuario_actual('login')."/".$_REQUEST['nombre_pdf'].date("y-m-d") . ".pdf";
				}	
			}else{
