...
function llena_formato($id) {
	...
	if ($id == "NULL") {
		$papas = busca_filtro_tabla("", "formato", $valida_item . " AND (cod_padre=0 OR cod_padre IS NULL)" . $adicionales . $formatoExcluido, "etiqueta ASC", $conn);
	} else {
		$papas = busca_filtro_tabla("", "formato", $valida_item . " AND cod_padre=" . $id . $adicionales . $formatoExcluido, "etiqueta ASC", $conn);
	}

	if ($papas["numcampos"]) {
		for ($i = 0; $i < $papas["numcampos"]; $i++) {
			$hijos = busca_filtro_tabla("count(*) as cantidad", "formato", $valida_item . " AND cod_padre=" . $papas[$i]["idformato"], "", $conn);
