...
for($i=0;$i<$recorrido["numcampos"];$i++){
    ...
    $mostrar.=('<tr'.$sItemRowClass.'><td><span class="phpmaker" ><a href="#" '.$leidos.'>'.utf8_encode(busca_entidad_ruta(1,$recorrido[$i]["origen"]))."</a></span></td>");
    ...
    if($recorrido[$i]["nombre"]=="DISTRIBUCION" && strpos($recorrido[$i]["notas"],"enviado por e-mail")===false)
      {if($documento[0]["plantilla"]=="")
         $mostrar.=('<td><span class="phpmaker" ><a href="#" '.$sig.'>'.utf8_encode(busca_entidad_ruta(2,$recorrido[$i]["destino"])).'</a></span></td>');          
       elseif($documento[0]["plantilla"]=="CARTA")
         {$destinos=busca_filtro_tabla("destinos","ft_carta","documento_iddocumento=$x_doc","",$conn);
          ...
          foreach($codigos as $filacod)
            $mostrar.= utf8_encode(busca_entidad_ruta(2,$filacod))."<br />";
          $mostrar.= ('</span></td>');
         }
       else
         {$mostrar.=('<td><span class="phpmaker" >');
          $mostrar.= utf8_encode(busca_entidad_ruta(1,$recorrido[$i]["destino"]))."<br />";
          $mostrar.= ('</span></td>');
         }  
      }
    else  
      $mostrar.=('<td><span class="phpmaker" >'.$leido.'<a href="#" '.$sig.'>'.utf8_encode(busca_entidad_ruta(1,$recorrido[$i]["destino"])).'</a></span></td>');
