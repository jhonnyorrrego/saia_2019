...
if (!isset($_REQUEST["ruta"]) || !$_REQUEST["ruta"]) {
	...
	$ruta_string = base64_decode($_REQUEST["ruta"]);
	$ruta_json = json_decode($ruta_string);
	if (is_object($ruta_json)) {
		//$_REQUEST["tipo"]=> 1=documento; 2=anexos; 3=anexos_transferencia
		$iddoc = intval($_REQUEST["iddocumento"]);
		$tipo_visor = intval($_REQUEST["tipo_visor"]);
		switch ($tipo_visor) {
			case 1 :
				$id = $iddoc;
				break;
			case 2 :
				$id = intval($_REQUEST["idanexo"]);
				break;
			case 3 :
				$id = intval($_REQUEST["idanexos_transferencia"]);
				break;
		}
		...
		if (is_file($temp_pdf)) {
			...
			file_put_contents($temp_pdf, $arr_almacen);
			if (is_file($temp_pdf)) {
				$url_pdf ="file=".$temp_pdf;
			} else {
				notificaciones("Error al intentar cargar el PDF", "error");
				redirecciona($ruta_db_superior . "vacio.php");
				die();
			}
		}
	} else {
		notificaciones("Error al resolver la ruta del pdf", "error");
		redirecciona($ruta_db_superior . "vacio.php");
		die();
	}
}
echo '<input type="hidden" name="url_pdf" id="url_pdf" value="' . $url_pdf . '">';
?>
					...
					<div id="toolbarContainer">
						<div id="toolbarViewer">
							...
							<div id="toolbarViewerMiddle">
								...
								<div class="splitToolbarButtonSeparator"></div>
								<button id="btn_notas_pdf" class="toolbarButton notas_pdf" title="Notas" data-l10n-id="notas_pdf" enlace="<?php echo($ruta_db_superior)?>visores/pdf-notas/docs/index.php?ruta=<?php echo $_REQUEST["ruta"];?>&iddoc=<?php echo $iddoc;?>&file=<?php echo $temp_relativo; ?>&tipo=<?php echo($tipo_visor); ?>&idtipo=<?php echo($id); ?>">
									<span>Notas PDF</span>
								</button>
								<div class="splitToolbarButtonSeparator"></div>
								<button id="btn_transferir" class="toolbarButton transferir" title="Transferir" data-l10n-id="transferir" enlace="<?php echo($ruta_db_superior); ?>transferenciaadd.php?iddoc=<?php echo $iddoc; ?>">
