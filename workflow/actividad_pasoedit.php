...
LoadData($_REQUEST["idactividad_paso"],$conn);
switch ($sAction)
{
	case "A": // Add
...
if($x_restrictivo == 1){
	$restrictivo1 = "checked='true'";
}
else if($x_restrictivo == 0 && $x_restrictivo != ''){
	$restrictivo2 = "checked='true'";
}

if($x_estado == 1){
	$estado1 = "checked='true'";
}
else if($x_estado == 0 && $x_estado != ''){
...
else if($x_tipo == 0 && $x_tipo != ''){
	$tipo2 = "checked='true'";
}

if($x_tipo_plazo == 'hour'){
	$tipo_plazo1 = "selected";
}
else if($x_tipo_plazo == 'day'){
	$tipo_plazo2 = "selected";
}
else if($x_tipo_plazo == 'month'){
	$tipo_plazo3 = "selected";
}
else if($x_tipo_plazo == 'year'){
	$tipo_plazo4 = "selected";
}
?>
...
function valores_entidad(identidad){
  if(identidad!=""){
    $.ajax({url: "arbol_entidades.php" ,
      data:"seleccionado=<?php echo $x_llave_entidad; ?>&entidad="+identidad+"<?php if(@$_REQUEST['tipo_entidad']) echo '&tipo_entidad='.$_REQUEST['tipo_entidad'] ; if(@$_REQUEST['llave_entidad']) echo '&llave_entidad='.$_REQUEST['llave_entidad']; ?>",
...
<p>&nbsp;&nbsp;EDITAR ACTIVIDAD<br><br><a href="actividades_paso.php?idpaso=<?php echo($_REQUEST["paso_idpaso"]);?>">Regresar al listado</a></p>
<form name="actividad_pasoedit" id="actividad_pasoedit" action="actividad_pasoedit.php" method="post" onSubmit="return EW_checkMyForm(this);">
<p>
...
<input type="hidden" name="paso_idpaso" value="<?php echo(@$x_paso_idpaso);?>">
...
<input type="text" name="x_descripcion" id="x_descripcion" size="30" maxlength="255" value="<?php echo htmlspecialchars(@$x_descripcion) ?>">
	...
	<tr>
      ...
      <span class="phpmaker" style="color: #FFFFFF;">RESTRICTIVA </span>
		...
		<td bgcolor="#F5F5F5"><span class="phpmaker">
      SI <input type="radio" name="x_restrictivo" id="x_restrictivo" value="1" <?php echo $restrictivo1; ?> >
      NO <input type="radio" name="x_restrictivo" id="x_restrictivo" value="0" <?php echo $restrictivo2; ?> ></span>
    </td>
	</tr>
  	<tr>
      ...
      <span class="phpmaker" style="color: #FFFFFF;">ESTADO</span>
		...
		<td bgcolor="#F5F5F5"><span class="phpmaker">
      ACTIVA <input type="radio" name="x_estado" id="x_estado" value="1" <?php echo $estado1; ?> >
      INACTIVA <input type="radio" name="x_estado" id="x_estado" value="0" <?php echo $estado2; ?> ></span>
    </td>
	</tr>
    <tr>
		...
		<td bgcolor="#F5F5F5"><span class="phpmaker">       
         <select name="x_orden" id="x_orden">
          ...
          for($i=0;$i<$orden["numcampos"];$i++){
          	$ordenx = '';
          	if($orden[$i]["orden"]+1 == $x_orden)
				$ordenx = "selected";
            echo "<option value='".($orden[$i]["orden"]+1)."' ".$ordenx.">".delimita($orden[$i]["descripcion"],50)."</option>";
		  }
          ?>
        </select>
            </span>
	...
	<tr>
	   ...
	   <td bgcolor="#F5F5F5"><span class="phpmaker">
        Sistema <input type="radio" name="x_tipo" id="x_tipo" value="1" <?php echo $tipo1; ?> >
        Manual <input type="radio" name="x_tipo" id="x_tipo" value="0" <?php echo $tipo2; ?> >
        </span>
       </td>
	</tr>
    <tr>
      ...
      for($i=0;$i<$accion["numcampos"];$i++){
      	$accionx = "";
		if($accion[$i]["idaccion"] == $x_accion_idaccion)
			$accionx = "selected";
        echo "<option value='".$accion[$i]["idaccion"]."' ".$accionx.">".$accion[$i]["nombre"]."</option>";
	  }
      ?>
      </select>
      <script>
	...
	function crear_arbol(accion){
		$.post('funciones_acciones_paso.php',{idaccion : accion, accion : 'arbol',campo : 'llave', idpaso_actividad : '<?php echo $x_idactividad_paso; ?>'},function(data){
			ruta_xmlx = 'test_formatos.php?seleccionado=<?php echo $x_llave_accion; ?>';
			$("#mostrar").html('');
			$("#mostrar").html(data);
			$("#mostrar").show();
		});
	...
	function llenar_valor_llave(){
		...
		document.actividad_pasoedit.submit();
	}
	</script>
	<?php
	if($x_accion_idaccion != ''){
	  	echo '<script>
	  		crear_arbol('.$x_accion_idaccion.');
	  	</script>';
	  }
	?>
</span>
<div id="mostrar_seleccionados">
<br><div id="mostrar" style="display:none"></div></td>
	</tr>	
    <tr>
          ...
          { 
           ...
           //if($entidad[$i]["identidad"]==1) 
            {$select_entidad.="<option value=".$entidad[$i]["identidad"];
             if($entidad[$i]["identidad"]==@$x_tipo_entidad)
                $select_entidad.=" selected ";
             $select_entidad.=">".ucfirst($entidad[$i]["nombre"])."</option>";
            } 
          }
          if(@$x_tipo_entidad)
            echo '<script>valores_entidad("'.$x_tipo_entidad.'");</script>';
    ...
    <tr>
        ...
        <td bgcolor="#F5F5F5"><span class="phpmaker">
            ...
            <select name="x_tipo_plazo" id="x_tipo_plazo">
              ...
              <option value="hour" <?php echo $tipo_plazo1; ?> >Hora(s)</option>
              <option value="day" <?php echo $tipo_plazo2; ?> >D&iacute;a(s)</option>
              <option value="month" <?php echo $tipo_plazo3; ?> >Mes(es)</option>
              <option value="year" <?php echo $tipo_plazo4; ?> >A&ntilde;o(s)</option>
            </select>   
            </span>
        </td>
    </tr>
        ...
        <td bgcolor="#F5F5F5">
            ...
            <script type="text/javascript">
              ...
              function fin_cargando_serie() {
                ...
                else$x_idactividad_paso if (browserType == "ie")
...
<input type="button" name="Action" value="Editar" onclick="llenar_valor_llave();">
</form>
...
$theValue = ($x_estado != "") ? $x_estado : 1;
	...
	$strsql .= "descripcion=".$fieldList["descripcion"].", 
	accion_idaccion=".$fieldList["accion_idaccion"].", restrictivo=".$fieldList["restrictivo"].", 
	estado=".$fieldList["estado"].", orden=".$fieldList["orden"].", tipo=".$fieldList["tipo"].", 
	entidad_identidad=".$fieldList["entidad_identidad"].", llave_entidad=".$fieldList["llave_entidad"].", 
	plazo=".$fieldList["plazo"].", tipo_plazo=".$fieldList["tipo_plazo"].", formato_idformato='".$fieldList["formato_idformato"]."' where idpaso_actividad=".$x_idactividad_paso;
