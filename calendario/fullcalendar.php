...
<head>
   ...
   */
    ...
    if (@$_REQUEST['nombre_calendario'] != '') {
      ...
      $idcalendario = busca_filtro_tabla("idcalendario_saia", "calendario_saia", "lower(nombre)='" . $_REQUEST['nombre_calendario'] . "'", "");
      if ($idcalendario['numcampos']) {
        $_REQUEST['idcalendario'] = $idcalendario[0]['idcalendario_saia'];
      }
    }

    $configuracion = busca_filtro_tabla(fecha_db_obtener('fecha') . ' AS fecha, tipo, estilo, datos, encabezado_izquierda, encabezado_centro, encabezado_derecho, adicionar_evento,busqueda_avanzada,nombre', "calendario_saia", "idcalendario_saia=" . $_REQUEST['idcalendario'], "");
...
<body>
  ...
  if (in_array($configuracion[0]['nombre'], $calendarios_tareas)) { ?>
    ...
    <?php
      ...
      $componente_planeador = busca_filtro_tabla("idbusqueda_componente", "busqueda_componente", "lower(nombre)='tareas_listado_paneador'", "");
      ?>
  ...
  <div id='wrap'>
    ...
    $datos_busqueda = busca_filtro_tabla("", "busqueda A,busqueda_componente B", "A.idbusqueda=B.busqueda_idbusqueda AND B.idbusqueda_componente=" . $datos_componente, "");
    ...
    $.ajax({
      ...
      data: "idbusqueda_componente=" + idcomponente + "&page=0&rows=" + $("#busqueda_registros").val() + "&actual_row=0&idbusqueda_filtro_temp=<?php echo (@$_REQUEST['idbusqueda_filtro_temp']); ?>&idbusqueda_filtro=<?php echo (@$_REQUEST['idbusqueda_filtro']); ?>&idbusqueda_temporal=<?php echo (@$_REQUEST['idbusqueda_temporal']);
                                                                                                                                                                                                                                                                                            if (@$_REQUEST['variable_busqueda']) {
                                                                                                                                                                                                                                                                                              echo ("&variable_busqueda=" . $_REQUEST['variable_busqueda']);
