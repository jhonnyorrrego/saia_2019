...
include_once($ruta_db_superior."formatos/librerias/funciones_formatos_generales.php");
	...
	$tareas=busca_filtro_tabla("A.*,A.fecha_formato,C.estado,D.fecha","ft_recordar_tarea A left join ft_avances C ON C.ft_recordar_tarea=A.idft_recordar_tarea left join ft_cierre_tarea D ON D.ft_recordar_tarea=A.idft_recordar_tarea, documento B","B.iddocumento=A.documento_iddocumento and B.estado NOT IN ('ELIMINADO','ANULADO') and B.responsable=".$rol[0][0],"GROUP BY A.documento_iddocumento",$conn);
	...
	if(!$tareas[$i]["fecha"]||$tareas[$i]["fecha"]==NULL||$tareas[$i]["fecha"]==""){
		...
		$datos.= strip_tags(html_entity_decode($tarea[$i]["idft_recordar_tarea"].'&&'.$tareas[$i]["tarea_asiganada"].'&&'.$tareas[$i]["fecha_entrega"].'&&'.$tareas[$i]["fecha_entrega"].'&&../ordenar.php?accion=mostrar&tipo_destino=1&mostrar_formato=1&key='.$tareas[$i]["documento_iddocumento"]."&&".$color."@"));
		//}
	}
	echo $datos;	
?>
