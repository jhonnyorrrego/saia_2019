		...
		e.create("tinymce.themes.AdvancedTheme", {
			...
			_nodeChanged : function(l, q, B, m, C) {
				...
				if (y.theme_advanced_path && y.theme_advanced_statusbar_location) {
					A = d.get(l.id + "_path") || d.add(l.id + "_path_row", "span", {
						id : l.id + "_path"
					});
					...
					o(function(F) {
						...
						case"i":
							...
							break;
						case"img":
							if ( w = d.getAttrib(F, "src")) {
								t += "src: " + w + " "
							}
							break;
						case"a":
							if ( w = d.getAttrib(F, "name")) {
								t += "name: " + w + " ";
								p += "#" + w
							}
					...
					if (d.select("a", A).length > 0) {
						x.statusKeyboardNavigation = new e.ui.KeyboardNavigation({
							root : l.id + "_path_row",
							items : d.select("a", A),
			...
			_mceCodeEditor : function(j, k) {
				...
				i.windowManager.open({
					...
					width : parseInt(i.getParam("theme_advanced_source_editor_width", 800)),
					height : parseInt(i.getParam("theme_advanced_source_editor_height", 500)),
					inline : true,
					resizable : true,
					maximizable : true
				}, {
