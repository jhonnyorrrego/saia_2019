...
$schema = ComponentFormGeneratorController::getSchemaFromField($CamposFormato);
...
function obtener_valores_campo($CamposFormato, $schema)
{
    $response = [];
    ...
    switch ($CamposFormato->etiqueta_html) {
        ...
        case 'checkbox':
            ...
            break;
        case 'ejecutor':
            //elimino la seleccion de opciones que ya no estan habilitadas
            $fields = ExternalUserGeneratorController::getFieldsLabel();
            $names = array_keys($fields);
            $options = json_decode($CamposFormato->opciones, true);
            $selected = explode(',', $options['adicional']);
            $accepted = array_intersect($selected, $names);
            $options['adicional'] = implode(',', $accepted);
            $response["fs_opciones"] = $options;
            break;
        default:
    ...
    if (isset($schema["data"])) {
        $response = array_merge_recursive_distinct($response, $schema["data"]);
    }
