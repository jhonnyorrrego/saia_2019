(function (modules) { // webpackBootstrap
	...
	([
		...
		function (module, exports, __webpack_require__) {
			...
			var params = JSON.parse(document.querySelector('script[data-params]').dataset.params);
			var baseUrl = document.querySelector('script[data-baseurl]').dataset.baseurl;
			var documentId = baseUrl + params.file;
			...
			window.localStorageManager = {
				...
				set: function (key, value) {
					let data = localStorage.getItem(this.prefix) || this.clean();
					...
					if (this.get('annotations')) {
						...
						$.ajax({
							...
							success: function (response) {
								if (response.success) {
									window.localStorageManager.set('annotations', response.data);
			...
			// Toolbar buttons
			(function () {
				var tooltype = window.localStorageManager.get('tooltype') || 'cursor';
				if (tooltype) {
			...
			(function () {
				function setScaleRotate(scale, rotate) {
					...
					if (RENDER_OPTIONS.scale !== scale || RENDER_OPTIONS.rotate !== rotate) {
						...
						window.localStorageManager.set('scale', RENDER_OPTIONS.scale)
						window.localStorageManager.set('rotate', RENDER_OPTIONS.rotate % 360)
			...
			(function () {
				function handleDownload() {
					$.post(`${baseUrl}app/visor/validar_descarga.php`, {
						key: localStorage.getItem('key'),
						file: params.file
					}, function (response) {
						if (response.success) {
				...
				document.querySelector('.toolbar .download').addEventListener('click', handleDownload);
			})();
			...
			(function () {
				...
				function showPage(pageId) {
					$('#content-wrapper').scrollTop((pageId - 1) * (PAGE_HEIGHT + 10));
				}

				$('.toolbar .thumbnails').on('click', function () {
					PDFJS.getDocument(THUMBNAILS_RENDER_OPTIONS.documentId).then(function (pdf) {
						...
						for (var i = 1; i <= NUM_PAGES; i++) {
							var page = UI.createPage(i + THUMBNAILS_RENDER_OPTIONS.suffix);
							thumbnails.appendChild(page);

							UI.renderPage(i, THUMBNAILS_RENDER_OPTIONS);
						}

						$('#thumbnails .page').on('click', function () {
							let pageId = $(this).data('pageNumber').replace(THUMBNAILS_RENDER_OPTIONS.suffix, '');
							showPage(pageId);
						})
					});
				});
			...
			(function () {
				$('.toolbar .print').on('click', function () {
					...
					iframe.src = baseUrl + params.file;
				});
			})();
			...
			(function (window, document) {
				...
				function handleAnnotationClick(target) {
					if (supportsComments(target)) {
						(function () {
							...
							PDFJSAnnotate.getStoreAdapter().getComments(documentId, annotationId).then(function (comments) {
								commentList.innerHTML = '';
								commentForm.style.display = '';
								commentText.focus();

								commentForm.onsubmit = function () {
									PDFJSAnnotate.getStoreAdapter().addComment(documentId, annotationId, commentText.value.trim()).then(insertComment).then(function () {
										commentText.value = '';
										commentText.focus();
									});
				...
				function handleCommentAdd(documentId, annotationId, comment) {
					adapter.getAnnotation(documentId, annotationId).then(annotation => {
						$.post(`${baseUrl}app/visor/guardar_nota.php`, {
							key: localStorage.getItem('key'),
							params: params,
			...
			function defaultActions() {
				//add window height to viewers
				let height = $(window).height() - $('#toolbar_container').height();
				$('.parent_list').height(height);
				...
				(function webpackUniversalModuleDefinition(root, factory) {
					...
					return ( /******/ function (modules) { // webpackBootstrap
						...
						function _getAnnotations(documentId) {
							return window.localStorageManager.getAnnotations(documentId)
						}

						function updateAnnotations(documentId, annotations) {
							window.localStorageManager.set('annotations', annotations);
						}
						...
						function transform(node, viewport) {
							...
							if (!isFirefox && node.nodeName.toLowerCase() === 'svg') {
								...
								transform(path, (0, _objectAssign2.default)({}, viewport, {
									scale: 1
								}));
								switch (viewport.rotation % 360) {
									case 90:
						...
						function renderPage(pageNumber, renderOptions) {
							...
							return Promise.all([pdfDocument.getPage(pageNumber), _PDFJSAnnotate2.default.getAnnotations(documentId, pageNumber)]).then(function (_ref) {
								...
								var realPageNumber = renderOptions.suffix ? pageNumber + renderOptions.suffix : pageNumber;
								var page = document.getElementById('pageContainer' + realPageNumber);
								...
								var transform = scalePage(realPageNumber, viewport, canvasContext); // Render the page
