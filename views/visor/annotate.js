(function(modules) {
    ...
    function(module, exports, __webpack_require__) {
        ...
        var NUM_PAGES = 0;
        var route = params.pdfRoute
            ? params.pdfRoute
            : findRoute(params, baseUrl);
        var RENDER_OPTIONS = {
        ...
        PDFJS.workerSrc = "../visor/shared/pdf.worker.js";
        ...
        function findRoute(params, baseUrl) {
            ...
            $.ajax({
                ...
                dataType: "json",
                async: false,
                url: `${baseUrl}app/visor/calcular_ruta.php`,
                data: data,
        ...
        function defaultActions() {
            ...
            for (var i = 2; i <= NUM_PAGES; i++) {
                UI.renderPage(i, RENDER_OPTIONS).then(function(_ref) {
                    var _ref2 = _slicedToArray(_ref, 2);
            ...
            $(".height_window").height(
                $(window).height() - $("#toolbar_container").height() - 10
            );
        }

        function createStorageManager(documentId) {
            window.localStorageManager = {
                prefix: "pdf/" + documentId,
                clean: function() {
                    localStorage.setItem(this.prefix, "{}");
                    return "{}";
                },
                set: function(key, value) {
                    let data =
                        localStorage.getItem(this.prefix) || this.clean();
                ...
                getAnnotations: function() {
                    ...
                    if (this.get("annotations")) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    window.localStorageManager.set(
                                        "annotations",
                                        response.data
                                    );
                                    annotations = window.localStorageManager.getAnnotations();
                                } else {
                                    top.notification({
                                        type: "error",
                                        message: response.message
            ...
            //remove historory
            window.localStorageManager.clean();
        }

        function defineCommentOptions(comments) {
        ...
        (function() {
            var tooltype =
                window.localStorageManager.get("tooltype") || "cursor";
            ...
            function setActiveToolbarItem(type, button) {
                ...
                if (tooltype !== type) {
                    window.localStorageManager.set("tooltype", type);
                }
                tooltype = type;
        ...
        (function() {
            function handleDownload() {
                $.post(
                    `${baseUrl}app/visor/validar_descarga.php`,
                    {
                        key: localStorage.getItem("key"),
                        file: route
                    },
            ...
            document
                .querySelector(".toolbar .download")
                .addEventListener("click", handleDownload);
        })();

        //render thumbnails
        (function() {
            var THUMBNAILS_RENDER_OPTIONS = Object.assign({}, RENDER_OPTIONS);

            function showPage(pageId) {
                $("#content-wrapper").scrollTop(
                    (pageId - 1) * (PAGE_HEIGHT + 10)
                );
            }

            $(".toolbar .thumbnails").on("click", function() {
                if (!$("#thumbnails").children().length) {
                    PDFJS.getDocument(
                        THUMBNAILS_RENDER_OPTIONS.documentId
                    ).then(function(pdf) {
                        THUMBNAILS_RENDER_OPTIONS.pdfDocument = pdf;
                        THUMBNAILS_RENDER_OPTIONS.suffix = "thumbnails";
                        THUMBNAILS_RENDER_OPTIONS.scale = 0.2;
                        NUM_PAGES = pdf.pdfInfo.numPages;

                        for (var i = 1; i <= NUM_PAGES; i++) {
                            var page = UI.createPage(
                                i + THUMBNAILS_RENDER_OPTIONS.suffix
                            );
                            $("#thumbnails").append(page);

                            UI.renderPage(i, THUMBNAILS_RENDER_OPTIONS);
                        }

                        $("#thumbnails .page").on("click", function() {
                            let pageId = $(this)
                                .data("pageNumber")
                                .replace(THUMBNAILS_RENDER_OPTIONS.suffix, "");
                            showPage(pageId);
                        });
                    });
                }
        ...
        (function() {
            $(".toolbar .print").on("click", function() {
                ...
                iframe.src = RENDER_OPTIONS.documentId;
            });
        })();
        ...
        (function(window, document) {
            var commentList = document.querySelector(
            ...
            function insertComment(comment) {
                let content = _twitterText2.default.autoLink(
                    _twitterText2.default.htmlEscape(comment.content)
                );
            ...
            function handleAnnotationClick(target) {
                if (supportsComments(target)) {
                    ...
                    var annotationId = target.getAttribute(
                            ...
                            commentForm.onsubmit = function() {
                                PDFJSAnnotate.getStoreAdapter()
                                    ...
                                    .then(function(comment) {
                                        window.localStorageManager.set("annotations",'');
                                        let annotations = window.localStorageManager.getAnnotations();
            ...
            function showComments(comments) {
                let options = defineCommentOptions(comments);
            ...
            function handleAnnotationDelete(documentId, uuid) {
                $.post(
                    `${baseUrl}app/visor/eliminar_nota.php`,
                    {
                        key: localStorage.getItem("key"),
                        uuid: uuid,
                        type: "NOTA_DOCUMENTO",
            ...
            function handleAnnotationEdit(documentId, uuid, annotation) {
                $.post(
                    `${baseUrl}app/visor/editar_nota.php`,
                    {
                        key: localStorage.getItem("key"),
                        uuid: uuid,
            ...
            UI.addEventListener("comment:add", handleCommentAdd);
        })(window, document);
    },
    /* 1 */
    function(module, exports, __webpack_require__) {
        ...
        (function() {
            ...
            twttr.txt.regexen.invalid_chars = regexSupplant(
                /[#{invalid_chars_group}]/
            );
            ...
            twttr.txt.regexen.endHashtagMatch = regexSupplant(
                /^(?:#{hashSigns}|:\/\/)/
            );
            ...
            twttr.txt.regexen.validDomainChars = regexSupplant(
                /[^#{invalidDomainChars}]/
            );
            ...
            twttr.txt.linkToUrl = function(entity, text, options) {
                ...
                var urlEntity =
                    (options.urlEntities && options.urlEntities[url]) || entity;
            ...
            twttr.txt.linkTextWithEntity = function(entity, options) {
                ...
                if (expandedUrl.indexOf(displayUrlSansEllipses) != -1) {
                    var displayUrlIndex = expandedUrl.indexOf(
                        displayUrlSansEllipses
                    );
                    var v = {
                        displayUrlSansEllipses: displayUrlSansEllipses,
                        // Portion of expandedUrl that precedes the displayUrl substring
                        beforeDisplayUrl: expandedUrl.substr(
                            0,
                            displayUrlIndex
                        ),
            ...
            twttr.txt.autoLinkEntities = function(text, entities, options) {
                ...
                options.hashtagClass =
                    options.hashtagClass || DEFAULT_HASHTAG_CLASS;
                options.hashtagUrlBase =
                    options.hashtagUrlBase ||
                    "https://twitter.com/#!/search?q=%23";
                options.cashtagClass =
                    options.cashtagClass || DEFAULT_CASHTAG_CLASS;
                options.cashtagUrlBase =
                    options.cashtagUrlBase ||
                    "https://twitter.com/#!/search?q=%24";
                options.listClass = options.listClass || DEFAULT_LIST_CLASS;
                options.usernameClass =
                    options.usernameClass || DEFAULT_USERNAME_CLASS;
                options.usernameUrlBase =
                    options.usernameUrlBase || "https://twitter.com/";
                options.listUrlBase =
                    options.listUrlBase || "https://twitter.com/";
                options.htmlAttrs = twttr.txt.extractHtmlAttrsFromOptions(
                    options
                );
                ...
                if (options.urlEntities) {
                    urlEntities = {};
                    for (
                        i = 0, len = options.urlEntities.length;
                        i < len;
                        i++
                    ) {
                        urlEntities[options.urlEntities[i].url] =
                            options.urlEntities[i];
                    }
                ...
                for (var i = 0; i < entities.length; i++) {
                    ...
                    result += nonEntity(
                        text.substring(beginIndex, entity.indices[0])
                    );

                    if (entity.url) {
                        result += twttr.txt.linkToUrl(entity, text, options);
                    } else if (entity.hashtag) {
                        result += twttr.txt.linkToHashtag(
                            entity,
                            text,
                            options
                        );
                    } else if (entity.screenName) {
                        result += twttr.txt.linkToMentionAndList(
                            entity,
                            text,
                            options
                        );
                    } else if (entity.cashtag) {
                        result += twttr.txt.linkToCashtag(
                            entity,
                            text,
                            options
                        );
            ...
            twttr.txt.autoLinkWithJSON = function(text, json, options) {
                ...
                if (json.user_mentions) {
                    for (var i = 0; i < json.user_mentions.length; i++) {
                        // this is a @mention
                        json.user_mentions[i].screenName =
                            json.user_mentions[i].screen_name;
                    }
                }
            ...
            twttr.txt.autoLinkUsernamesOrLists = function(text, options) {
                var entities = twttr.txt.extractMentionsOrListsWithIndices(
                    text
                );
                return twttr.txt.autoLinkEntities(text, entities, options);
            };
            ...
            twttr.txt.extractEntitiesWithIndices = function(text, options) {
                var entities = twttr.txt
                    ...
                    .concat(
                        twttr.txt.extractHashtagsWithIndices(text, {
            ...
            twttr.txt.extractMentions = function(text) {
                var screenNamesOnly = [],
                    screenNamesWithIndices = twttr.txt.extractMentionsWithIndices(
                        text
                    );
            ...
            twttr.txt.extractMentionsWithIndices = function(text) {
                var mentions = [],
                    mentionOrList,
                    mentionsOrLists = twttr.txt.extractMentionsOrListsWithIndices(
                        text
                    );
            ...
            twttr.txt.extractReplies = function(text) {
                ...
                var possibleScreenName = text.match(
                    twttr.txt.regexen.validReply
                );
                if (
                    !possibleScreenName ||
            ...
            twttr.txt.extractUrls = function(text, options) {
                var urlsOnly = [],
                    urlsWithIndices = twttr.txt.extractUrlsWithIndices(
                        text,
                        options
                    );
                ...
                while (twttr.txt.regexen.extractUrl.exec(text)) {
                    ...
                    if (!protocol) {
                        ...
                        var lastUrl = null,
                            asciiEndPosition = 0;
                        domain.replace(
                            twttr.txt.regexen.validAsciiDomain,
                            function(asciiDomain) {
                                ...
                                asciiEndPosition =
                                    asciiStartPosition + asciiDomain.length;
                                lastUrl = {
                                    url: asciiDomain,
                                    indices: [
                                ...
                                if (
                                    ...
                                    !asciiDomain.match(
                                        twttr.txt.regexen.invalidShortDomain
                                    )
                                ) {
                                    urls.push(lastUrl);
                                }
            ...
            twttr.txt.extractHashtags = function(text) {
                var hashtagsOnly = [],
                    hashtagsWithIndices = twttr.txt.extractHashtagsWithIndices(
                        text
                    );
            ...
            twttr.txt.extractCashtags = function(text) {
                var cashtagsOnly = [],
                    cashtagsWithIndices = twttr.txt.extractCashtagsWithIndices(
                        text
                    );
            ...
            twttr.txt.modifyIndicesFromUnicodeToUTF16 = function(
                text,
                entities
            ) {
            ...
            twttr.txt.modifyIndicesFromUTF16ToUnicode = function(
                text,
                entities
            ) {
            ...
            twttr.txt.getUnicodeTextLength = function(text) {
                return text.replace(twttr.txt.regexen.non_bmp_code_pairs, " ")
                    .length;
            };

            twttr.txt.convertUnicodeIndices = function(
                text,
                entities,
                indicesInUTF16
            ) {
                ...
                while (charIndex < text.length) {
                    if (
                        entity.indices[0] ==
                        (indicesInUTF16 ? charIndex : codePointIndex)
                    ) {
                        var len = entity.indices[1] - entity.indices[0];
                        entity.indices[0] = indicesInUTF16
                            ? codePointIndex
                            : charIndex;
            ...
            twttr.txt.hitHighlight = function(text, hits, options) {
                ...
                for (index = 0; index < flatHits.length; index += 1) {
                    ...
                    while (
                        chunk != null &&
                        hit >= prevChunksLen + chunk.length
                    ) {
                        result += chunkChars.slice(chunkCursor);
                        if (
                            startInChunk &&
                            hit === prevChunksLen + chunkChars.length
                        ) {
                            result += tag;
                            placed = true;
                        }
            ...
            twttr.txt.getTweetLength = function(text, options) {
                ...
                twttr.txt.modifyIndicesFromUTF16ToUnicode(
                    text,
                    urlsWithIndices
                );
                ...
                for (var i = 0; i < urlsWithIndices.length; i++) {
                    ...
                    textLength +=
                        urlsWithIndices[i].indices[0] -
                        urlsWithIndices[i].indices[1];

                    // Add 23 characters for URL starting with https://
                    // http:// URLs still use https://t.co so they are 23 characters as well
                    if (
                        urlsWithIndices[i].url
                            .toLowerCase()
                            .match(twttr.txt.regexen.urlHasHttps)
                    ) {
            ...
            twttr.txt.isValidUsername = function(username) {
                ...
                return (
                    extracted.length === 1 && extracted[0] === username.slice(1)
                );
            };
            ...
            twttr.txt.isValidHashtag = function(hashtag) {
                ...
                return (
                    extracted.length === 1 && extracted[0] === hashtag.slice(1)
                );
            };

            twttr.txt.isValidUrl = function(
                url,
                unicodeDomains,
                requireProtocol
            ) {
                ...
                var urlParts = url.match(
                    twttr.txt.regexen.validateUrlUnencoded
                );
                ...
                if (
                    !(
                        (!requireProtocol ||
                            (isValidMatch(
                                scheme,
                                twttr.txt.regexen.validateUrlScheme
                            ) &&
                                scheme.match(/^https?$/i))) &&
                        isValidMatch(path, twttr.txt.regexen.validateUrlPath) &&
                        isValidMatch(
                            query,
                            twttr.txt.regexen.validateUrlQuery,
                            true
                        ) &&
                        isValidMatch(
                            fragment,
                            twttr.txt.regexen.validateUrlFragment,
                            true
                        )
                    )
                ) {
                ...
                return (
                    ...
                    (!unicodeDomains &&
                        isValidMatch(
                            authority,
                            twttr.txt.regexen.validateUrlAuthority
                        ))
                );
            };
            ...
            function isValidMatch(string, regex, optional) {
                ...
                // RegExp["$&"] is the text of the last match
                return (
                    !string || (string.match(regex) && RegExp["$&"] === string)
                );
            }
                ...
                return /******/ (function(modules) {
                    ...
                    /************************************************************************/ /******/ [
                        /* 0 */ function(module, exports, __webpack_require__) {
                            ...
                            var _PDFJSAnnotate2 = _interopRequireDefault(
                                _PDFJSAnnotate
                            );
                        ...
                        /* 1 */ function(module, exports, __webpack_require__) {
                            ...
                            var _StoreAdapter2 = _interopRequireDefault(
                                _StoreAdapter
                            );
                            var _LocalStoreAdapter = __webpack_require__(8);
                            var _LocalStoreAdapter2 = _interopRequireDefault(
                                _LocalStoreAdapter
                            );
                            ...
                            exports.default = {
                                 ...
                                 */
                                setStoreAdapter: function setStoreAdapter(
                                    adapter
                                ) {
                        ...
                        /* 2 */ function(module, exports, __webpack_require__) {
                            ...
                            var _abstractFunction2 = _interopRequireDefault(
                                _abstractFunction
                            );
                                ...
                                _createClass(StoreAdapter, [
                                    {
                                        ...
                                        value: function __getAnnotations(
                                            documentId,
                                            pageNumber
                                        ) {
                                            (0, _abstractFunction2.default)(
                                                "getAnnotations"
                                            );
                                        }
                                    },
                                    ...
                                    },
                                    {
                                         ...
                                         * @return {Promise}
                                         */
                                        value: function __deleteAnnotation(
                                            documentId,
                                            annotationId
                                        ) {
                                            (0, _abstractFunction2.default)(
                                    ...
                                    },
                                    {
                                         ...
                                         * @return {Promise}
                                         */
                                        value: function __deleteComment(
                                            documentId,
                                            commentId
                                        ) {
                                            (0, _abstractFunction2.default)(
                                                ...
                                                return fn
                                                    ...
                                                    .then(function(
                                                        ...
                                                        if (
                                                            ...
                                                            annotations.annotations.forEach(
                                                                function(a) {
                        ...
                        /* 6 */ function(module, exports, __webpack_require__) {
                            ...
                            var _createStylesheet2 = _interopRequireDefault(
                                _createStylesheet
                            );
                            ...
                            var BORDER_COLOR = (exports.BORDER_COLOR =
                                "#00BFFF");
                            var userSelectStyleSheet = (0,
                            _createStylesheet2.default)({
                                body: {
                            ...
                            function getAnnotationRect(el) {
                                ...
                                if (
                                    ...
                                    result = scaleUp(
                                        findSVGAtPoint(rect.left, rect.top),
                                        result
                                    );
                                }
                                return result;
                            ...
                            function enableUserSelect() {
                                if (userSelectStyleSheet.parentNode) {
                                    userSelectStyleSheet.parentNode.removeChild(
                                        userSelectStyleSheet
                                    );
                                }
                            }
                        ...
                        /* 8 */ function(module, exports, __webpack_require__) {
                            ...
                            var _StoreAdapter3 = _interopRequireDefault(
                                _StoreAdapter2
                            );
                            ...
                            function _getAnnotations(documentId) {
                                return window.localStorageManager.getAnnotations(
                                    documentId
                                );
                            }
                        ...
                        /* 10 */ function(
                            ...
                            var _appendChild2 = _interopRequireDefault(
                                _appendChild
                            );
                            var _renderScreenReaderHints = __webpack_require__(
                                20
                            );
                            ...
                            function render(svg, viewport, data) {
                                return new Promise(function(resolve, reject) {
                                    ...
                                    data.annotations.forEach(function(a) {
                                        (0,
                                        _appendChild2.default)(svg, a, viewport);
                                    });
                                    resolve(svg);
                                });
                            }
                        ...
                        /* 11 */ function(
                            ...
                            var _renderLine2 = _interopRequireDefault(
                                _renderLine
                            );
                            var _renderPath = __webpack_require__(16);
                            var _renderPath2 = _interopRequireDefault(
                                _renderPath
                            );
                            var _renderPoint = __webpack_require__(17);
                            var _renderPoint2 = _interopRequireDefault(
                                _renderPoint
                            );
                            var _renderRect = __webpack_require__(18);
                            var _renderRect2 = _interopRequireDefault(
                                _renderRect
                            );
                            var _renderText = __webpack_require__(19);
                            var _renderText2 = _interopRequireDefault(
                                _renderText
                            );
                            ...
                            function transform(node, viewport) {
                                ...
                                if (
                                    ...
                                    node.setAttribute(
                                        "x",
                                        parseInt(node.getAttribute("x"), 10) *
                                            viewport.scale
                                    );
                                    node.setAttribute(
                                        "y",
                                        parseInt(node.getAttribute("y"), 10) *
                                            viewport.scale
                                    );
                                    var x = parseInt(
                                        node.getAttribute("x", 10)
                                    );
                                    var y = parseInt(
                                        node.getAttribute("y", 10)
                                    );
                                    var width = parseInt(
                            ...
                            function appendChild(svg, annotation, viewport) {
                                ...
                                switch (annotation.type) {
                                    case "area":
                                    case "highlight":
                                        child = (0, _renderRect2.default)(
                                            annotation
                                        );
                                        break;
                                    case "strikeout":
                                        child = (0, _renderLine2.default)(
                                            annotation
                                        );
                                        break;
                                    case "point":
                                        child = (0, _renderPoint2.default)(
                                            annotation
                                        );
                                        break;
                                    case "textbox":
                                        child = (0, _renderText2.default)(
                                            annotation
                                        );
                                        break;
                                    case "drawing":
                                        child = (0, _renderPath2.default)(
                                            annotation
                                        );
                                        break;
                        ...
                        /* 13 */ function(
                            ...
                            var _setAttributes2 = _interopRequireDefault(
                                _setAttributes
                            );
                            ...
                            function renderLine(a) {
                                ...
                                (0, _setAttributes2.default)(group, {
                                    stroke: (0, _normalizeColor2.default)(
                                        a.color || "#f00"
                                    ),
                                    strokeWidth: 1
                                });
                        ...
                        /* 14 */ function(module, exports) {
                            ...
                            var keyCase = function keyCase(key) {
                                if (BLACKLIST.indexOf(key) === -1) {
                                    key = key.replace(UPPER_REGEX, function(
                                        match
                                    ) {
                                        return "-" + match.toLowerCase();
                                    });
                        ...
                        /* 17 */ function(
                            ...
                            function renderPoint(a) {
                                var outerSVG = document.createElementNS(
                                    "http://www.w3.org/2000/svg",
                                    "svg"
                                );
                            ...
                            function renderRect(a) {
                                if (a.type === "highlight") {
                                    var _ret = (function() {
                                        ...
                                        (0, _setAttributes2.default)(group, {
                                            fill: (0, _normalizeColor2.default)(
                                                a.color || "#ff0"
                                            ),
                                            fillOpacity: 0.2
                                        });
                        ...
                        /* 20 */ function(
                            ...
                            var _insertScreenReaderHint = __webpack_require__(
                                21
                            );
                            ...
                            var _initEventHandlers2 = _interopRequireDefault(
                                _initEventHandlers
                            );
                                ...
                                Object.keys(SORT_TYPES).forEach(function(type) {
                                    ...
                                    annotations
                                        ...
                                        .forEach(function(a, i) {
                                            return (0,
                                            _insertScreenReaderHint2.default)(a, i + 1);
                                        });
                                });
                        ...
                        /* 21 */ function(
                            ...
                            var _createScreenReaderOnly = __webpack_require__(
                                22
                            );
                            var _createScreenReaderOnly2 = _interopRequireDefault(
                                _createScreenReaderOnly
                            );
                            var _insertElementWithinChildren = __webpack_require__(
                                23
                            );
                            ...
                            var _renderScreenReaderComments = __webpack_require__(
                                25
                            );
                            ...
                            function insertScreenReaderHint(annotation) {
                                ...
                                switch (annotation.type) {
                                    ...
                                    case "strikeout":
                                        ...
                                        (0,
                                        _insertElementWithinElement2.default)(
                                            (0,
                                            _createScreenReaderOnly2.default)(
                                                "Begin " +
                                                    annotation.type +
                                                    " annotation " +
                                                    num,
                                    ...
                                    case "point":
                                        var text =
                                            annotation.type === "textbox"
                                                ...
                                                : "";
                                        (0,
                                        _insertElementWithinChildren2.default)(
                                            (0,
                                            _createScreenReaderOnly2.default)(
                                                annotation.type +
                                                    " annotation " +
                        ...
                        /* 23 */ function(
                             ...
                             */
                            function insertElementWithinChildren(
                                el,
                                x,
                                y,
                                pageNumber
                        ...
                        /* 24 */ function(
                            ...
                            function insertElementWithinElement(
                                ...
                                x = Math.max(
                                    x + OFFSET_ADJUST * (insertBefore ? 1 : -1),
                                    0
                                );
                                var node = textLayerElementFromPoint(
                                    x,
                                    y + OFFSET_ADJUST,
                                    pageNumber
                                );
                                ...
                                while (head.length) {
                                    ...
                                    var width = (0, _utils.scaleDown)(svg, {
                                        width: temp.getBoundingClientRect()
                                            .width
                                    }).width;
                                    if (left + width <= x) {
                                        break;
                             ...
                             */
                            function textLayerElementFromPoint(
                                x,
                                y,
                                pageNumber
                            ) {
                        ...
                        /* 25 */ function(
                            ...
                            var _insertScreenReaderComment = __webpack_require__(
                                26
                            );
                            ...
                            function renderScreenReaderComments(
                                ...
                                return promise.then(function(comments) {
                                    ...
                                    if (node) {
                                        ...
                                        list.setAttribute(
                                            "id",
                                            "pdf-annotate-screenreader-comment-list-" +
                                                annotationId
                                        );
                                        list.setAttribute(
                                            "aria-label",
                                        ...
                                        comments.forEach(
                                            _insertScreenReaderComment2.default
                                        );
                                    }
                                });
                            }
                        ...
                        /* 26 */ function(module, exports) {
                            ...
                            function insertScreenReaderComment(comment) {
                                ...
                                if (list) {
                                    ...
                                    item.setAttribute(
                                        "id",
                                        "pdf-annotate-screenreader-comment-" +
                                            comment.uuid
                                    );
                                    item.appendChild(
                        ...
                        /* 27 */ function(
                             ...
                             */
                            function reorderAnnotationsByType(
                                documentId,
                                pageNumber,
                                type
                            ) {
                            ...
                            function insertComment(
                                ...
                                if (!list) {
                                    promise = (0,
                                    _renderScreenReaderComments2.default)(
                                        documentId,
                                        annotationId,
                                        []
                                    ).then(function() {
                                ...
                                promise.then(function() {
                                    (0,
                                    _insertScreenReaderComment2.default)(comment);
                                });
                            }
                            /**
                        ...
                        /* 29 */ function(
                            ...
                            function createEditOverlay(target) {
                                ...
                                var parentNode = (0, _utils.findSVGContainer)(
                                    target
                                ).parentNode;
                                var id = target.getAttribute(
                                    "data-pdf-annotate-id"
                                );
                                var rect = (0, _utils.getAnnotationRect)(
                                    target
                                );
                            ...
                            function deleteAnnotation() {
                                ...
                                var annotationId = overlay.getAttribute(
                                    "data-target-id"
                                );
                                ...
                                []
                                    .concat(_toConsumableArray(nodes))
                                    .forEach(function(n) {
                            ...
                            function handleDocumentMousemove(e) {
                                ...
                                if (
                                    y > minY &&
                                    y + overlay.offsetHeight < maxY
                                ) {
                                    overlay.style.top = y + "px";
                                }
                                if (
                                    x > minX &&
                                    x + overlay.offsetWidth < maxX
                                ) {
                            ...
                            function handleDocumentMouseup(e) {
                                ...
                                var type = target[0].getAttribute(
                                    "data-pdf-annotate-type"
                                );
                                var svg = overlay.parentNode.querySelector(
                                    "svg.annotationLayer"
                                ...
                                function getDelta(propX, propY) {
                                    return calcDelta(
                                        parseInt(
                                            target[0].getAttribute(propX),
                                            10
                                        ),
                                        parseInt(
                                            target[0].getAttribute(propY),
                                            10
                                        )
                                    );
                                }
                                            ...
                                            (function() {
                                                ...
                                                annotation.lines.forEach(
                                                    function(line, i) {
                                                        ...
                                                        var x =
                                                            _annotation$lines$i[0];
                                                        var y =
                                                            _annotation$lines$i[1];
                                                        annotation.lines[i][0] =
                                                            x + deltaX;
                                                        annotation.lines[i][1] =
                                                            y + deltaY;
                                                    }
                                                );
                                                target[0].parentNode.removeChild(
                                                    target[0]
                                                );
                                ...
                                if (
                                    ...
                                    _PDFJSAnnotate2.default
                                        ...
                                        .then(function(annotation) {
                                            ...
                                            (0,
                                            _appendChild2.default)(svg, annotation);
                                        });
                                }
                                document.removeEventListener(
                                    "mousemove",
                            ...
                            function savePoint() {
                                if (input.value.trim().length > 0) {
                                    var _ret = (function() {
                                        ...
                                        var clientY = parseInt(
                                            input.style.top,
                                            10
                                        );
                                        var content = input.value.trim();
                                        var svg = (0, _utils.findSVGAtPoint)(
                                            clientX,
                                            clientY
                                        );
                                        if (!svg) {
                                        ...
                                        var rect = svg.getBoundingClientRect();
                                        var _getMetadata = (0,
                                        _utils.getMetadata)(svg);
                                        var documentId =
                                            _getMetadata.documentId;
                                        var pageNumber =
                                            _getMetadata.pageNumber;
                        ...
                        /* 32 */ function(
                            ...
                            function handleDocumentMousemove(e) {
                                ...
                                var rect = svg.getBoundingClientRect();
                                if (
                                    originX + (e.clientX - originX) <
                                    rect.right
                                ) {
                                    overlay.style.width =
                                        e.clientX - originX + "px";
                                }
                                if (
                                    originY + (e.clientY - originY) <
                                    rect.bottom
                                ) {
                                    overlay.style.height =
                                        e.clientY - originY + "px";
                                }
                            }
                            /**
                             * Handle document.mouseup event
                            ...
                            function handleDocumentMouseup(e) {
                                ...
                                if (
                                    ...
                                    var _svg = overlay.parentNode.querySelector(
                                        "svg.annotationLayer"
                                    );
                        ...
                        /* 34 */ function(
                            ...
                            function renderPage(pageNumber, renderOptions) {
                                ...
                                return Promise.all([
                                    ...
                                    _PDFJSAnnotate2.default.getAnnotations(
                                        documentId,
                                        pageNumber
                                    )
                                ]).then(function(_ref) {
                                    ...
                                    var realPageNumber = renderOptions.suffix
                                        ? pageNumber + renderOptions.suffix
                                        : pageNumber;
                                    var page = document.getElementById(
                                        "pageContainer" + realPageNumber
                                    );
                                    var svg = page.querySelector(
                                        ".annotationLayer"
                                    );
                                    var canvas = page.querySelector(
                                            ...
                                            return pdfPage
                                                ...
                                                .then(function(textContent) {
                                                    return new Promise(function(
                                                        ...
                                                        var textLayer = page.querySelector(
                                                            ".textLayer"
                                                        );
                                                        ...
                                                        textLayerBuilder.setTextContent(
                                                            textContent
                                                        );
                            ...
                            function scalePage(pageNumber, viewport, context) {
                                ...
                                var wrapper = page.querySelector(
                                    ".canvasWrapper"
                                );
                                var textLayer = page.querySelector(
                                    ".textLayer"
                                ...
                                canvas.style.width =
                                    roundToDivide(viewport.width, sfx[1]) +
                                    "px";
                                canvas.style.height =
                                    roundToDivide(viewport.height, sfx[1]) +
                                    "px";
