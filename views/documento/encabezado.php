...
function getTransfer($transferId)
{
    ...
    if ($transferId) {
        $findTransfer = Model::getQueryBuilder()
            ->select('*')
            ->from('buzon_salida')
            ->where('idtransferencia = :transferId')
            ->setParameter(':transferId', $transferId, 'integer')
            ->execute()->fetch();

        $code = $findTransfer['origen'] != $userCode ?
            $findTransfer['origen'] : $findTransfer['destino'];
        $Funcionario = Funcionario::findByAttributes([
            'funcionario_codigo' => $code
        ]);

        $Response = (object) $findTransfer;
    ...
    <div class="row mx-0 px-1">
        <div class="col-12 p-0 m-0" id="document_information">
            ...
            <div class="row px-0 mx-0 py-2">
                ...
                <div class="col-auto col-md pr-0 d-flex align-items-center">
                    ...
                    <span class="px-1 cursor fa fa-history f-20" id="show_versions" data-toggle="tooltip" data-placement="bottom" title="Versiones"></span>
                    <?php if ($Documento->estado_aprobacion) : ?>
                        <span class="label" id="document_state"><?= $Documento->estado_aprobacion ?></span>
                    <?php endif; ?>
                </div>
