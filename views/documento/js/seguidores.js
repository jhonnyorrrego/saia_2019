$(function() {
    ...
    function findFollowers(documentId) {
        $.post(
            `${params.baseUrl}app/documento/seguidores.php`,
            {
                key: localStorage.getItem('key'),
    ...
    function showFollowers(data) {
        $('#follower_list').empty();
        data.forEach(i => {
            let deleteTemplate = i.delete
                ? `<span class="float-right pr-3 delete_follower" data-user="${
                      i.user
                  }">
            ...
            var template = `<div class="row mx-0 py-1">
                    <div class="col px-0 bg-master-lighter" style="border-radius:5px">
                        <div class="media">
                            <img class="align-self-center ml-1 mr-2 my-2 rounded-circle" src="${params.baseUrl +
    ...
    function modifyFollower(user, remove = 0) {
        $.post(
            `${params.baseUrl}app/documento/guardar_seguidor.php`,
            {
                key: localStorage.getItem('key'),
                user: user,
                remove: remove,
