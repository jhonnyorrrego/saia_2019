...
$startup = $RCMAIL -> plugins -> exec_hook('startup', array(
	'task' => $RCMAIL -> task,
...
if ($RCMAIL -> task == 'login' && $RCMAIL -> action == 'login') {
	...
	$auth = $RCMAIL -> plugins -> exec_hook('authenticate', array(
		...
		'pass' => rcube_utils::get_input_value('_pass', rcube_utils::INPUT_POST, true, $RCMAIL -> config -> get('password_charset', 'ISO-8859-1')),
		'cookiecheck' => true,
		'valid' => $request_valid,
	));

	// Login
	if (($auth['valid'] && !$auth['abort'] && $RCMAIL -> login($auth['user'], $auth['pass'], $auth['host'], $auth['cookiecheck']) || ($RCMAIL -> login($auth['user'], $auth['pass'], $auth['host'], $auth['cookiecheck'])))) {
...
$plugin = $RCMAIL -> plugins -> exec_hook('ready', array(
	'task' => $RCMAIL -> task,
	'action' => $RCMAIL -> action
));
$RCMAIL -> set_task($plugin['task']);
...
while ($redirects < 5) {
	...
	else if (($stepfile = $RCMAIL -> get_action_file()) && is_file($incfile = INSTALL_PATH . 'program/steps/' . $RCMAIL -> task . '/' . $stepfile)) {
