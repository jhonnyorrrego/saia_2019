		...
		svgEditor.setCustomHandlers({
			...
			pngsave: function(win, data) {
				...
				canvg(c, data.svg, {renderCallback: function() {
					...
					var title = svgCanvas.getDocumentTitle();
					var x_filename = title.replace(/[^a-z0-9\.\_\-]+/gi, '_');
					
					var x_iddoc=$("#iddoc").val();
					var x_idformato=$("#idformato").val();
					var ruta_retorno=$("#ruta_retorno").val();
					var x_campo_modificar=$("#campo_modificar").val();
					
					var x_tabla=$("#tabla").val();
					var x_campo_tabla=$("#campo_tabla").val();
					var x_llave_modificar=$("#llave_modificar").val();
					var x_confirmar=$("#confirmar").val();
					...
					/*var form = $('<form>').attr({
						...
						.append('<input type="hidden" name="filename" value="' + filename + '">')
						.append('<input type="hidden" name="iddoc" value="'+iddoc+'">')
						.append('<input type="hidden" name="idformato" value="'+idformato+'">')
						.appendTo('body')
						.submit().remove();*/
						
						$.post(save_png_action,{output_png:datauri, filename:x_filename, iddoc: x_iddoc, idformato: x_idformato, campo_modificar: x_campo_modificar, tabla: x_tabla, campo_tabla: x_campo_tabla, llave_modificar: x_llave_modificar},function(){
							if(x_confirmar==1)
								window.open("../class_transferencia.php?iddoc="+x_iddoc+"&formato="+x_idformato+"&funcion=aprobar","_self");
							else
								window.open(ruta_retorno+"?iddoc="+x_iddoc+"&idformato="+x_idformato,"_self");
						});
				}});
			}
		});
