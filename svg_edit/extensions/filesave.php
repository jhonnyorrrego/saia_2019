...
if($suffix == '.svg') {
	...
	crear_destino($ruta_db_superior.$ruta_guardar);
	$aleatorio=rand(1,5999);
	$ruta_guardar2=$ruta_guardar.$aleatorio.".png";
	
	file_put_contents($ruta_db_superior.$ruta_guardar2,$contents);
	
	$input_file=$ruta_db_superior.$ruta_guardar2;
	$output_file=$ruta_db_superior.$ruta_guardar.$aleatorio.".jpg";
	...
	$campo="firma_externa";
	if(@$_REQUEST["campo_modificar"])$campo=$_REQUEST["campo_modificar"];
	...
	if(@$_REQUEST["campo_tabla"])$campo_tabla=$_REQUEST["campo_tabla"];
	$llave_modificar=$_REQUEST["iddoc"];
	if(@$_REQUEST["llave_modificar"])$llave_modificar=$_REQUEST["llave_modificar"];
	$sql1="update ".$tabla." set ".$campo."='".$ruta_guardar.$aleatorio.".jpg"."' where ".$campo_tabla."=".$llave_modificar;
	phpmkr_query($sql1);
}
