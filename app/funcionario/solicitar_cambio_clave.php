...
if ($_REQUEST['username']) {
    ...
    if ($total) {
        ...
        if ($users) {
            $message = $_REQUEST['message'] ?? 'Por favor restablecer clave para ingreso al Sistema SAIA del usuario ' . $_REQUEST['username'];

            foreach ($users as $key => $Funcionario) {
                $emails[] = $Funcionario->email;
            }

            enviar_mensaje('', ['para' => 'email'], ['para' => $emails], 'Restablecer clave de acceso.', $message);
