...
$sSqlWrk = "SELECT A.idserie, A.codigo, A.nombre FROM serie A where estado=1" . " ORDER BY A.cod_padre Asc";
...
if ($rswrk) {
	...
	while ($datawrk = phpmkr_fetch_array($rswrk)) {
		...
		$x_cod_padreList .= ">" . $datawrk["codigo"] . ValueSeparator($rowcntwrk) . $datawrk["nombre"] . "</option>";
		$rowcntwrk++;
	}
}
