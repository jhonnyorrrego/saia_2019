<html>
  ...
  $(document).ready(function() {
			...
			<body bgcolor="#F5F5F5"><?php llama_funcion_accion(@$_REQUEST["iddoc"],@$_REQUEST["idformato"],"ingresar","ANTERIOR");?><form name="formulario_formatos" id="formulario_formatos" method="post" action="../../class_transferencia.php" enctype="multipart/form-data"><table width="100%" cellspacing="1" cellpadding="4"><tr><td colspan="2" class="encabezado_list">RIESGOS</td></tr><input type="hidden" name="estado_documento" value="<?php echo(validar_valor_campo(6458)); ?>"><input type="hidden" name="riesgo_antiguo" value="<?php echo(validar_valor_campo(6356)); ?>"><tr id="tr_identificacion_riesg">
                     ...
                     <?php buscar_dependencia(499,6358);?></tr><tr id="tr_fecha_riesgo">
                     <td class="encabezado" width="20%" title="">FECHA*</td>
                     <?php fecha_bloqueada(499,6359);?></tr><tr id="tr_estado" >
                     <td class="encabezado" width="20%" title="Estado Actual: ELABORACION,INACTIVO ">ESTADO*</td><td bgcolor="#F5F5F5"><?php genera_campo_listados_editar(499,6360,$_REQUEST['iddoc']);?></td></tr><input type="hidden" name="serie_idserie" value="<?php echo(validar_valor_campo(6361)); ?>"><tr id="tr_consecutivo">
                     <td class="encabezado" width="20%" title="">NUMERO*</td>
                     <?php consecutivo_riesgo(499,6362);?></tr><tr id="tr_nombre">
                     <td class="encabezado" width="20%" title="Actividad">ACTIVIDAD*</td>
                     <td class="celda_transparente"><textarea  tabindex='1'  name="nombre" id="nombre" cols="53" rows="3" class="tiny_basico required"><?php echo(validar_valor_campo(6363)); ?></textarea></td>
								...
								<a href="javascript:void(0)" onclick="tree_area_responsable.findItem((document.getElementById('stext_area_responsable').value))">
									<img src="../../botones/general/siguiente.png"border="0px"></a><br/><input type="hidden" maxlength="255"  class="required"  name="area_responsable" id="area_responsable"   value="" ><label style="display:none" class="error" for="area_responsable">Campo obligatorio.</label><div id="esperando_area_responsable">
                     ...
                     <td class="celda_transparente"><textarea  tabindex='3'  name="riesgo" id="riesgo" cols="53" rows="3" class="tiny_basico required"><?php echo(validar_valor_campo(6365)); ?></textarea></td>
                    </tr><input type="hidden" name="controles" value="<?php echo(validar_valor_campo(6366)); ?>"><tr id="tr_descripcion">
                     ...
                     <td class="celda_transparente"><textarea  tabindex='4'  name="descripcion" id="descripcion" cols="53" rows="3" class="tiny_basico required"><?php echo(validar_valor_campo(6367)); ?></textarea></td>
...
CORRUPCI&Oacute;N: Se entiende por Riesgo de Corrupci&oacute;n la posibilidad de que por acci&oacute;n u omisi&oacute;n, mediante el uso indebido del poder, de los recursos o de la informaci&oacute;n, se lesionen los intereses de una entidad y en consecuencia del Estado, para la obtenci&oacute;n de un beneficio particular.">TIPO DE RIESGO*</td><td bgcolor="#F5F5F5"><?php genera_campo_listados_editar(499,6368,$_REQUEST['iddoc']);?></td></tr><tr id="tr_fuente_causa">
                     ...
                     <td class="celda_transparente"><textarea  tabindex='5'  name="fuente_causa" id="fuente_causa" cols="53" rows="3" class="tiny_basico required"><?php echo(validar_valor_campo(6369)); ?></textarea></td>
                     ...
                     <td class="celda_transparente"><textarea  tabindex='6'  name="consecuencia" id="consecuencia" cols="53" rows="3" class="tiny_basico required"><?php echo(validar_valor_campo(6370)); ?></textarea></td>
                    </tr><input type="hidden" name="opciones_manejo" value="<?php echo(validar_valor_campo(6371)); ?>"><input type="hidden" name="acciones" value="<?php echo(validar_valor_campo(6372)); ?>"><tr id="tr_analisis_riego">
                     <td class="encabezado" width="20%" title="" colspan="2" id="analisis_riego"><h1><center><strong>ANALISIS DE RIESGO</strong></center></h1></td>
                    </tr><input type="hidden" name="responsables" value="<?php echo(validar_valor_campo(6374)); ?>"><input type="hidden" name="indicador" value="<?php echo(validar_valor_campo(6375)); ?>"><input type="hidden" name="cronograma" value="<?php echo(validar_valor_campo(6376)); ?>"><tr id="tr_impacto" >
...
">IMPACTO</td><td bgcolor="#F5F5F5"><?php genera_campo_listados_editar(499,6377,$_REQUEST['iddoc']);?></td></tr><?php if($_REQUEST["padre"]) {?><input type="hidden"  name="ft_proceso"  value="<?php echo $_REQUEST["padre"]; ?>"><?php } ?><?php if($_REQUEST["anterior"]) {?><input type="hidden"  name="ft_proceso"  value="<?php echo $_REQUEST["anterior"]; ?>"><?php }  else {listar_select_padres(ft_proceso);} ?><tr id="tr_probabilidad" >
...
">PROBABILIDAD*</td><td bgcolor="#F5F5F5"><?php genera_campo_listados_editar(499,6379,$_REQUEST['iddoc']);?></td></tr><input type="hidden" name="idft_riesgos_proceso" value="<?php echo(validar_valor_campo(6380)); ?>"><input type="hidden" name="documento_iddocumento" value="<?php echo(validar_valor_campo(6381)); ?>"><input type="hidden" name="encabezado" value="<?php echo(validar_valor_campo(6382)); ?>"><input type="hidden" name="firma" value="<?php echo(validar_valor_campo(6383)); ?>"><?php selecion_tipo_riesgo(499,NULL);?><input type="hidden" name="campo_descripcion" value="6365,6367"><input type="hidden" name="padre" value="<?php echo $_REQUEST["padre"]; ?>"><input type="hidden" name="anterior" value="<?php echo $_REQUEST["anterior"]; ?>"><input type="hidden" name="accion" value="guardar_detalle" ><tr>
			<td colspan='2'><?php submit_formato(499);?></td>
		</tr>
		</table></form>
</body>
		</html>