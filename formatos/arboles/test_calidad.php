...
function crear_bases_calidad() {
	global $conn, $texto;
	$bases_calidad = busca_filtro_tabla("b.iddocumento,c.etiqueta,c.idformato,c.nombre,c.nombre_tabla,a.tipo_base_calidad,a.idft_bases_calidad", "ft_bases_calidad a, documento b, formato c", "lower(b.plantilla)=c.nombre AND a.documento_iddocumento=b.iddocumento AND b.estado not in ('ELIMINADO','ANULADO','ACTIVO')", "", $conn);
...
function llena_datos($idformato, $tabla, $campo, $categoria) {//--
	...
	if ($campo_ordenar["numcampos"]) { $orden = "a." . $campo_ordenar[0]["nombre"] . " asc";
	} else
		$orden = "id$tabla asc";
