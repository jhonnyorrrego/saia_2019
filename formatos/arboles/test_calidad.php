<?php $max_salida=10; // Previene algun posible ciclo infinito limitando a 10 los ../
...
//redirecciona('test_cambio_calidad.php');
...
$formato=busca_filtro_tabla("A.idformato,A.nombre,A.nombre_tabla,A.etiqueta","formato A","A.nombre_tabla LIKE '".$nombre."'","nombre DESC",$conn);
  if($formato["numcampos"]){
    ...
    llenar_documentos($iddoc,$where);
    $texto.="</item>\n";
  }
}
function llenar_documentos($iddoc,$where){
global $conn,$texto;
  $arreglo=explode("-",$iddoc);
if($arreglo[2]!='ft_proceso'){
	$where='';
}
  ...
  for($i=0;$i<$formato["numcampos"];$i++){
      $papas=busca_filtro_tabla("id".$arreglo[2]." AS llave,'".$arreglo[2]."' AS nombre_tabla",$arreglo[2],"documento_iddocumento=".$formato[$i]["iddocumento"].$where,"",$conn);
    //print_r($papas);
    if($papas["numcampos"])
...
?>
<?php
function llena_datos_formato($formato,$estado=0,$where=''){
...
if($formato["numcampos"]){
  ...
  llena_datos($idformato,$formato[0]["nombre_tabla"],$campo_descripcion,$where);
  if($estado)
    $texto.="</item>\n";
...
function llenar_macro(){
global $conn,$texto,$imagenes,$formatos_calidad;
$formato=busca_filtro_tabla("","formato","nombre_tabla='ft_macroproceso'","",$conn);
...
for($i=0;$i<$macro["numcampos"];$i++){
  ...
  $imagenes='im0="'.strtolower($formato[0]["nombre"]).'.gif" im1="'.strtolower($formato[0]["nombre"]).'.gif" im2="'.strtolower($formato[0]["nombre"]).'.gif" ';
  $texto.='<item style="font-family:verdana; font-size:7pt;" '.$imagenes;
  $texto.=strip_tags('text="'.decodifica(($marco[0]["nombre"])).'" id="'.$macro[0]["idft_macroproceso"]."-".$formato[0]["idformato"]."-r".rand().'">'."\n");
  llena_documento($formato[0]["idformato"]."-".strtolower($formato[0]["nombre"])."-".strtolower($formato[0]["nombre_tabla"])," AND (macroproceso=".$macro[$i]["idft_macroproceso"].")");
  $texto.="</item>\n";
  /*Aqui se deben adicionar los formatos o consideraciones adicionales para el arbol de calidad. Especificamente la parte de Planes de Mejoramiento para los procesos*/
