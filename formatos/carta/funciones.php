...
function mostrar_anexos_externa($idformato, $iddoc)
{
	...
	$fisicos = ComponentFormGeneratorController::callShowValue($idformato, $iddoc, $CamposFormato->nombre);

	$CamposFormato = CamposFormato::findByAttributes([
		'formato_idformato' => $idformato,
		'nombre' => 'anexos_digitales'
	]);
	$digitales = ComponentFormGeneratorController::callShowValue($idformato, $iddoc, $CamposFormato->nombre);
...
function mostrar_cop_interna_externa($copiainterna)
{
	...
	$lista = array();
	for ($i = 0; $i < count($destinos); $i++) {
		if (strpos($destinos[$i], "#") > 0) { //si el destino es una dependencia
