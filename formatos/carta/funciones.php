...
{if($iddoc==NULL)
   {echo '<td bgcolor="#F5F5F5">
    ...
    <hr />
    <iframe name="frame_copias" id="frame_copias" src="'.compara_ruta_archivos('/'.RUTA_SCRIPT.'/' . FORMATOS_CLIENTE . 'carta/funciones_adicionales.php').'?funcion=elegir_destinos&copia=1" width=100% height=190px class=phpmkr border=0 frameborder="0" y framespacing="0">
    </iframe></td>';
   }
 else
 {echo '<td bgcolor="#F5F5F5"><iframe src="'.compara_ruta_archivos('/'.RUTA_SCRIPT.'/' . FORMATOS_CLIENTE . 'carta/funciones_adicionales.php').'?funcion=editar_copias&iddoc='.$iddoc.'&tabla=ft_carta" width=100% height=170px class=phpmkr border=0 frameborder="0" framespacing="0" >
            </iframe></td>';
   }
}
...
{global $conn;
 ...
 if($iddoc==NULL)
    {echo '<td bgcolor="#F5F5F5">
     ...
     <hr />
     <iframe name="frame_destinos" id="frame_destinos" src="'.compara_ruta_archivos('/'.RUTA_SCRIPT.'/' . FORMATOS_CLIENTE . 'carta/funciones_adicionales.php').'?funcion=elegir_destinos" width=100% height=210px class=phpmkr border=0 frameborder="0" y framespacing="0">
     </iframe></td>';
    }
 else
 {echo '<td bgcolor="#F5F5F5"><iframe src="'.compara_ruta_archivos('/'.RUTA_SCRIPT.'/' . FORMATOS_CLIENTE . 'carta/funciones_adicionales.php').'?funcion=editar_destinos&iddoc='.$iddoc.'&tabla='.$tabla[0]["nombre_tabla"].'" width=100% height=210px class=phpmkr border=0 frameborder="0" framespacing="0" >
            </iframe></td>';
    }
}
 ...
 $.ajax({url: "carga_remitentes.php",
   ...
   if(msg==0)
     ...
     {vector=msg.split('|');
      ...
      document.getElementById("frame_destinos").src="../../<?php echo FORMATOS_SAIA;?>librerias/acciones_ejecutor.php?formulario_autocompletar=formulario_formatos&campo_autocompletar=copia&tabla=ft_carta&campos_auto=nombre,identificacion&tipo=multiple&campos=cargo,empresa,direccion,telefono,email,titulo,ciudad&destinos="+vector[1];
     }
  }
  });
});
