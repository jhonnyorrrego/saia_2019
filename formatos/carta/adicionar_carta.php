<?php
                        ...
                        <!DOCTYPE html>
                            <html>
                                <head>
                                    ...
                                    <meta charset="utf-8" />
                                    <title>.:ADICIONAR COMUNICACI&Oacute;N EXTERNA:.</title>
                                    <meta name="viewport"
                	...
                	<div class="container-fluid container-fixed-lg col-lg-8" style="overflow: auto;" id="content_container">
                       ...
                       <form name="formulario_formatos" id="formulario_formatos" role="form" autocomplete="off" method="post" action="<?= $ruta_db_superior ?>class_transferencia.php"" enctype="multipart/form-data"><input type="hidden" name="estado_documento" value="<?php echo(validar_valor_campo(4793)); ?>"><input type="hidden" name="expediente_serie" value="<?php echo(validar_valor_campo(5071)); ?>"><input type="hidden" name="tipo_copia_interna" value="<?php echo(validar_valor_campo(3421)); ?>"><div class="form-group" id="tr_fecha_carta"><label title="Fecha en la que fue Creada la Carta.">FECHA DE CREACION*</label><?php fecha_formato(1,3);?></div><input type="hidden" name="firma" value="<?php echo(validar_valor_campo(15)); ?>"><div class="form-group" id="tr_destinos">
                                        <label title="">DESTINOS*</label>
                                        <input type="hidden" maxlength="2000"  class="required"  name="destinos" id="destinos" value=""><?php componente_ejecutor("9",@$_REQUEST["iddoc"]); ?></div><input type="hidden" name="encabezado" value="<?php echo(validar_valor_campo(14)); ?>"><input type="hidden" name="idft_carta" value="<?php echo(validar_valor_campo(17)); ?>"><div class="form-group" id="tr_dependencia"><label title="">DEPENDENCIA*</label><?php buscar_dependencia(1,13);?></div><input type="hidden" name="documento_iddocumento" value="<?php echo(validar_valor_campo(16)); ?>"><div class="form-group  " id="tr_requiere_recogida">
                            <label title="">REQUIERE RECOGIDA?</label><?php genera_campo_listados_editar(1,5201,$_REQUEST['iddoc']);?><br></div><div class="form-group" id="tr_copia">
                                        ...
                                        <input type="hidden" maxlength="2000"  name="copia" id="copia" value=""><?php componente_ejecutor("6",@$_REQUEST["iddoc"]); ?></div><div class="form-group" id="tr_serie_idserie">
                                ...
                                <a href="javascript:void(0)" onclick="tree_serie_idserie.findItem((document.getElementById('stext_serie_idserie').value))">
                                    <img src="../../botones/general/siguiente.png"border="0px"></a><br/><input type="hidden" maxlength="11"  class="required"  name="serie_idserie" id="serie_idserie"   value="" ><label style="display:none" class="error" for="serie_idserie">Campo obligatorio.</label><div id="esperando_serie_idserie">
                            ...
                            <label title="">TIPO DE MENSAJER&Iacute;A</label><?php genera_campo_listados_editar(1,5200,$_REQUEST['iddoc']);?><br></div><div class="form-group form-group-default required"  id="tr_asunto">
                                        ...
                                        <input class="form-control"  maxlength="255"  class="required"   tabindex='2'  type="text"  size="100" id="asunto" name="asunto" required value="<?php echo(validar_valor_campo(10)); ?>">
                                       </div><div class="form-group" id="tr_contenido">
                                        <label title="">CONTENIDO*</label>
                                        <div class="celda_transparente"><textarea  tabindex='3'  name="contenido" id="contenido" cols="53" rows="3" class="form-control required"><?php echo(validar_valor_campo(7104)); ?></textarea><script>
                            var config = {
                            ...
                            var editor = CKEDITOR.replace("contenido", config);
                            ...
                            </div></div><div class="form-group" id="tr_despedida"><label title="Despedida de la Carta, Atentamente, Cordialmente, ...">DESPEDIDA</label><?php despedida(1,7);?></div><div class="form-group" id="tr_copiainterna">
                                ...
                                <a href="javascript:void(0)" onclick="tree_copiainterna.findItem((document.getElementById('stext_copiainterna').value))">
                                    <img src="../../botones/general/siguiente.png"border="0px"></a><br/><input type="hidden"  name="copiainterna" id="copiainterna"   value="" ><label style="display:none" class="error" for="copiainterna">Campo obligatorio.</label><div id="esperando_copiainterna">
                                    <img src="../../imagenes/cargando.gif">
                                </div>
                                ...
                                tree_copiainterna.setOnLoadingEnd(fin_cargando_copiainterna);tree_copiainterna.enableSmartXMLParsing(true);tree_copiainterna.loadXML("../../test.php?rol=1");tree_copiainterna.setOnCheckHandler(onNodeSelect_copiainterna);
                                ...
                                function cargando_copiainterna() {
                                    ...
                                    document.poppedLayer.style.display = "";
                                }</script></div></div><div class="form-group" id="tr_iniciales"><label title="Persona que Genera la Carta ">PERSONA QUE GENERA LA CARTA*</label><?php iniciales(1,5);?></div><div class="form-group" id="tr_anexos_digitales">
                                       ...
                                       <div class="card-body no-scroll no-padding"><div id="dz_campo_11" class="saia_dz dropzone no-margin" data-nombre-campo="anexos_digitales" data-idformato="1" data-idcampo-formato="11" data-extensiones="<?php echo $extensiones;?>" data-multiple="multiple"><div class="dz-message"><span>Arrastra el anexo hasta aqu&iacute;. </br> O si prefieres...</br></br> <span class="boton_upload">Elije un anexo para subir.</span> </span></div></div></div></div><div class="form-group" id="tr_anexos_fisicos"><label title="Listado con Los Anexos de la Carta">ANEXOS FISICOS</label><?php anexos_fisicos(1,4);?></div><input type="hidden" name="varios_radicados" value="<?php echo(validar_valor_campo(18)); ?>"><div class="form-group  required" id="tr_vercopiainterna">
                            <label title="">VISIBLE LA COPIA INTERNA*</label><?php genera_campo_listados_editar(1,12,$_REQUEST['iddoc']);?><label id="-error" class="error" f or="vercopiainterna" style="display: none;"></label><br></div><?php cargar_destinos_carta(1,NULL);?><?php guardar_plantilla(1,NULL);?><?php asignar_responsables(1,NULL);?><?php mostrar_imagenes_escaneadas(1,NULL);?><?php parsear_arbol_expediente_serie_carta(1,NULL);?><input type="hidden" name="campo_descripcion" value="10,7104"><tr><td colspan='2'><?php submit_formato(1);?></td></tr></table><input type='hidden' name='permisos_anexos' id='permisos_anexos' value=''><input type='hidden' name='form_uuid'       id='form_uuid'       value='<?php echo (uniqid("-") . "-" . uniqid());?>'></form></body>
