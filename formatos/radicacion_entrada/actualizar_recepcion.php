...
for($i=0;$i<count($idft_funcionario);$i++){
	...
	if(($tipo_mensajeria_radicacion[0]['tipo_mensajeria']==2 || $tipo_mensajeria_radicacion[0]['tipo_mensajeria']==1) && !$datos[0]['estado_recogida']){
		...
		$sql1="UPDATE ft_destino_radicacion SET ruta_destino=".$responsable[0]['idft_ruta_distribucion'].",tipo_mensajero='i',mensajero_encargado='".$responsable[0]['mensajero_ruta']."' WHERE idft_destino_radicacion=".$separacion_idft_funcionario[0];		 
