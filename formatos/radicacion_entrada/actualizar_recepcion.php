...
for($i=0;$i<count($idft_funcionario);$i++){
	...
	$tipo_mensajeria_radicacion=busca_filtro_tabla("tipo_mensajeria,area_responsable","ft_radicacion_entrada","idft_radicacion_entrada=".$separacion_idft_funcionario[2],"",$conn);
	...
	if($tipo_mensajeria_radicacion[0]['tipo_mensajeria']==2 && !$datos[0]['estado_recogida']){
		...
		$sql="UPDATE ft_destino_radicacion SET estado_recogida=1,estado_item=1,mensajero_encargado=".$responsable[0]['mensajero_ruta']." WHERE idft_destino_radicacion=".$separacion_idft_funcionario[0];
	}else{
