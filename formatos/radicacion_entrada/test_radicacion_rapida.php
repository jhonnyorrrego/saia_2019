...
if ($formato_radicacion["numcampos"]) {
	$ok_radicacion_entrada = $PermisoController->profileModuleAccess("crear_" . $formato_radicacion[0]['nombre']);
...
if ($formatos["numcampos"]) {
	$ok_formato = $PermisoController->profileModuleAccess("crear_" . $formatos[0]['nombre']);
	if ($ok_formato) {
		echo ("<item style=\"font-family:verdana; font-size:7pt;\" ");
