                 ...
                 success: function(datos){
                     ...
                     for (var i=1; i<datos.length; i++){
                         ...
                         if(cargado.indexOf(datos[i])<0){// devuleve -1
                             tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&admin=1&dependencia="+datos[i]+"&mostrar_nodos=dsa&sin_padre_dependencia=1&cargar_series="+datos[i-1]+"&uid="+x+"&id=d"+datos[i-1]);
                             cargado.push(datos[i]);
                         }/*else{
