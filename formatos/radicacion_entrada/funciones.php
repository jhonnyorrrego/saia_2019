...
function enviar_adicionar($idformato,$iddoc){
	...
	if($datos[0]["estado_radicado"]==1){
		if(@$_REQUEST["iddoc"]){
			$enlace="paginaadd.php?key=".$_REQUEST["iddoc"]."&enlace2=".FORMATOS_CLIENTE."radicacion_entrada/detalles_mostrar_radicacion_entrada.php?iddoc=".$_REQUEST["iddoc"];

		}
		else{
			$enlace=$ruta_db_superior.FORMATOS_CLIENTE."radicacion_entrada/detalles_mostrar_radicacion_entrada.php?iddoc=".$iddoc."&idformato=".$idformato;
		}
...
function validar_digitalizacion_formato_radicacion($idformato,$iddoc){
  ...
  if($_REQUEST["digitalizacion"]==1){
    ...
    //redirecciona($ruta_db_superior."paginaadd.php?&key=".$iddoc."&enlace=".$enlace);
  	...
  	abrir_url($ruta_db_superior.FORMATOS_CLIENTE."radicacion_entrada/mostrar_radicacion_entrada.php?iddoc=".$iddoc."&idformato=".$idformato,'_self');
		...
		$(document).ready(function(){
			...
			top.hs.Expander.prototype.onAfterClose = function() {
                if(tipo=='finalizacion'){
                    window.location = "<?php echo(FORMATOS_CLIENTE); ?>radicacion_entrada/mostrar_radicacion_entrada.php?idformato=<?php echo($idformato); ?>&iddoc=<?php echo($iddoc); ?>";
                }
            }
		});
		</script>
