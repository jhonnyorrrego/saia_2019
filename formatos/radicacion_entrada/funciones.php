...
function mostrar_item_destino_radicacion($idformato,$iddoc){
	...
	if($padre[0]['despachado']==0){
    	$tabla='<form id="item_prerequisitos" action="actualizar_item_destino_radicacion.php"><table class="table-bordered adicionar_campo" style="width: 100%; font-size:10px; text-align:center;" border="1">
...
function campos_adicionales_radicacion($idformato,$iddoc){
    ...
    $datos=busca_filtro_tabla("","ft_radicacion_entrada","documento_iddocumento=".$iddoc,"",$conn);
    $tabla='<table style="width: 100%; border-collapse: collapse; font-size: 10pt;" border="1">';
    if($datos[0]['tipo_origen']==1){
        $tabla.="<tr><td style='width:20%;'><strong>NUMERO DE DOCUMENTO:</strong></td><td style='width:80%;'>".$datos[0]['numero_oficio']."</td></tr>";
    }
