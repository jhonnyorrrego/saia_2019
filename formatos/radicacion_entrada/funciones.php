	...
	$(document).ready(function(){
	    ...
	    tree_serie_idserie.setOnLoadingEnd(cargar_arbol());
                ...
                setTimeout(function(){  
                    ...
                    tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&admin=1&dependencia=38&mostrar_nodos=dsa");
                    tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&admin=1&mostrar_nodos=dsa&uid=1477409126024&id=d"+dependencia_principal);
                }, 1000);
                }
                 ...
                 success: function(datos){
                     ...
                     for (var i=1; i<datos.length; i++){
                         ...
                         if(cargado.indexOf(datos[i])<0){// devuleve -1
                             tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&admin=1&mostrar_nodos=dsa&dependencia="+datos[i]+"&sin_padre=1&uid="+x+"&id=d"+datos[i-1]);
                             cargado.push(datos[i]);
                         }/*else{
