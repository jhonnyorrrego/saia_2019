	...
	$(document).ready(function(){
	    ...
	    tree_serie_idserie.setOnLoadingEnd(cargar_arbol());
                ...
                setTimeout(function(){  
                    ...
                    tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&admin=1&dependencia=38&sin_padre=1&mostrar_nodos=dsa");
                    tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&admin=1&sin_padre=1&uid=1477409126024&id=d"+dependencia_principal+"&mostrar_nodos=dsa");
                }, 1000);
                }
                 ...
                 success: function(datos){
                     ...
                     for (var i=1; i<datos.length; i++){
                         ...
                         if(cargado.indexOf(datos[i])<0){// devuleve -1
                             tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&admin=1&dependencia="+datos[i]+"&sin_padre=1&uid="+x+"&id=d"+datos[i-1]+"&mostrar_nodos=dsa");
                             cargado.push(datos[i]);
                         }/*else{
                ...
                function obtener_dependencia(rol){
                    $.ajax({
                        ...
                        success: function(datos){
                            //alert(datos[1]);
                            var x = Math.floor((Math.random() * 100000) + 1);
                            tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&admin=1&dependencia="+datos[1]+"&sin_padre=1&uid="+x+"&id=d"+datos[2]+"&mostrar_nodos=dsa");
