...
function mostrar_item_destino_radicacion($idformato,$iddoc){
    ...
    if($_REQUEST['tipo']!=5 && $dato[0]['estado']!='APROBADO' && $dato[0]['despachado']!=1){
						
				echo '<a href="../destino_radicacion/adicionar_destino_radicacion.php?pantalla=padre&amp;idpadre='.$iddoc.'&amp;idformato='.$idformato.'&amp;padre='.$dato[0]['idft_radicacion_entrada'].'" target="_self">Adicionar destino</a>'; //
		}
	...
	$(document).ready(function(){
	    ...
	    tree_serie_idserie.setOnLoadingEnd(cargar_arbol());
                ...
                setTimeout(function(){  
                    ...
                    tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&admin=1&dependencia=38&mostrar_nodos=dsa&sin_padre_dependencia=1");
                    tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&sin_padre_dependencia=1&admin=1&mostrar_nodos=dsa&uid=1477409126024&id=d"+dependencia_principal);
                }, 1000);
                }
                 ...
                 success: function(datos){
                     ...
                     for (var i=1; i<datos.length; i++){
                         ...
                         if(cargado.indexOf(datos[i])<0){// devuleve -1
                            // tree_serie_idserie.loadXML("<?php echo($ruta_db_superior); ?>test_dependencia_serie.php?tabla=dependencia&admin=1&dependencia="+datos[i]+"&mostrar_nodos=dsa&sin_padre_dependencia=1&uid="+x+"&id=d"+datos[i-1]);
                             cargado.push(datos[i]);
                         }/*else{
