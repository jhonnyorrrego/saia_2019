...
function remitente($remitente){
	...
	$remi=busca_filtro_tabla("a.nombre","ejecutor a,datos_ejecutor b","a.idejecutor=b.ejecutor_idejecutor and b.iddatos_ejecutor=".$remitente,"",$conn);
...
function estado($estado,$idft){
	...
	$item_accion=busca_filtro_tabla("accion","ft_item_aprobaciones","ft_radicacion_facturas=".$idft,"idft_item_aprobaciones desc",$conn);
...
function fecha_programa($fecha,$idft){
	...
	$padre=busca_filtro_tabla("estado","ft_radicacion_facturas","idft_radicacion_facturas=".$idft,"",$conn);
	if($padre[0]['estado']==4 || $padre[0]['estado']==5){
		$item=busca_filtro_tabla("fecha_aprobacion","ft_item_aprobaciones","ft_radicacion_facturas=".$idft,"idft_item_aprobaciones desc",$conn);
...
function val_factura($valor){
	...
	$funcionario=busca_filtro_tabla("","vfuncionario_dc","(cargo like 'Aprobador Contabilidad' or cargo like 'Aprobador Tesoreria' or cargo like 'Aprobador Juridica' or cargo like 'Aprobador Presupuesto' or cargo like 'Clasificador factura' or login like 'mavega') and estado=1 and estado_dc=1 and funcionario_codigo=".usuario_actual('funcionario_codigo'),"",$conn);
...
function indicador_cont($where){
	if($where){
		$datos=busca_filtro_tabla("count(a.estado) as cont","ft_radicacion_facturas a,documento b","a.documento_iddocumento=b.iddocumento and b.estado not in ('ELIMINADO','ANULADO','ACTIVO')".$where,"",$conn);
