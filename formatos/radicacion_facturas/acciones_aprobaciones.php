...
if(isset($_REQUEST['aprobar'])){
	if($_REQUEST['accion']=='contabilidad'){
		...
		$devolver_doc=busca_filtro_tabla("funcionario_codigo","vfuncionario_dc","iddependencia_cargo=".$_REQUEST['arbol_devuelto'],"",$conn);
	...
	if(isset($cargo)){
		$funcionario=busca_filtro_tabla("funcionario_codigo","vfuncionario_dc","estado=1 and estado_dc=1 and tipo_cargo=2 and cargo like '".$cargo."'","",$conn);
  ...
  <div class="form-group">
    ...
    <div class="col-lg-10">
      <select name="accion" id="accion" required="true">
      	...
      	<?php
      	$datos_padre=busca_filtro_tabla("b.funcionario_codigo,clasificacion_fact","ft_item_facturas a,vfuncionario_dc b","a.responsable=b.iddependencia_cargo and a.ft_radicacion_facturas=".$_REQUEST['idft']." and b.funcionario_codigo=".usuario_actual('funcionario_codigo'),"",$conn);
		
		$datos_clasificacion=busca_filtro_tabla("clasificacion_fact","ft_item_facturas a","ft_radicacion_facturas=".$_REQUEST['idft'],"",$conn);
		...
		$dato_recibido=busca_filtro_tabla("tipo_recibido","ft_item_recibidos","ft_radicacion_facturas=".$_REQUEST['idft'],"idft_item_recibidos desc",$conn); 
		
		$cargo_contabilidad=busca_filtro_tabla("funcionario_codigo","vfuncionario_dc","estado=1 and estado_dc=1 and cargo like 'Aprobador Contabilidad' and funcionario_codigo=".usuario_actual('funcionario_codigo'),"",$conn);
		...
		$cargo_presupuesto=busca_filtro_tabla("funcionario_codigo","vfuncionario_dc","estado=1 and estado_dc=1 and cargo like 'Aprobador Presupuesto' and funcionario_codigo=".usuario_actual('funcionario_codigo'),"",$conn);
		
		if($cargo_presupuesto['numcampos']){
			echo '<option value="contabilidad">Para aprobacion de contabilidad</option>';
		}

		$cargo_tesoreria=busca_filtro_tabla("funcionario_codigo","vfuncionario_dc","estado=1 and estado_dc=1 and cargo like 'Aprobador Tesoreria' and funcionario_codigo=".usuario_actual('funcionario_codigo'),"",$conn);
		
		if($cargo_tesoreria['numcampos']){
			echo '<option value="pagada">Pagada</option>';
		}

		$cargo_juridica=busca_filtro_tabla("funcionario_codigo","vfuncionario_dc","estado=1 and estado_dc=1 and cargo like 'Aprobador Juridica' and funcionario_codigo=".usuario_actual('funcionario_codigo'),"",$conn);
		
		if($cargo_juridica['numcampos']){
			echo '<option value="contabilidad">Para aprobacion de contabilidad</option>';
		}
		
		$cargo_clasificador=busca_filtro_tabla("funcionario_codigo","vfuncionario_dc","estado=1 and estado_dc=1 and lower(cargo) like 'clasificador factura' and funcionario_codigo=".usuario_actual('funcionario_codigo'),"",$conn);
		if($cargo_clasificador['numcampos']){
			echo '<option value="contabilidad">Para aprobacion de contabilidad</option>';
		}
		
		$cargo_aprobacion_compras=busca_filtro_tabla("funcionario_codigo","vfuncionario_dc","estado=1 and estado_dc=1 and cargo like 'Aprobador presupuesto' and funcionario_codigo=".usuario_actual('funcionario_codigo'),"",$conn);
