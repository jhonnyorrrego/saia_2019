      ...
      for($j=1;$j<($numcols-1);$j+=2){
        if($sExport =="csv"){
          if($nom_col[$j]=="tiempo_seguimiento" || $nom_col[$j]=="tiempo_cumplimiento"){
            $valor=mostrar_valor_campo($nom_col[$j],2,$datos[$i]["hallazgo_iddoc"],1);
            if($valor=='null'){
              $valor='"ND"';
            }
            ...
            $valor1=str_replace("&nbsp;","",htmlspecialchars_decode(str_replace(",",";",mostrar_valor_campo($nom_col[$j],2,$datos[$i]["hallazgo_iddoc"],1))));
            $valor1=trim(strip_tags(html_entity_decode($valor1)));
            $valor1=str_replace("\n","",$valor1);
            $valor1=str_replace("'",'"',$valor1);
            $valor1=str_replace('"','""',$valor1);
            $j+=2;
            $valor2=str_replace("&nbsp;","",htmlspecialchars_decode(str_replace(",",";",mostrar_valor_campo($nom_col[$j],2,$datos[$i]["hallazgo_iddoc"],1))));
            ...
            $texto1=str_replace("&nbsp;","",htmlspecialchars_decode(str_replace(",",";",mostrar_valor_campo($nom_col[$j],2,$datos[$i]["hallazgo_iddoc"],1))));
