	...
	$('#formulario_formatos').validate();
                  ...
                  <!--
                    ...
                    tree_serie_idserie.enableRadioButtons(true);tree_serie_idserie.setOnLoadingStart(cargando_serie_idserie);
                      tree_serie_idserie.setOnLoadingEnd(fin_cargando_serie_idserie);tree_serie_idserie.enableSmartXMLParsing(true);tree_serie_idserie.loadXML("../../test_serie_funcionario.php?con_padres=1");
                	...
                	--></script></td></tr><tr>
                     ...
                     <?php fecha_formato(308,3610);?></tr><tr>
                   <td class="encabezado" width="20%" title="">DESTINOS*</td>
                   <td bgcolor="#F5F5F5">
                   <input type="hidden" maxlength="2000"  class="required"  name="destinos" id="destinos" value=""><?php componente_ejecutor("3609",@$_REQUEST["iddoc"]); ?></td>
                  </tr><tr>
                   <td class="encabezado" width="20%" title="Personas a quienes se les Envia Copia de la Carta">CON COPIA A</td>
                   <td bgcolor="#F5F5F5">
                   <input type="hidden" maxlength="2000"  name="copia" id="copia" value=""><?php componente_ejecutor("3606",@$_REQUEST["iddoc"]); ?></td>
                  </tr><tr>
                     <td class="encabezado" width="20%" title="">ASUNTO*</td>
                     <td bgcolor="#F5F5F5"><input  maxlength="255"  class="required"   tabindex='2'  type="text" size="100" id="asunto" name="asunto"  value="<?php echo(validar_valor_campo(3604)); ?>"></td>
                    </tr><tr>
                     <td class="encabezado" width="20%" title="">CONTENIDO*</td>
                     <td class="celda_transparente"><textarea  tabindex='3'  name="contenido" id="contenido" cols="53" rows="3" class="tiny_basico required"><?php echo(validar_valor_campo(3605)); ?></textarea></td>
                    </tr><tr>
                     <td class="encabezado" width="20%" title="Despedida de la Carta, Atentamente, Cordialmente, ...">DESPEDIDA</td>
                     <?php despedida(308,3608);?></tr><tr>
                   ...
                   <a href="javascript:void(0)" onclick="tree_copiainterna.findItem(htmlentities(document.getElementById('stext_copiainterna').value))"><img src="../../botones/general/siguiente.png"border="0px"></a> 
                          <br /><div id="esperando_copiainterna"><img src="../../imagenes/cargando.gif"></div><div id="treeboxbox_copiainterna" height="90%"></div><input type="hidden" maxlength="11"  class="required"  name="copiainterna" id="copiainterna"   value="" ><label style="display:none" class="error" for="copiainterna">Campo obligatorio.</label><script type="text/javascript">
                  <!--
                      var browserType;
                      if (document.layers) {browserType = "nn4"}
                      if (document.all) {browserType = "ie"}
                	...
                	--></script></td></tr><tr id="tr_vercopiainterna" >
                     ...
                     <?php iniciales(308,3611);?></tr><tr>
                     <td class="encabezado" width="20%" title="">ANEXOS DIGITALES</td>
                     <td class="celda_transparente"><input  tabindex='5'  type="file" maxlength="2000"  class='multi'  name="anexos_digitales[]" accept="<?php echo $extensiones;?>"><tr>
                     <td class="encabezado" width="20%" title="Listado con Los Anexos de la Carta">ANEXOS FISICOS</td>
                     <?php anexos_fisicos(308,3618);?></tr><tr id="tr_varios_radicados" >
                     <td class="encabezado" width="20%" title="">&iquest;ASIGNAR UN N&Uacute;MERO DE RADICADO DIFERENTE PARA CADA DESTINO?*</td><td bgcolor="#F5F5F5"><?php genera_campo_listados_editar(308,3619,$_REQUEST['iddoc']);?></td></tr><input type="hidden" name="idft_carta_pqrsf" value="<?php echo(validar_valor_campo(3613)); ?>"><input type="hidden" name="documento_iddocumento" value="<?php echo(validar_valor_campo(3614)); ?>"><input type="hidden" name="firma" value="<?php echo(validar_valor_campo(3617)); ?>"><?php if($_REQUEST["padre"]) {?><input type="hidden"  name="ft_pqrsf"  value="<?php echo $_REQUEST["padre"]; ?>"><?php } ?><?php if($_REQUEST["anterior"]) {?><input type="hidden"  name="ft_pqrsf"  value="<?php echo $_REQUEST["anterior"]; ?>"><?php }  else {listar_select_padres(ft_pqrsf);} ?><?php asignar_responsables(308,NULL);?><input type="hidden" name="campo_descripcion" value="3604"><input type="hidden" name="padre" value="<?php echo $_REQUEST["padre"]; ?>"><input type="hidden" name="anterior" value="<?php echo $_REQUEST["anterior"]; ?>"><input type="hidden" name="accion" value="guardar_detalle" ><tr><td colspan='2'><?php submit_formato(308);?></td></tr></table><input type='hidden' name='permisos_anexos' id='permisos_anexos' value=''></form></body></html>