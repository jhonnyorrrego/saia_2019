...
function mostrar_mensajeros($idformato, $iddoc)
{
	...
	if ($nmensajeros) {
		...
		for ($i = 0; $i < $nmensajeros; $i++) {
			$dato_mensajero = busca_filtro_tabla("A.idfuncionario,A.nombres,A.apellidos", "funcionario A", "A.idfuncionario=" . $mensajero[$i], "");
			//redirecciona(implode("-",$dato_mensajero));
			if ($dato_mensajero["numcampos"])
				echo ("<option value=" . $dato_mensajero[0]["idfuncionario"] . ">" . ucwords(strtolower($dato_mensajero[0]["nombres"] . " " . $dato_mensajero[0]["apellidos"])) . "</option>");
		}
		echo ('</select></td>');
	}
	?>
