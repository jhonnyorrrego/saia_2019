...
function mostrar_mensajeros($idformato,$iddoc){
	...
	$(document).ready(function(){
		...
		<?php
		if($_REQUEST["iddoc"]!=''){
			$dato=busca_filtro_tabla("","ft_radicacion_salia A","A.documento_iddocumento=".$_REQUEST["iddoc"],"",$conn);
...
function registrar_salida($idformato,$iddoc){
	...
	$dato=busca_filtro_tabla("","ft_radicacion_salida A","A.documento_iddocumento=".$iddoc,"",$conn);
...
function obtener_informacion_proveedor_salida($idformato,$iddoc){
	global $conn,$ruta_db_superior;
	$datos=busca_filtro_tabla("","ft_radicacion_salida A, datos_ejecutor B, ejecutor C","A.persona_natural=B.iddatos_ejecutor AND B.ejecutor_idejecutor=C.idejecutor AND A.documento_iddocumento=".$iddoc,"",$conn);
