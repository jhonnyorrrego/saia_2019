...
function guardar_traza($sql,$nombre_formato,$sql_export){
	...
	$ruta_evento = RUTA_ABS_SAIA . RUTA_EVENTO_FORMATO.strtolower($nombre_formato);
    $nombre=$ruta_evento."/".DB."_".date("Ymd").".txt";
	if(!@is_file($nombre)){
	...
	if(file_put_contents($nombre,$sql.";\n",FILE_APPEND)){
    	if($sql_export){
    	    ...
    	    $nombre_export=$ruta_evento."/export_".DB."_".date("Ymd").".txt";
