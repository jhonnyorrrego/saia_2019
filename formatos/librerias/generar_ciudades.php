...
$funciones = array("titulo" => "tituloEjecutor","cargo" => "textoEjecutor", "direccion" => "textoEjecutor", "telefono" => "textoEjecutor", "email" => "textoEjecutor", "ciudad" => "ciudadEjecutor", "empresa" => "textoEjecutor", "identificacion" => "textoEjecutor", "nombre" => "textoEjecutor", "nombre_pj" => "textoEjecutor");

$texto .= '<div class="">';

$cantCampos = 0;
foreach ($funciones as $funcion=>$accion) {
	if($accion == 'textoEjecutor' && in_array($funcion,$campos)){
		$cantCampos++;
	}
}	
...
foreach ($campos as $nombre) {
	if ($nombre <> ''){
		$texto .= $funciones[$nombre]($nombre);
	}
}
$texto .= '</div>';
echo $texto;

function textoEjecutor($nombre){
	global $conn, $etiquetas, $contador, $cantCampos;

	$contador++;
	$texto = '';
	$textoFin ='';

	if($contador%2 != 0  && $contador != $cantCampos){
		$texto .= '<div class="row">';
	}else if($contador%2 == 0){	
		$textoFin ='</div>';
	}
//print_r("<br/>" . $contador . " -" . $cantCampos);
	if($contador%2 != 0 && $contador == $cantCampos){
		$texto .= '<div class="row">
								<div class="col-md-6">
									<div class="form-group form-group-default">
										<label id="label_' . $nombre . '">' . $etiquetas[$nombre] . '</label>
										<input type="text" placeholder="' . $etiquetas[$nombre] . '" class="form-control" id="'.$nombre.'_ejecutor" name="cargo_ejecutor" value="' . @$datos_ejecutor[0][$nombre] . '">
									</div>
								</div>
								<div class="col-md-6">
								</div>
...
function tituloEjecutor($nombre){
	global $conn, $etiquetas;

	$texto = '
	<div class="row" id="div_titulo_ejecutor">
		<div class="col-md-6">
			<div class="form-group form-group-default" id="div_select_titulo_ejecutor">
				<label id="label_' . $nombre . '">' . $etiquetas[$nombre] .':</label>
				...
				<select style="width: 60%;" data-init-plugin="select2" name="titulo_ejecutor" id="titulo_ejecutor">';
...
function ciudadEjecutor($nombre){
	global $conn, $etiquetas;
	...
	if ($nombre == "estudios") {
	    $texto .= '<textarea style="font-size:x-small;font-family:verdana" id="' . $nombre . '_ejecutor" cols=35 rows=2>' . @$datos_ejecutor[0][$nombre] . '</textarea>';
			...
			$texto .= '';
				...
				$texto .= '<input type="text" placeholder="' . $etiquetas[$nombre] . '" class="form-control" id="'.$nombre.'_ejecutor" name="cargo_ejecutor" value="' . @$datos_ejecutor[0][$nombre] . '">
				</div>
		</div>';
		}
	return ($texto);
    ...
    function actualiza_ciudad_' . $campo . '(pais,departamento){
      $.ajax({
		...
		$texto .= '
			...
			<div class="col-md-4">
				<span class="label label-success" style="cursor:pointer;" id="nuevo_municipio_' . $campo . '">Otro</span>
			</div>
		</div>
	</div>
</div></div>';
