    ...
    if ($campos1) {
        for ($i = 0; $i < count($campos1); $i++) {
            ...
            if ($compara["numcampos"]) {
                $texto = str_replace($campos1[$i], ComponentFormGeneratorController::callShowValue($idformato, $_REQUEST["iddoc"], $compara[0]['nombre']), $texto);
            }
        }
...
function recorrido($idformato, $iddoc)
{
    ...
    return ComponentFormGeneratorController::callShowValue($idformato, $iddoc, $CamposFormato->nombre);
}
