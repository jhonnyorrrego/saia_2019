...
function permisos_modulo_menu_intermedio($modulo_padre,$lista){    
    ...
    for($i=0;$i<$modulo["numcampos"];$i++){
        ...
        if($ok){
            ...
            if($lista){
               $texto.='<li><a href="'.$ruta_db_superior.$dir.'" class="kenlace_saia_propio" enlace="'.$dir.'"><img hspace="0" height="16" width="16" vspace="0" border="0" src="'.$ruta_db_superior.$modulo[$i]["imagen"].'"> '.$modulo[$i]["etiqueta"].'</a></li>'; 
            }
            else{
