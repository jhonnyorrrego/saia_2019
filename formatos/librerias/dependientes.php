...
{$componente=busca_filtro_tabla("valor","campos_formato","idcampos_formato=".$_REQUEST["idcomponente"],"",$conn);

 if($componente["numcampos"])
   {$select=explode("|",$componente[0][0]);
    $sql=explode(";",$select[$_REQUEST["pos"]+1]);
    $datos=ejecuta_filtro_tabla($sql[1].$_REQUEST["valor"]." order by nombre",$conn);
