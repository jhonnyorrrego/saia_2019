...
{global $conn;
 ...
 if($iddoc<>"")
  ...
  if($campo[0]["valor"]<>"")
    ...
    $parametros=array("multiple","nombre,identificacion","cargo,empresa,direccion,telefono,email,titulo,ciudad");
 //$parametros=explode("@",$campo[0]["valor"]);
...
function editar_anexos_digitales($idformato,$idcampo,$iddoc=NULL)
{
 ?>
  <tr> 
        ...
        <td bgcolor="#F5F5F5">
          <iframe src="../../upload.php?iddoc=<?php echo $iddoc; ?>" width="500" height="50" frameborder=0 scrolling="no" marginwidth=0>
          </iframe>
          </font>
        </td>
      </tr>
      <tr> 
        ...
        <td bgcolor="#F5F5F5">
        <iframe name='listar_archivos' id='listar_archivos' src="../../listar_anexos.php?iddoc=<?php echo $iddoc; ?>" width="100%" height="100%" frameborder=0 scrolling="no" marginwidth=0>
        </iframe>
        </td>
      </tr>
  <?php
...
{global $conn;
 ?>
  <td bgcolor="#F5F5F5">    
    ...
    ?>
    <input type="hidden" name="anexos_fisicos" id="anexos_fisicos" value="<?php if($anexos_fisicos["numcampos"]){ echo $anexos_fisicos[0]["anexos_fisicos"];}?>">
    <input type=button value="Adicionar Anexo F&iacute;sico" onclick="adicionar_anexo();">
    <input type=button value="Borrar Anexos F&iacute;sicos" onclick="document.getElementById('anexos_fisicos').value=''; document.getElementById('mostrar_archivos2').innerHTML='';">
...
function buscar_listado_formato($tabla_formato,$formato_detalle){
  ...
  if($formato["numcampos"] && $tabla_formato<>""){
   ...
   <tr>';
      ...
      else $texto.="<td>No existe detalle</td>";
      $texto.="</tr></table>";
    } 
  echo($enlace.$texto);  
  }
...
if($respuesta["numcampos"]){
  ...
  if($origen["numcampos"]){
    ...
    $datos["tipo_destino"]=1;
    $datos["tipo"]="";
    transferir_archivo_prueba($datos,array($origen[0]["ejecutor"]),"");
  }
}
}
...
function generar_ruta_documento_fija_formato($idformato,$iddoc){
	...
	for($i=0;$i<$dato["numcampos"];$i++){
    	...
    	$funcionario_temp=busca_filtro_tabla("","vfuncionario_dc","iddependencia_cargo=".$dato[$i]["llave"],"",$conn);
			...
			$funcionario=call_user_func_array($dato[$i]["funcion"],array($idformato,$iddoc));
		}
		if($i==0&&$funcionario==usuario_actual("funcionario_codigo"))
			continue;
		if($funcionario!=''){
			array_push($rut,array("funcionario"=>$funcionario,"tipo_firma"=>$dato[$i]["firma"]));
		}	
	}
	if($dato["numcampos"])
...
function firma_externa_funcion($idformato,$iddoc,$tabla,$campo="firma_externa",$campo_tabla="documento_iddocumento",$llave_modificar="",$adicional,$retorno=2){
	...
	if(!$tabla)$tabla=$formato[0]["nombre_tabla"];
	if(!$llave_modificar)$llave_modificar=$iddoc;
...
function fk_idexpediente_funcion($idformato,$iddoc){
  ...
  <td bgcolor="#F5F5F5">
    ...
    <br />  Buscar: <input  tabindex='2'  type="text" id="stext_fk_idexpediente" width="200px" size="25"><a href="javascript:void(0)" onclick="tree_fk_idexpediente.findItem(htmlentities(document.getElementById('stext_fk_idexpediente').value),1)"> <img src="../../botones/general/anterior.png"border="0px"></a>
    <a href="javascript:void(0)" onclick="tree_fk_idexpediente.findItem(htmlentities(document.getElementById('stext_fk_idexpediente').value),0,1)"><img src="../../botones/general/buscar.png"border="0px"></a>                          
    <a href="javascript:void(0)" onclick="tree_fk_idexpediente.findItem(htmlentities(document.getElementById('stext_fk_idexpediente').value))"><img src="../../botones/general/siguiente.png"border="0px"></a> 
    <br /><div id="esperando_fk_idexpediente"><img src="../../imagenes/cargando.gif"></div>
    <div id="treeboxbox_fk_idexpediente" height="90%"></div>
    <input type="hidden" maxlength="255"  class="required"  name="fk_idexpediente" id="fk_idexpediente" value="" >
    <label style="display:none" class="error" for="fk_idexpediente">Campo obligatorio.</label>
    <script type="text/javascript">
                  <!--
                      var browserType;
...
function crear_pdf_documento_tcpdf($datos_documento, $datos_ejecutor=null){	
	...
	if(!file_exists($ruta_pdf)){
		...
		$ruta .= strtoupper($datos_documento["plantilla"])."_".strtoupper($datos_documento["numero"])."_".$fecha[0]"_".$fecha[1]."_".$fecha[2].".pdf";
