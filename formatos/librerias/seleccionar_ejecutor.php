...
if (isset($_REQUEST["datosCiudad"])) {
	...
	$pais = Model::getQueryBuilder()
		->select("pais_idpais")
		->from("departamento")
		->where("iddepartamento = " . $departamento[0]['departamento_iddepartamento'])->execute()->fetchAll();
	...
	if ($busqueda['numcampos']) {
		...
		for ($i = 0; $i < $busqueda["numcampos"]; $i++) {
			...
			$html .= "<li class='list-group-item' onclick=\"cargar_datos(" . $busqueda[$i]["idejecutor"] . ",'" . html_entity_decode($busqueda[$i]["nombre"]) . "','" . $busqueda[$i]["identificacion"] . "','" . $busqueda[$i]["direccion"] . "','" . $busqueda[$i]["titulo"] . "','" . $busqueda[$i]["telefono"] . "','" . $busqueda[$i]["cargo"] . "','" . $busqueda[$i]["email"] . "','" . $busqueda[$i]["ciudad"] . "')\">" . $busqueda[$i]["nombre"] . "</li>";
			if ($tipo == 'codigo') {
