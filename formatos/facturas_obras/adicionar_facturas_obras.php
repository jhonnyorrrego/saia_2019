<?php
                        ...
                        <!DOCTYPE html>
                            <html>
                                <head>
                                    ...
                                    <meta charset="utf-8" />
                                    <title>.:ADICIONAR RADICACI&Oacute;N FACTURAS DE OBRA:.</title>
                                    <meta name="viewport"
                	...
                	<div class="container-fluid container-fixed-lg col-lg-8" style="overflow: auto;" id="content_container">
                    	...
                    	<div class="card card-default">
                            <div class="card-body"><center><h5 class="text-black">RADICACIÓN FACTURAS DE OBRA</h5></center><?php llama_funcion_accion(@$_REQUEST["iddoc"],422,"ingresar","ANTERIOR"); ?>
                       <form name="formulario_formatos" id="formulario_formatos" role="form" autocomplete="off" method="post" action="<?= $ruta_db_superior ?>class_transferencia.php"" enctype="multipart/form-data"><div class="form-group" id="tr_dependencia"><label title="">VENTANILLA DEL CREADOR DEL DOCUMENTO*</label><?php buscar_dependencia(422,6974);?></div><div class="form-group" id="tr_fecha_radicacion"><label title="">FECHA DE RADICACI&Oacute;N*</label><?php fecha_formato(422,6977);?></div><div class="form-group" id="tr_numero_radicado"><label title="">N&Uacute;MERO DE RADICADO</label><?php mostrar_radicado_obra(422,6994);?></div><div class="form-group" id="tr_fecha_factura">
...
<script type="text/javascript">
            $(function () {
                ...
                $("#content_container").height($(window).height());
                                        ...
                                        <input class="form-control"  maxlength="255"   tabindex='2'  type="text"  size="100" id="numero_factura" name="numero_factura"  value="<?php echo(validar_valor_campo(6979)); ?>">
                                       </div><div class="form-group" id="tr_concepto_factura">
                                        <label title="">CONCEPTO DE LA FACTURA*</label>
                                        <div class="celda_transparente"><textarea  tabindex='3'  name="concepto_factura" id="concepto_factura" cols="53" rows="3" class="form-control required"><?php echo(validar_valor_campo(6980)); ?></textarea></div></div><div class="form-group form-group-default required"  id="tr_valor_factura">
                                        <label title="">VALOR DE LA FACTURA</label>
                                        <input class="form-control"  maxlength="30"  class="required"   tabindex='4'  type="text"  size="100" id="valor_factura" name="valor_factura" required value="<?php echo(validar_valor_campo(6981)); ?>">
                                       </div><div class="form-group" id="tr_vence_factura"><label title="">VENCIMIENTO DE LA FACTURA*</label><?php fecha_formato(422,6982);?></div><div class="form-group form-group-default required"  id="tr_numero_guia">
                                        <label title="">N&Uacute;MERO DE GU&Iacute;A</label>
                                        <input class="form-control"  maxlength="50"   tabindex='5'  type="text"  size="100" id="numero_guia" name="numero_guia" required value="<?php echo(validar_valor_campo(6983)); ?>">
                                        ...
                                        <input class="form-control"  maxlength="50"   tabindex='6'  type="text"  size="100" id="numero_folios" name="numero_folios" required value="<?php echo(validar_valor_campo(6985)); ?>">
                                       </div><div class="form-group" id="tr_anexos_fisicos">
                                        <label title="">ANEXOS F&Iacute;SICOS</label>
                                        <div class="celda_transparente"><textarea  tabindex='7'  name="anexos_fisicos" id="anexos_fisicos" cols="53" rows="3" class="form-control"><?php echo(validar_valor_campo(6986)); ?></textarea></div></div><div class="form-group" id="tr_anexos_digitales">
                                       ...
                                       <div class="card-body no-scroll no-padding"><div id="dz_campo_6987" class="saia_dz dropzone no-margin" data-nombre-campo="anexos_digitales" data-idformato="422" data-idcampo-formato="6987" data-extensiones="<?php echo $extensiones;?>" data-multiple="unico"><div class="dz-message"><span>Arrastra el anexo hasta aqu&iacute;. </br> O si prefieres...</br></br> <span class="boton_upload">Elije un anexo para subir.</span> </span></div></div></div></div><div class="form-group" id="tr_persona_natural">
                                        <label title="">PERSONA NATURAL/JURIDICA*</label>
                                        <input type="hidden" maxlength="11"  class="required"  name="persona_natural" id="persona_natural" value=""><?php componente_ejecutor("6988",@$_REQUEST["iddoc"]); ?></div><div class="form-group" id="tr_destino">
                                       <label title="">DESTINO*</label><input type="text" class="form-control" name="destino" id="destino" value=""required></div>
    <script>
	            ...
	            success: function(res) {
	                callback(res);
    ...
    });</script><input type="hidden" name="serie_idserie" value="<?php echo(validar_valor_campo(6971)); ?>"><div class="form-group" id="tr_copia">
                                ...
                                <a href="javascript:void(0)" onclick="tree_copia.findItem((document.getElementById('stext_copia').value))">
                                    <img src="../../botones/general/siguiente.png"border="0px"></a><br/><input type="hidden" maxlength="255"  name="copia" id="copia"   value="" ><label style="display:none" class="error" for="copia">Campo obligatorio.</label><div id="esperando_copia">
                                ...
                                tree_copia.setOnLoadingEnd(fin_cargando_copia);tree_copia.setXMLAutoLoading("../../test_funcionario.php?rol=1&sin_padre=1&cargar_partes=1");tree_copia.loadXML("../../test_funcionario.php?rol=1&sin_padre=1&cargar_partes=1");tree_copia.setOnCheckHandler(onNodeSelect_copia);
                    ...
                    $.ajax({
                        ...
                        success : function(data) {
                            if(data['error']){
                                ...
                                }</script></div></div><input type="hidden" name="encabezado" value="<?php echo(validar_valor_campo(6975)); ?>"><input type="hidden" name="documento_iddocumento" value="<?php echo(validar_valor_campo(6973)); ?>"><input type="hidden" name="idft_facturas_obras" value="<?php echo(validar_valor_campo(6972)); ?>"><input type="hidden" name="firma" value="<?php echo(validar_valor_campo(6976)); ?>"><input type="hidden" name="estado_documento" value="<?php echo(validar_valor_campo(6970)); ?>"><input type="hidden" name="campo_descripcion" value="6979"><tr><td colspan='2'><?php submit_formato(422);?></td></tr></table><input type='hidden' name='permisos_anexos' id='permisos_anexos' value=''><input type='hidden' name='form_uuid'       id='form_uuid'       value='<?php echo (uniqid("-") . "-" . uniqid());?>'></form></body><script type='text/javascript'>
