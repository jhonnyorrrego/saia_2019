...
function mostrar_radicado_obra($idformato, $iddoc)
{
	...
	$fecha = date('Y-m-d');
	if ($_REQUEST["iddoc"]) {
		$doc = busca_filtro_tabla("numero", "documento a", "iddocumento=" . $_REQUEST["iddoc"], "");
...
function post_edit_rad_obras($idformato, $iddoc)
{
	...
	$datos = busca_filtro_tabla("d.estado,ft.vence_factura,ft.destino,ft.copia", "ft_facturas_obras ft,documento d", "d.iddocumento=ft.documento_iddocumento and d.iddocumento=" . $iddoc, "");
...
function mostrar_destino_facturas_obras($idformato, $iddoc)
{
	...
	$funcionario = busca_filtro_tabla("a.nombres,a.apellidos", "vfuncionario_dc a, ft_facturas_obras b", "a.iddependencia_cargo=b.destino AND b.documento_iddocumento=" . $iddoc, "");
...
function ver_qr_rad_obras($idformato, $iddoc)
{
	...
	$dependencia_creador = busca_filtro_tabla("b.codigo", "vfuncionario_dc a, dependencia b", "b.iddependencia=a.iddependencia AND a.iddependencia_cargo=" . $datos[0]['dependencia'], "");
...
function post_aprob_rad_obras($idformato, $iddoc)
{
	...
	$datos = busca_filtro_tabla("vence_factura,valor_factura,persona_natural,destino", "ft_facturas_obras", "documento_iddocumento=" . $iddoc, "");
