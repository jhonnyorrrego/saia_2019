...
<form name="formatoadd" id="formatoadd" action="formatoadd.php" method="post">
	<p>
	...
	<table border="0" cellspacing="1" cellpadding="4" bgcolor="#CCCCCC">
		<tr>
			...
			<td bgcolor="#F5F5F5"><span class="phpmaker"> <input type="text" maxlength="20" name="x_nombre" id="x_nombre" class="required" value="<?php echo htmlspecialchars(@$x_nombre) ?>">
				</span></td>
		</tr>
		<tr>
			<td class="encabezado"><span class="phpmaker" style="color: #FFFFFF;">Etiqueta*</span></td>
			<td bgcolor="#F5F5F5"><span class="phpmaker"> <input type="text" name="x_etiqueta" id="x_etiqueta" value="<?php echo htmlspecialchars(@$x_etiqueta) ?>" class="required">
				</span></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td bgcolor="#F5F5F5"><span class="phpmaker">
					<?php if (!(!is_null($x_contador_idcontador)) || ($x_contador_idcontador == "")) {
						$x_contador_idcontador = 0;
					} // Set default value 
					?>
					<?php
					...
					?>
					<input type="checkbox" value="1" name="reiniciar_contador" id="reiniciar_contador">Reiniciar contador con el cambio de
					a&ntilde;o
				</span></td>
		</tr>
		<tr>
...
<input type="text" name="x_tabla" id="x_tabla" value="<?php echo htmlspecialchars(@$x_tabla) ?>">
		...
		<tr>
			...
			<td bgcolor="#F5F5F5"><span class="phpmaker"> <input type="text" name="x_librerias" id="x_librerias" value="<?php echo htmlspecialchars(@$x_librerias) ?>">
				</span></td>
		</tr>
		<tr>
			<td class="encabezado"><span class="phpmaker" style="color: #FFFFFF;">Tipo</span></td>
			<td bgcolor="#F5F5F5"><span class="phpmaker"> <input type="checkbox" name="x_item" id="x_item" value="1">Item <input type="checkbox" name="x_detalle" id="x_detalle" value="1">Detalle <input type="checkbox" name="x_tipo_edicion" id="x_tipo_edicion" value="1">Edicion
					Continua <input type="checkbox" name="x_banderas[]" id="x_banderas" value="e">Aprobacion Automatica <input type="checkbox" name="x_mostrar" id="x_mostrar" value="1" checked>Mostrar <br> <input type="checkbox" name="x_paginar" id="x_paginar" value="1" checked>Paginar
					al mostrar <input type="checkbox" name="x_banderas[]" id="x_banderas" value="r">Tomar el asunto del padre al responder<br>
		...
		<tr>
			...
			<td bgcolor="#F5F5F5"><span class="phpmaker"> <select name="x_font_size" id="x_font_size">
						<?php
						if (!$x_font_size) {
							$x_font_size = 11;
						}
		...
		<tr>
			...
			<td bgcolor="#F5F5F5"><span class="phpmaker"> <input type="radio" name="enter2tab" value="0" <?php if (!$enter2tab) echo "checked"; ?>>No&nbsp;&nbsp;
		...
		<tr>
			...
			<td bgcolor="#F5F5F5"><span class="phpmaker"> <input type="radio" name="x_orientacion" id="x_orientacion" value="0" checked="true">Vertical
		...
		<tr>
			<td class="encabezado"><span class="phpmaker" style="color: #FFFFFF;">Tama&ntilde;o
					del Papel</span></td>
			<td bgcolor="#F5F5F5"><span class="phpmaker"> <select name="x_papel" id="x_papel">
						<option value="A4" <?php if (@$x_papel == "A4") {
												echo (' selected="selected"');
											} ?>>A4</option>
						<option value="A5" <?php if (@$x_papel == "A5") {
												echo (' selected="selected"');
											} ?>>Media
							Carta</option>
						<option value="letter" <?php if (@$x_papel == "letter") {
													echo (' selected="selected"');
												} ?>>Carta</option>
						<option value="legal" <?php if (@$x_papel == "legal") {
													echo (' selected="selected"');
												} ?>>Oficio</option>
					</select>
				</span></td>
		...
		<tr>
			...
			<td bgcolor="#F5F5F5"><input type="text" class="required" name="x_autoguardado" id="x_autoguardado" value="300000"></td>
		</tr>
		<tr>
			...
			<td bgcolor="#F5F5F5"><span class="phpmaker">
					...
					if (!(!is_null($x_exportar)) || ($x_exportar == "")) {
						$array_check_exportar["tcpdf"] = "checked";
					} // Set default value
					?>
		...
		<tr>
			...
			<td bgcolor="#F5F5F5"><span class="phpmaker"> <input type="radio" name="x_pertenece_nucleo" value=0 <?php if (!$x_pertenece_nucleo) echo "checked"; ?>>No&nbsp;&nbsp; <input type="radio" name="x_pertenece_nucleo" value=1 <?php if ($x_pertenece_nucleo) echo "checked"; ?>>Si
				</span></td>
		</tr>
		<tr>
		...
		<tr>
			<input type="hidden" name="x_flujo_idflujo" id="x_flujo_idflujo" value="0">
			<td class="encabezado"><span class="phpmaker" style="color: #FFFFFF;">Funci&oacute;n
					predeterminada</span></td>
			<td bgcolor="#F5F5F5"><span class="phpmaker"> <input type="checkbox" name="x_funcion_predeterminada[]" value="1">Varios responsables <input type="checkbox" name="x_funcion_predeterminada[]" value="2">Digitalizaci&oacute;n
				</span></td>
		</tr>
	</table>
	<p>
...
function AddData($conn)
{
	...
	if ($x_serie_idserie == "") { // crear la serie con el nombre del formato
		...
		if ($nomb_serie_papa["numcampos"]) {
			...
			guardar_traza($sql_serie_papa, $x_nombre);
...
{ //Esta funcion es la misma que en formatoedit.php =>actualizar_modulo_formato
	global $conn;
