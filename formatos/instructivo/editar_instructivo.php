<html><title>.:EDITAR INSTRUCTIVOS:.</title>
  ...
  $(document).ready(function() {
			...
			<body bgcolor="#F5F5F5"><?php llama_funcion_accion(@$_REQUEST["iddoc"],@$_REQUEST["idformato"],"ingresar","ANTERIOR");?><form name="formulario_formatos" id="formulario_formatos" method="post" action="../../class_transferencia.php" enctype="multipart/form-data"><table width="100%" cellspacing="1" cellpadding="4"><tr><td colspan="2" class="encabezado_list">INSTRUCTIVOS</td></tr><input type="hidden" name="idft_instructivo" value="<?php echo(mostrar_valor_campo('idft_instructivo',491,$_REQUEST['iddoc'])); ?>"><input type="hidden" name="documento_iddocumento" value="<?php echo(mostrar_valor_campo('documento_iddocumento',491,$_REQUEST['iddoc'])); ?>"><input type="hidden" name="encabezado" value="<?php echo(mostrar_valor_campo('encabezado',491,$_REQUEST['iddoc'])); ?>"><input type="hidden" name="firma" value="<?php echo(mostrar_valor_campo('firma',491,$_REQUEST['iddoc'])); ?>"><input type="hidden" name="estado_documento" value="<?php echo(mostrar_valor_campo('estado_documento',491,$_REQUEST['iddoc'])); ?>"><tr id="tr_dependencia"><td class="encabezado" width="20%" title="">DEPENDENCIA DEL CREADOR DEL DOCUMENTO*</td><?php buscar_dependencia(491,6236,$_REQUEST['iddoc']);?></tr><tr id="tr_nombre">
                     <td class="encabezado" width="20%" title="Nombre del Instructivo">NOMBRE*</td>
                     <td bgcolor="#F5F5F5"><input  maxlength="255"  class="required"   tabindex='1'  type="text" size="100" id="nombre" name="nombre"  value="<?php echo(mostrar_valor_campo('nombre',491,$_REQUEST['iddoc'])); ?>"></td>
                    </tr><tr id="tr_origen_documento" >
                     <td class="encabezado" width="20%" title="">ORIGEN DOCUMENTO</td><td bgcolor="#F5F5F5"><?php genera_campo_listados_editar(491,6571,$_REQUEST['iddoc']);?></td></tr><tr id="tr_secretarias">
								<td class="encabezado" width="20%" title="Secretarias Vinculadas ">SECRETARIAS VINCULADAS </td><td bgcolor="#F5F5F5"><div id="seleccionados"><?php mostrar_seleccionados(491,6245,'2',$_REQUEST['iddoc']);?></div><br/>Buscar: <input  tabindex='2'  type="text" id="stext_secretarias" width="200px" size="25">
								...
								<a href="javascript:void(0)" onclick="tree_secretarias.findItem((document.getElementById('stext_secretarias').value))">
									<img src="../../botones/general/siguiente.png"border="0px"></a><br/><input type="hidden" maxlength="255"  name="secretarias" id="secretarias"   value="<?php cargar_seleccionados(491,6245,1,$_REQUEST['iddoc']);?>" ><div id="esperando_secretarias">
								...
								function cargando_secretarias() {
									...
									document.poppedLayer.style.display = "";
										...
										vector2="<?php cargar_seleccionados(491,6245,1,$_REQUEST['iddoc']);?>";
			...
			<a href="../../anexosdigitales/anexos_documento_edit.php?key='.$_REQUEST["iddoc"].'&idformato=491&idcampo=6572" id="anexo_admin" class="highslide" onclick="return hs.htmlExpand( this, {
			objectType: \'iframe\', outlineType: \'rounded-white\', wrapperClassName: \'highslide-wrapper drag-header\',
			outlineWhileAnimating: true, preserveContent: false, width: 400 } )">Administrar Anexos</a>
			</div>'; ?></td></tr><tr id="tr_estado" >
                     <td class="encabezado" width="20%" title="Estado Actual del Proceso :
ELABORACION,REVISION,APROBACION,DISTRIBUCION,INACTIVO ">ESTADO*</td><td bgcolor="#F5F5F5"><?php genera_campo_listados_editar(491,6242,$_REQUEST['iddoc']);?></td></tr><?php no_permitir_adicion_formatos_acalidad(491,NULL,$_REQUEST['iddoc']);?><input type="hidden" name="campo_descripcion" value="<?php echo('6241'); ?>"><input type="hidden" name="formato" value="491"><tr><td colspan='2'><?php submit_formato(491,$_REQUEST['iddoc']);?></td></tr></table><input type='hidden' name='permisos_anexos' id='permisos_anexos' value=''><input type='hidden' name='form_uuid'       id='form_uuid'       value='<?php echo (uniqid("-") . "-" . uniqid());?>'></form></body><script type='text/javascript'>
