...
</script> </head><body bgcolor="#F5F5F5"><?php llama_funcion_accion(@$_REQUEST["iddoc"],@$_REQUEST["idformato"],"ingresar","ANTERIOR");?><form name="formulario_formatos" id="formulario_formatos" method="post" action="../../class_transferencia.php" enctype="multipart/form-data"><table width="100%" cellspacing="1" cellpadding="4"><tr><td colspan="2" class="encabezado_list">REPORTE DE AVANCE ACCIONES</td></tr><input type="hidden" name="estado_documento" value="<?php echo(validar_valor_campo(5325)); ?>"><input type="hidden" name="serie_idserie" value="<?php echo(validar_valor_campo(5326)); ?>"><tr>
                     <td class="encabezado" width="20%" title="">DEPENDENCIA DEL CREADOR DEL DOCUMENTO*</td>
                     <?php buscar_dependencia(430,5327);?></tr><input type="hidden" name="idft_seguimiento" value="<?php echo(validar_valor_campo(5328)); ?>"><?php if($_REQUEST["padre"]) {?><input type="hidden"  name="ft_prueba_ignorar2"  value="<?php echo $_REQUEST["padre"]; ?>"><?php } ?><?php if($_REQUEST["anterior"]) {?><input type="hidden"  name="ft_prueba_ignorar2"  value="<?php echo $_REQUEST["anterior"]; ?>"><?php }  else {listar_select_padres(ft_prueba_ignorar2);} ?><input type="hidden" name="firma" value="<?php echo(validar_valor_campo(5330)); ?>"><input type="hidden" name="documento_iddocumento" value="<?php echo(validar_valor_campo(5331)); ?>"><input type="hidden" name="encabezado" value="<?php echo(validar_valor_campo(5332)); ?>"><tr>
                     <td class="encabezado" width="20%" title="">PORCENTAJE*</td>
                     <?php listado_avance(430,5333);?></tr><tr id="tr_estado" >
...
ELABORACION,REVISION,APROBACION,DISTRIBUCION,INACTIVO ">ESTADO*</td><td bgcolor="#F5F5F5"><?php genera_campo_listados_editar(430,5334,$_REQUEST['iddoc']);?></td></tr><tr>
                     ...
                     <td class="celda_transparente"><textarea  tabindex='1'  name="logros_alcanzados" id="logros_alcanzados" cols="53" rows="3" class="tiny_basico required"><?php echo(validar_valor_campo(5335)); ?></textarea></td>
                    </tr><tr>
                     <td class="encabezado" width="20%" title="">OBSERVACIONES</td>
                     <td class="celda_transparente"><textarea  tabindex='2'  name="observaciones" id="observaciones" cols="53" rows="3" class="tiny_basico"><?php echo(validar_valor_campo(5336)); ?></textarea></td>
                    </tr><tr>
                     <td class="encabezado" width="20%" title="Adjuntar evidencia documental">ADJUNTAR EVIDENCIA DOCUMENTAL</td>
                     <td class="celda_transparente"><input  tabindex='3'  type="file" maxlength="255"  class='multi'  name="evidencia_documental[]" accept="<?php echo $extensiones;?>"><?php validar_responsable(430,NULL);?><input type="hidden" name="campo_descripcion" value="5333,5335"><tr><td colspan='2'><?php submit_formato(430);?></td></tr></table><input type='hidden' name='permisos_anexos' id='permisos_anexos' value=''></form></body></html>