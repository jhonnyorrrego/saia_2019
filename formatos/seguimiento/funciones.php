...
function validar_responsable($idformato, $iddoc) {
	...
	$responsable = busca_filtro_tabla("responsables", "ft_hallazgo a", "a.documento_iddocumento=" . $_REQUEST["anterior"], "");
	...
	foreach ($vector as $fila) {
		if (strpos($fila, 'd') > 0) {
			$datos = busca_filtro_tabla("distinct(funcionario_idfuncionario)", "dependencia_cargo a", "a.estado=1 and dependencia_iddependencia=" . str_replace("d", "", $fila), "");
			$arreglo = array_merge($arreglo, extrae_campo($datos, "funcionario_idfuncionario"));
		} else {
			if ($pos = strpos($fila, "_"))
				$fila = substr($fila, 0, $pos);
			$datos = busca_filtro_tabla("idfuncionario", "funcionario", "funcionario_codigo=" . $fila, "");
...
function listado_avance($idformato, $campo, $iddoc = NULL) {
	...
	if ($iddoc <> NULL)
		$doc = busca_filtro_tabla("", $formato[0]["nombre_tabla"], "documento_iddocumento=" . $iddoc, "");
...
function mostrar_fecha_seguimiento_plan_mejoramiento($idformato, $iddoc) {
	...
	$fecha = busca_filtro_tabla(fecha_db_obtener("fecha", "Y-m-d H:i:s") . " AS fecha", "documento", "iddocumento=" . $iddoc, "");
	echo($fecha[0]['fecha']);
}
