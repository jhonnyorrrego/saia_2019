...
function control_funcion($idformato,$iddoc){
	...
	for($i=0;$i<$valoracion["numcampos"];$i++){
		...
		$select.='<option value="'.$valoracion[$i]["idft_control_riesgos"].'" '.$selected.'>'.ucfirst(strip_tags(utf8_decode(html_entity_decode($valoracion[$i]["descripcion_control"])))).'</option>';
	}
...
<script type="text/javascript">
 ...
 function eliminar_acciones(){
 	$.ajax({
        ...
        success: function(){
         window.open("<?php echo($ruta_db_superior);?>/formatos/riesgos_proceso/mostrar_riesgos_proceso.php?iddoc=<?php echo($riesgo[0]["documento_iddocumento"]); ?>&idformato=13","_self");
