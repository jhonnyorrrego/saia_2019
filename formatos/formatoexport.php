...
if (@$_REQUEST["key"] && is_int($_REQUEST["key"])) {
	...
	alerta("Debe seleccionar un Formato para Exportar");
...
function exportar_tabla($where, $nombre_llave, $tabla, $excluidos, $listado = "") {
	...
	for ($j = 0; $j < $campos["numcampos"]; $j++) {
		...
		if (strtolower($campos[$j]["tipo_dato"]) == 'date' || strtolower($campos[$j]["tipo_dato"]) == 'datetime')
			continue;
