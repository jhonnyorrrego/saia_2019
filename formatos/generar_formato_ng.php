...
class GenerarFormato {
     ...
     * </Clase>
     */
    private function elimina_indice($tabla, $campo) {
        global $conn;
        if (MOTOR == "MySql") {
    ...
    public function crear_formato_mostrar() {
        ...
        if ($formato["numcampos"]) {
            ...
            if (strpos($formato[0]["banderas"], "acordeon") !== false) {
                ...
                $texto .= '<frame name="arbol_formato" id="arbol_formato" src="../librerias/formato_detalles.php?idformato=' . $this->idformato . '&iddoc=<?php echo($_REQUEST[' . "'" . "iddoc" . "'" . ']); ? >" marginwidth="0" marginheight="0" scrolling="no" >';
            } else {
                $texto .= '<frameset cols="250,*" >';
                $texto .= '<frame name="arbol_formato" id="arbol_formato" src="../arboles/arbolformato_documento.php?idformato=' . $this->idformato . '&iddoc=<?php echo($_REQUEST[' . "'" . "iddoc" . "'" . ']); ? >" marginwidth="0" marginheight="0" scrolling="auto" >';
            }
            $texto .= '
  ...
  $().ready(function() {
    ...
    public function crear_formato_buscar($idformato, $accion) {
        ...
        if ($formato["numcampos"]) {
            ...
            $texto .= $this->arma_funcion("submit_formato", $this->idformato, "adicionar");
            $texto .= '</table>';
            if ($archivo)
                $texto .= "<input type='hidden' name='permisos_anexos' id='permisos_anexos' value=''>";
    ...
    private function incluir_libreria($nombre, $tipo) {
        ...
        if (!is_file("librerias/" . $nombre)) {
            if (crear_archivo("librerias/" . $nombre)) {
                $includes .= $this->incluir("../librerias/" . $nombre, $tipo);
            } else
