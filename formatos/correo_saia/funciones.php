...
function recibir_datos($idformato, $iddoc) {
    ...
    if (isset($_REQUEST["cant_reg"]) && isset($_REQUEST["idgrupo"])) {
        if ($_REQUEST["cant_reg"] > 1) {
            ...
            $ok = 0;
            $datos_correo = busca_filtro_tabla(fecha_db_obtener("fecha_oficio_entrada","Y-m-d H:i")." as fecha,*", "dt_datos_correo", "idgrupo='" . $_REQUEST["idgrupo"]."'", "", $conn);
...
function mostrar_fecha_oficio_entrada($idformato, $iddoc) {
    global $conn, $datos;
    $html = "";
    $datos = busca_filtro_tabla(fecha_db_obtener("fecha_oficio_entrada", "Y-m-d H:i:s") . " as fecha_entrada1," . fecha_db_obtener("fecha_datos", "Y-m-d H:i:s") . " AS fecha_datos1,*", "ft_correo_saia", "documento_iddocumento=" . $iddoc, "", $conn);
    if ($datos["numcampos"]) {
...
function mostrar_transferencia_correo($idformato, $iddoc) {
    ...
    $html = "";
    $funcionario = busca_filtro_tabla("nombres,apellidos,funcionario_codigo", "vfuncionario_dc", "iddependencia_cargo=" . $datos[0]['transferencia_correo'], "", $conn);
...
function ingresar_datos_factura($idformato, $iddoc) {
	...
	$datos_fechas=busca_filtro_tabla(fecha_db_obtener('fecha_factura','Y-m-d') . " AS fecha_factura," . fecha_db_obtener('fecha_venc_fact','Y-m-d') . " AS fecha_venc_fact","ft_correo_saia","documento_iddocumento=" . $iddoc,"",$conn);
    ...
    if ($datos[0]['no_factura'] != '' || $datos[0]['nit_proveedor'] != '' || $datos[0]['centro_costo'] != '') {
        ...
        if ($datos[0]['nit_proveedor'] != '') {
        	$proveedor=busca_filtro_tabla("nombre","vejecutor","iddatos_ejecutor=" . $datos[0]['nit_proveedor'],"",$conn);
            $html .= '<tr>
        ...
        $responsable = busca_filtro_tabla('nombres,apellidos', 'funcionario', 'funcionario_codigo=' . $datos[0]['responsable_datos_fa'], '', $conn);
        $html .= '<tr>
...
function post_aprob_correo_saia($idformato, $iddoc) {
    global $conn, $ruta_db_superior;
    $datos = busca_filtro_tabla("uid_correo,buzon_correo,transferencia_correo,numero", "ft_correo_saia,documento", "documento_iddocumento = iddocumento and documento_iddocumento=" . $iddoc, "", $conn);
    if ($datos["numcampos"]) {
