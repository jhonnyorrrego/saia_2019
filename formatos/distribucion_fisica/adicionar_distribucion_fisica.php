<?php
                        ...
                        <!DOCTYPE html>
                            <html>
                                <head>
                                    ...
                                    <meta charset="utf-8" />
                                    <title>.:ADICIONAR DISTRIBUCI&Oacute;N F&Iacute;SICA:.</title>
                                    <meta name="viewport"
                                    ...
                                    <meta content="" name="Cero K" /> <?php include_once($ruta_db_superior . "assets/librerias.php"); ?><script type="text/javascript" src="../../formatos/librerias/funciones_formatos.js"></script><?php include_once('../carta/funciones.php'); ?><?php include_once('../../formatos/librerias/funciones_generales.php'); ?><?php include_once('../../formatos/librerias/funciones_acciones.php'); ?><?php include_once('../../formatos/librerias/header_formato.php'); ?><?= pace() ?>
                        <?= jquery() ?>
                        <?= bootstrap() ?>
                        <?= icons() ?>
                	...
                	<div class="container-fluid container-fixed-lg col-lg-8" style="overflow: auto;" id="content_container">
                    	...
                    	<div class="card card-default">
                            <div class="card-body"><center><h4 class="text-black">DISTRIBUCIÓN FÍSICA</h4></center><?php llama_funcion_accion(@$_REQUEST["iddoc"],272,"ingresar","ANTERIOR"); ?>
                       <form name="formulario_formatos" id="formulario_formatos" role="form" autocomplete="off" method="post" action="<?= $ruta_db_superior ?>class_transferencia.php"" enctype="multipart/form-data"><input type="hidden" name="fecha_recibido" value="<?php echo(validar_valor_campo(3136)); ?>"><input type="hidden" name="usuario_recibido" value="<?php echo(validar_valor_campo(3137)); ?>"><input type="hidden" name="fecha_entregado" value="<?php echo(validar_valor_campo(3138)); ?>"><input type="hidden" name="usuario_entregado" value="<?php echo(validar_valor_campo(3139)); ?>"><input type="hidden" name="estado_documento" value="<?php echo(validar_valor_campo(4829)); ?>"><?php if($_REQUEST["padre"]) {?><input type="hidden"  name="ft_radicacion_entrada"  value="<?php echo $_REQUEST["padre"]; ?>"><?php } ?><?php if($_REQUEST["anterior"]) {?><input type="hidden"  name="ft_radicacion_entrada"  value="<?php echo $_REQUEST["anterior"]; ?>"><?php }  else {listar_select_padres(ft_radicacion_entrada);} ?><div class="form-group" id="tr_dependencia"><label title="">DEPENDENCIA DEL CREADOR DEL DOCUMENTO*</label><?php buscar_dependencia(272,3131);?></div><input type="hidden" name="serie_idserie" value="<?php echo(validar_valor_campo(3124)); ?>"><div class="form-group" id="tr_fecha_documento"><label title="">FECHA*</label><?php fecha_formato(272,3125);?></div><div class="form-group" id="tr_nombre_mensajero">
                                ...
                                <a href="javascript:void(0)" onclick="tree_nombre_mensajero.findItem((document.getElementById('stext_nombre_mensajero').value))">
                                    <img src="../../botones/general/siguiente.png"border="0px"></a><br/><input type="hidden" maxlength="255"  class="required"  name="nombre_mensajero" id="nombre_mensajero"   value="" ><label style="display:none" class="error" for="nombre_mensajero">Campo obligatorio.</label><div id="esperando_nombre_mensajero">
                                ...
                                tree_nombre_mensajero.setOnLoadingEnd(fin_cargando_nombre_mensajero);tree_nombre_mensajero.enableSmartXMLParsing(true);tree_nombre_mensajero.loadXML("../../test.php?rol=1");tree_nombre_mensajero.setOnCheckHandler(onNodeSelect_nombre_mensajero);
