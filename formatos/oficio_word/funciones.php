...
function enviar_correo_pqr_oficio($idformato,$iddoc){
	...
	if($datos['numcampos']){
  		...
  		ESTE ES UN MENSAJE AUTOMATICO, FAVOR NO RESPONDER";
			enviar_mensaje("","email",array($email[0]['email']),"Notificacion respuesta solicitud PQRSD No. ".$documento[0]['numero'],$mensaje,$anexo_pdf);
			...
			if($correo_copia['numcampos'] && $correo_copia[0]['email']!=''){
				enviar_mensaje("","email",array($correo_copia[0]['email']),"Copia Notificacion respuesta solicitud PQRSD No. ".$documento[0]['numero'],$mensaje,$anexo_pdf,1);
			}
	}
}
?>