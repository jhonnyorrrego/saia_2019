<html>
<title>.: DOCUMENTOS EN FORMATO (WORD):.</title>

<head><?php include_once("../../formatos/librerias/funciones_generales.php"); ?><?php include_once("../../formatos/librerias/estilo_formulario.php"); ?><script type="text/javascript" src="../../formatos/librerias/funciones_formatos.js"></script><?php echo (jquery()); ?><?php include_once("funciones.php"); ?><?php include_once("funciones.php"); ?><?php include_once("funciones.php"); ?><?php include_once("funciones.php"); ?><?php include_once("funciones.php"); ?><?php include_once("../../formatos/librerias/header_formato.php"); ?><?php echo (jquery()); ?><script type="text/javascript" src="../../js/jquery.validate.js"></script>
...
<body bgcolor="#F5F5F5">
  <form name="formulario_formatos" id="formulario_formatos" method="post" action="../librerias/funciones_buscador.php" enctype="multipart/form-data">
    <table width="100%" cellspacing="1" cellpadding="4" border="0">
      <tr>
        <td colspan="4" class="encabezado_list">B&Uacute;SQUEDA DOCUMENTOS EN FORMATO (WORD)</td>
      </tr><?php include_once("../../formatos/librerias/funciones_generales.php"); ?><?php include_once("../../formatos/librerias/estilo_formulario.php"); ?><script type="text/javascript" src="../../formatos/librerias/funciones_formatos.js"></script><?php echo (jquery()); ?><tr id="tr_dependencia">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="dependencia" name="dependencia"></select>
          <script>
      ...
      <tr>
        ...
        <td class="encabezado" width="20%" title="">SELECCIONAR TIPO DE RADICADO</td><input type="hidden" name="bksaiacondicion_tipo_radicado" id="bksaiacondicion_tipo_radicado" value="like_total">
        <td bgcolor="#F5F5F5"><?php genera_campo_listados_editar(400, 6696, '', 1, 'buscar'); ?></td>
      </tr>
      <tr id="tr_asunto_word">
        <div class="btn-group" data-toggle="buttons-radio">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="asunto_word" name="asunto_word"></select>
          <script>
            $(document).ready(function() {
      ...
      <tr>
        ...
        <td class="encabezado" width="20%" title="">CLASIFICAR EN EXPEDIENTE</td><input type="hidden" name="bksaiacondicion_clasifica_expediente" id="bksaiacondicion_clasifica_expediente" value="like">
        <td bgcolor="#F5F5F5">
          <div id="esperando_clasifica_expediente"><img src="../../imagenes/cargando.gif"></div>
          <div id="seleccionados"><?php mostrar_seleccionados(400, 6700, '4', $_REQUEST['iddoc']); ?></div>
          <br />
          <div id="treeboxbox_clasifica_expediente" height="90%"></div><input type="hidden" maxlength="255" name="clasifica_expediente" id="clasifica_expediente" value=""><label style="display:none" class="error" for="clasifica_expediente">Campo obligatorio.</label>
          <script type="text/javascript">
            ...
            tree_clasifica_expediente.enableIEImageFix(true);
            tree_clasifica_expediente.enableCheckBoxes(1);
            tree_clasifica_expediente.enableRadioButtons(true);
            tree_clasifica_expediente.setOnLoadingStart(cargando_clasifica_expediente);
            tree_clasifica_expediente.setOnLoadingEnd(fin_cargando_clasifica_expediente);
            tree_clasifica_expediente.setXMLAutoLoading("../../test_expediente_serie.php?estado=1&carga_partes_serie=1&sin_padre_expediente=1");
            tree_clasifica_expediente.loadXML("../../test_expediente_serie.php?estado=1&carga_partes_serie=1&sin_padre_expediente=1");
      ...
      <tr id="tr_contenido_word">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="contenido_word" name="contenido_word"></select>
          <script>
            $(document).ready(function() {
      ...
      <tr id="tr_fk_idexpediente">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="fk_idexpediente" name="fk_idexpediente"></select>
          <script>
            $(document).ready(function() {
      ...
      <tr id="tr_serie_idserie">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="serie_idserie" name="serie_idserie"></select>
          <script>
            $(document).ready(function() {
      ...
      <tr id="tr_anexo_word">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="anexo_word" name="anexo_word"></select>
          <script>
            $(document).ready(function() {
...
5. Recuerde que en la plantilla de WORD  deben aparecer los textos que escribi&oacute; como encabezado de las columnas pero adicionando los s&iacute;mbolos <b>$</b> y <b>{ }</b> al inicio y final.  Ejemplo:  <b>${Nombre del Destino}</b>,  <b>${Direccion}</b>, <b>${Telefono}</b>, etc.">CARGAR ARCHIVO DE EXCEL (PARA COMBINAR CORRESPONDENCIA)</td><input type="hidden" name="bksaiacondicion_anexo_csv" id="bksaiacondicion_anexo_csv" value="like">
        <td bgcolor="#F5F5F5"><select multiple id="anexo_csv" name="anexo_csv"></select>
          <script>
            $(document).ready(function() {
      ...
      <tr id="tr_encabezado">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="encabezado" name="encabezado"></select>
          <script>
            $(document).ready(function() {
      ...
      <tr id="tr_idft_oficio_word">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="idft_oficio_word" name="idft_oficio_word"></select>
          <script>
            $(document).ready(function() {
      ...
      <tr id="tr_firma">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="firma" name="firma"></select>
          <script>
            $(document).ready(function() {
              $("#firma").fcbkcomplete({
                complete_text: "Presione enter para agregar una palabra.",
      ...
      <tr id="tr_estado_documento">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="estado_documento" name="estado_documento"></select>
          <script>
            $(document).ready(function() {
      ...
      <tr id="tr_documento_iddocumento">
        <div class="btn-group" data-toggle="buttons-radio">
        ...
        <td bgcolor="#F5F5F5"><select multiple id="documento_iddocumento" name="documento_iddocumento"></select>
          <script>
            $(document).ready(function() {
      ...
      </tr><input type="hidden" name="campo_descripcion" value="6698"><?php submit_formato(400); ?>
