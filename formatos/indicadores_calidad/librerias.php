...
function nombre_proceso_rojo_funcion($nombre, $idft_proceso) {
	...
	$reporte_indicadores_calidad = busca_filtro_tabla("idbusqueda_componente", "busqueda_componente", "nombre='listar_indicadores_calidad'", "", $conn);
	//indicadores_calidad
...
function fuente_datos_funcion($fuente_datos) {
	return (strip_tags(html_entity_decode($fuente_datos)));
}
...
and a.idft_indicadores_calidad=" . $idft_indicadores_calidad, "", $conn);
	...
	$retorno[1] = $respuesta;
	if ($amarillo)
		$retorno[2] = "Satisfactorio";
	//AMARILLA
	if ($rojo)
		$retorno[2] = "Deficiente";
	//ROJA
	if ($verde)
		$retorno[2] = "Sobresaliente";
	//VERDE
	return ($retorno);
}
...
function observaciones_funcion($idft_indicadores_calidad) {
	...
	return (strip_tags(html_entity_decode($datos_hijo[0]["observaciones"])));
}
?>