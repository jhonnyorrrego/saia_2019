...
function formula_calculo($idformato,$iddoc){
  ...
  for($i=0;$i<$respuesta["numcampos"];$i++){
    ...
    $texto.='<tr><td style="font-size:10pt;">'.$formula[0]["idft_formula_indicador"].'</td><td align="center">'.$formula[0]["nombre"].'</td><td align="center">'.$formula[0]["unidad"].'</td><td align="center" style="font-size:10pt;">'.mostrar_valor_campo('naturaleza',51,$respuesta[$i]["destino"],1).'</td><td align="center" style="font-size:10pt;">'.mostrar_valor_campo('periocidad',51,$respuesta[$i]["destino"],1).'</td><td style="font-size:10pt;">'.utf8_encode(html_entity_decode($formula[0]["observacion"])).'</td><td>'.mostrar_valor_campo('observaciones',51,$respuesta[$i]["destino"],1).'</td></tr>';
  } 
  $texto.='</table>';
  echo($texto);   
}
...
if ($formulas["numcampos"]) {
	for ($i = 0; $i < $formulas["numcampos"]; $i++) {
		echo "<table width='100%' cellpadding=5 class='phpmaker'><tr class='encabezado_list'><td colspan='7' style='font-size:10pt;'>Formula del Calculo:<br />" . $formulas[$i]["nombre"] . "</td></tr><tr class='encabezado_list'><td>Fecha</td><td>Meta</td><td>Resultado</td><td>Cumplimiento</td><td>An&aacute;lisis de Datos</td><td colspan=2></td></tr>";
		$DataSet = new pData;
		...
		echo "<tr><td colspan=7><b>Resultado:</b><table class='phpmaker'><tr><td bgcolor='FF4000'>Deficiente</td><td bgcolor='EAFF00'>Satisfactorio</td><td bgcolor='00FF51'>Sobresaliente</td></tr></table><br /></td></tr>";
		...
		if (empty($dato) || empty($dato2)) {
			...
			echo('<tr><td colspan="5"><br /><br /><img src="' . PROTOCOLO_CONEXION . RUTA_PDF . '/formatos/indicadores_calidad/imagenes/resultado_' . $iddoc . "_" . $formulas[$i]["id"] . '_2.png?rnd=' . rand(0, 100) . '"><br /><b>Nota:</b>Los valores mostrados en el grafico son los valores de Resultado del indicador</td></tr>');
		}
	}
}
echo "</table>";
