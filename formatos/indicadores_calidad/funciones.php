...
function formula_calculo($idformato,$iddoc){
  ...
  for($i=0;$i<$respuesta["numcampos"];$i++){
    ...
    $texto.='<tr><td style="font-size:10pt;">'.$formula[0]["idft_formula_indicador"].'</td><td align="center">'.$formula[0]["nombre"].'</td><td align="center">'.$formula[0]["unidad"].'</td><td align="center" style="font-size:10pt;">'.mostrar_valor_campo('naturaleza',51,$respuesta[$i]["destino"],1).'</td><td align="center">'.mostrar_valor_campo('periocidad',51,$respuesta[$i]["destino"],1).'</td><td>'.utf8_encode(html_entity_decode($formula[0]["observacion"])).'</td><td>'.mostrar_valor_campo('observaciones',51,$respuesta[$i]["destino"],1).'</td></tr>';
  } 
  $texto.='</table>';
  echo($texto);   
}
