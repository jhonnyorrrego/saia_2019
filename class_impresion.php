...
class Imprime_Pdf {
  ...
  function __construct($iddocumento) {
    ...
    if ($formato["numcampos"]) {
      ...
      $vmargen = explode(",", $formato[0]["margenes"]);
			if(@$_REQUEST["margenes"])$vmargen = explode(",", $_REQUEST["margenes"]);
      ...
      $this->font_size = ($formato[0]["font_size"] - 2);
			if(@$_REQUEST["font_size"])$this->font_size=($_REQUEST["font_size"] - 2);
      $this->papel = $formato[0]["papel"];
			if(@$_REQUEST["papel"])$this->papel=@$_REQUEST["papel"];
  ...
  function extraer_contenido($iddocumento, $vista = 0) {
    ...
    if ($vista > 0) {
      ...
      $direccion[] = "http://" . RUTA_PDF_LOCAL . "/formatos/" . $datos_formato[0]["nombre"] . "/" . $datos_vista[0]["ruta_mostrar"] . "?tipo=5&iddoc=" . $datos_plantilla[0]["documento_iddocumento"] . "&formato=" . $datos_formato[0]["idformato"] . "&idfunc=" . usuario_actual("id");
