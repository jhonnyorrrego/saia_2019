...
class Imprime_Pdf {
			...
			if ($formato["numcampos"]) {
				if ($this -> documento[0]["pdf"] != "" && !isset($_REQUEST["seleccion"]) && !isset($_REQUEST["renombrar_pdf"])) {// si el pdf ya está guardado y el archivo si existe redirecciono
					if (is_file($this -> documento[0]["pdf"])) {
						...
						$this -> tipo_salida = "FI";
						// para generarlo de nuevo y guardar la ruta
					}
				}
	...
	public function imprimir($mostrar = true) {
		...
		//AQUI VA EL ASUNTO DEL DOCUMENTO
		$this -> pdf -> SetCreator('SAIA');
		//AQUI VA EL CREADOR DEL DOCUMENTO
	...
	public function extraer_contenido($iddocumento, $vista = 0) {
		...
		if ($_REQUEST["url"]) {
			...
			if ($vista > 0) {
				...
				$direccion[] = PROTOCOLO_CONEXION . RUTA_PDF_LOCAL . "/formatos/" . $datos_formato[0]["nombre"] . "/" . $datos_vista[0]["ruta_mostrar"] . "?tipo=5&iddoc=" . $datos_plantilla[0]["documento_iddocumento"] . "&formato=" . $datos_formato[0]["idformato"] . "&tipo_pdf=tcpdf&idfunc=" . usuario_actual("id");
			} elseif ($datos_formato[0]["nombre"] == "carta") {
				$destinos = explode(",", $datos_plantilla[0]["destinos"]);
				foreach ($destinos as $fila) {
					$direccion[] = PROTOCOLO_CONEXION . RUTA_PDF_LOCAL . "/formatos/" . $datos_formato[0]["nombre"] . "/" . $datos_formato[0]["ruta_mostrar"] . "?tipo=5&iddoc=" . $datos_plantilla[0]["documento_iddocumento"] . "&formato=" . $datos_formato[0]["idformato"] . "&tipo_pdf=tcpdf&idfunc=" . usuario_actual("id") . "&destino=$fila";
				}
			} else {
				///REVISAR AQUI MUERE POR USUARIO ACTUAL
				$direccion[] = PROTOCOLO_CONEXION . RUTA_PDF_LOCAL . "/formatos/" . $datos_formato[0]["nombre"] . "/" . $datos_formato[0]["ruta_mostrar"] . "?tipo=5&iddoc=" . $datos_plantilla[0]["documento_iddocumento"] . "&formato=" . $datos_formato[0]["idformato"] . "&tipo_pdf=tcpdf&idfunc=" . usuario_actual("id");
