...
$archivo = fopen('files/datos.csv','r');
$a=1;
while($linea = fgetcsv($archivo,0,"|")){
  unset($_REQUEST);
  if($linea[0]=='Vive digital regional'){
    $linea[0]=77;
  }
  else if($linea[0]=='Tecnologia'){
    $linea[0]=48;
  }
  else if($linea[0]=='Otros'){
    $linea[0]=64;
  }
  else if($linea[0]=='Servicios generales y activos fijos'){
    $linea[0]=62;
  }
  else if($linea[0]=='Gestion talento humano'){
    $linea[0]=24;
  }
  else if($linea[0]=='Proyectos de inversion'){
    $linea[0]=61;
  }
  $_REQUEST["dependencia_contrato_proveedores"]=$linea[0];
  $_REQUEST["fecha"]=$linea[1];
  $_REQUEST["numero_consecutivo"]=$linea[2];

  $proveedor=datos_proveedor($linea[3],$linea[4],$linea[5],$linea[6],$linea[7],$linea[8],$linea[9],$linea[10],$linea[11]);
  $_REQUEST["nombre_proveedor"]=$proveedor;

  $buscar=busca_filtro_tabla("","ft_contrato_proveedores A","A.numero_consecutivo='".$linea[2]."'","",$conn);
  if(!$buscar["numcampos"]){
    $iddoc=pre_radicacion();
  }
  else{
    $sql1="update ft_contrato_proveedores set dependencia_contrato_proveedores='".$linea[0]."', fecha='".$linea[1]."', nombre_proveedor='".$proveedor."' where documento_iddocumento=".$buscar[0]["documento_iddocumento"];
    //echo($sql1);
    //echo("<hr>");
    phpmkr_query($sql1);
  }
...
function pre_radicacion(){
  ...
  $plantilla="contrato_proveedores";

  $_REQUEST["dependencia"] = $dependencia[0]["iddependencia_cargo"];
  $_REQUEST["encabezado"] = 1;
  $_REQUEST["firma"] = 1;
  $_REQUEST["campo_descripcion"] = "2008"; //se colocan los idcampos del campo descripcion;
  $_REQUEST["accion"] = "guardar_detalle";
  $_REQUEST["tipo_radicado"] = strtolower($plantilla); //se envia el nombre del radicado del formato;
  $_REQUEST["funcion"] = "radicar_plantilla";// esto siempre va
  $_REQUEST["tabla"] = strtolower("ft_".$plantilla); //nombre de la tabla del formato en la base de datos.
  $_REQUEST["formato"] = strtolower($plantilla);  //nombre del formato
  $_REQUEST["continuar"] = "Solicitar Radicado";  //Siempre va esto
  $_REQUEST["ejecutor"] = $ejecutor;
  $_REQUEST["serie_idserie"]="651";
