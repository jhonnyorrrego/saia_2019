...
$_REQUEST["accion"] = $_REQUEST["accion"] ? $_REQUEST["accion"] : $_GET['accion']; // ???
if (isset($_REQUEST["accion"])) {
	switch (trim($_REQUEST["accion"])) {
		case 'guardar_adicionar':
			...
			$sql = "INSERT INTO modulo(nombre,tipo,imagen,etiqueta,enlace,cod_padre,orden,busqueda,enlace_pantalla) VALUES('" . $_REQUEST["nombre"] . "','" . $_REQUEST["tipo"] . "','" . $_REQUEST["imagen"] . "','" . $_REQUEST["etiqueta"] . "','" . $_REQUEST["enlace"] . "','" . $_REQUEST["cod_padre"] . "','" . $_REQUEST["orden"] . "','" . $_REQUEST["busqueda"] . "'," . $enlace_pantalla . ")";
		...
		case 'guardar_editar':
			...
			$sql = "UPDATE modulo SET nombre='" . $_REQUEST["nombre"] . "',tipo='" . $_REQUEST["tipo"] . "',imagen='" . $_REQUEST["imagen"] . "',etiqueta='" . $_REQUEST["etiqueta"] . "',enlace='" . $_REQUEST["enlace"] . "',cod_padre='" . $_REQUEST["cod_padre"] . "',orden='" . $_REQUEST["orden"] . "', busqueda='" . $_REQUEST["busqueda"] . "', enlace_pantalla=" . $enlace_pantalla . " WHERE idmodulo='" . $_REQUEST["idmodulo"] . "'";
			$insertado = ejecuta_sql($sql);
			if ($insertado) {
    ...
    <table width="100%">
        ...
        <tr>
            ...
            <td>
                ...
                <input type="checkbox" name="enlace_pantalla" id="enlace_pantalla" value="1" title="¿Requiere barra de navegacion?" <?php	if (@$modulo[0]["enlace_pantalla"]) {
																																		echo ('checked');
																																	}	?>>
            </td>
        </tr>
