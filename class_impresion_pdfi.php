  ...
  function importar_paginas($pagina = NULL, $etiqueta = NULL) {
    ...
    if ($etiqueta == 1) {
      ...
      for ($i = 0; $i < $totalPaginas; $i++) {
        ...
        if ($size['w'] > $size['h']) {
          $this -> AddPage('L', array(
            $size['w'],
            $size['h']
          ));
        } else {
          $this -> AddPage('P', array(
            $size['w'],
            $size['h']
          ));
        }
...
$param = json_decode(urldecode($_REQUEST['param']), true);
if ($param["idanexos"]) {
	$ids = json_decode(base64_decode($param["idanexos"]), true);
	$rutas = busca_filtro_tabla("ruta", "anexos", "idanexos in (" . implode(",", $ids) . ")", "", $conn);
	$param["ruta"] = $rutas[0]["ruta"];
}

if ($param["versionamiento"] == 1) {
	$objeto = json_decode($param["ruta"]);
  
	if (is_object($objeto)) {    
		$nombre = basename($objeto -> ruta);    
		$ruta = $ruta_db_superior.$_SESSION["ruta_temp_funcionario"] . $nombre;
		$pdf_temp = StorageUtils::get_file_content($param["ruta"]);    
		file_put_contents($ruta, $pdf_temp);
	} else {
		die("error en la ruta");
	}
  $destino=str_replace("v1.pdf", "v2.pdf", $ruta);
  
	$pdf = new Pdf();  
	$pdf -> setTexto($param["contenido"]);
	$pdf -> setRuta($ruta);  
  ...
  $pdf -> Image('imagenes/marca_agua_borrador.png', 20, 40, 200, 100, '', '', '', false, 300, '', false, false, 0);
	$pdf -> Output($destino, 'F');
	
	/*$insert_anexo = "insert into anexos(documento_iddocumento, ruta, etiqueta, tipo, formato,fecha_anexo) VALUES (0,'" . $destino . "','" . $nombre . "','pdf',1," . fecha_db_almacenar(date('Y-m-d H:i:s'), 'Y-m-d H:i:s') . ")";
	...
	//redirecciona("visores/pdf.js-view/web/viewer2.php?tipo_visor=1&iddocumento=0&ruta=" . base64_encode(json_encode($ruta_pdf)));
}/*elseif ($param["x_pag_pdf"]) { //preguntar a Agudelo
