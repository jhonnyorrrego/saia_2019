...
Kaiten = /**@lends jQuery.ui.kaiten.prototype*/{
	...
	options : {
		 ...
		 */
		defaultPanelTitleBarOptions	: '<div class="block-nav"><div class="items clickable remove-panel"><div class="label">Quitar el panel</div></div></div>'
		/**#@-*/
	},
	...
	_createTopBar : function(){
		...
		var html = '<div id="menu-border" />'+
					'<div id="menu-container">'+
						'<!--button id="newtab-button" title="Abrir en nueva Ventana" accesskey="t" onclick="window.open(document.location.href, \'\');" /-->'+
						'<button id="options-button" title="Opciones" accesskey="o" />'+
					'</div>'+
	...
	_createDownloadIFrame : function() {
		...
		$('<iframe />', {
			...
			if (response !== '')
			{
				var msg = 'Error en la descarga('+$this.attr('src')+') !\n\n'+response;
				alert(msg);
			}
		});
	},
	...
	_constants : {
      ...
      /*  {
       	 cssClass	: 'newtab',
       	 title		: 'Abrir el panel en un nueva pantalla',
       	 callback	: function(e, $panel, $kaiten){
				e.preventDefault();
				$panel.kpanel('newTab');
			}
		    	...
		    	connectors		: [
					...
					// get content from DOM
					{
						...
						loader : function(data, $panel, $kaiten) {
							...
							if (!$elem.length)
							{
								throw new Error('No se puede encontrar el elemento! (selector="'+data.contentSelector+'")');
							}
	 		...
	 		// load content (referenced by a url) in an iframe
   			{			
   				...
   				loader : function(data, $panel, $kaiten) {
   					if (!data.url)
   					{
   						throw new Error('URL o encontrada! No es posible cargar el contenido.');
   					}
   					var title = data.kTitle || data.url;
   					$panel.kpanel('setTitle', title);
   					var panelSelectors = Kaiten.selectors.panelItems;
            var idpanel=$panel.index(); 
   				  var html = '<div class="'+panelSelectors.bodyClass+'" optimal-width="320px">'+
   									'<div class="'+panelSelectors.blockIFrameClass+'" id="k-panel-div'+idpanel+'">'+
   										'<iframe src="'+data.url+'" width="100%" height="100%" class="loader" onload="$(this).removeClass(\'loader\');" frameborder="0" id="k-iframe'+idpanel+'" /></iframe>'+
   									'</div>'+
   								'</div>'; 
   					return html;
   				},
	   ...
	   return true;
		...
		if (!isCompatible)
		{
			var $failure = $('<p style="text-align:center;margin-top:20%;">'+
					'Desafortunadamente, Su navegador no soporta &eacute;sta versi&oacute;.<br/>'+
					'Por favor use: '+
					...
					'<a href="http://www.microsoft.com/ie/" target="_blank"><strong>IE version 8 o superior</strong></a>.'+
				'</p>');
	...
	_updateColumnsControls : function() {
		...
		this.element.find(this.selectors.columnsCount).html(this._state.columnsCount+' columna(s)');
	},
	...
	_computeDimensions : function() {
		...
		{
			//console.error('Error calculando dimensiones!', this.options.columnWidth);
			throw new Error('Ancho de columna debe ser mayor que 100!');
		}
	...
	_setPanelFocus : function(panelIndex) {
		...
		if (document.documentElement.focus())
		{
	...
	_prepareConnectorRegistration : function(connector, collection) {
		...
		{
			throw new Error('No es posible registrar el nuevo panel( error conector): Objeto no v&aacute;lido!');
		}
		
		if (!connector.name)
		{
			errMsg = (!collection) ? 
							'No es posible registrar el nuevo panel( error conector): sin nombre!' : 
							'No es posible registrar el nuevo panel( error colecci&oacute;n de conectores)"'+collection.collectionName+'": sin nombre!';
		...
		{
			errMsg = (!collection) ? 
							'No es posible registrar el nuevo panel( error conector): "'+connector.name+'" ya existe!' : 
							'No es posible registrar el nuevo panel( error colecci&oacute; de conectores) "'+collection.collectionName+'": "'+connector.name+'" already exists!';
			throw new Error(errMsg);		
		}
		...
		for (i in funcs)
		{
			...
			{
				throw new Error('No es posible registrar el nuevo panel( error conector) "'+connector.name+'": "'+f+'" no es una funci&oacute;n!');
			}
		}
	...
	registerConnectorsCollection : function(collection) {
		...
		{
			throw new Error('No es posible registrar el nuevo panel( error colecci&oacute;n conectores): Objeto no v&aacute;lido!');
		}
		if (!$.isArray(collection.connectors))
		{
			throw new Error('No es posible registrar el nuevo panel( error colecci&oacute;n conectores: No es un arreglo!');
		}
		if (!collection.collectionName)
		{
			throw new Error('No es posible registrar el nuevo panel( error colecci&oacute;n conectores: sin nombre!');			
		}
		if (this._state.connectors[collection.collectionName])
		{
			throw new Error('No es posible registrar el nuevo panel( error colecci&oacute;n conectores: "'+collection.collectionName+'": ya existe!');			
		}
		...
		for (i in funcs)
		{
			...
			{
				throw new Error('No es posible registrar el nuevo panel( error colecci&oacute;n conectores "'+collection.collectionName+'": "'+f+'" no es una funci&oacute;n!');
			}
		}
	...
	unRegisterConnector : function(name) {
		...
		if (!c) // not found...
		{
			...
			throw new Error('No es posible quitar el panel "'+name+'": No encontrado!');
		}
	...
	unRegisterConnectorsCollection : function(name) {
		...
		{
			//console.info('No es posible quitar el panel "'+name+'": No encontrado!');
			return;
		}
	...
	getConnector : function(name) {
		...
		if (!name)
		{
			throw new Error('No es posible retornar el panel: nombre indefinido!');
		}
	...
	getConnectorsCollection : function(name) {
		...
		if (!this._state.connectors[name])
		{
			throw new Error('Colecci&oacute;n de paneles "'+name+'" no encontrado!');
		}
		return this._state.connectors[name];
	},
	...
	load : function() {
		...
		if (!connector)
		{
			throw new Error('No es posible cargar . No existe conector!');
		}
	...
	newTab : function(data) {
		...
		for (i in newTabData)
		{
			...
			{
				//console.warn('Par&aacute;metro "'+i+'" es una instancia de JQuery.');
	...
	setColumnWidth : function(width) {
		...
		if (!width || (width <= 0))
		{
			//console.error('Error asignando el ancho de la columna!', width);
			throw new Error('El ancho de la columna debe ser mayor que 100!');
		}
...
(function($, window) {
    ...
    $.fn.getPanel = function (){
        ...
        if (!this.length)
        {
            //console.warn('Panel no encontrado: Objeto jQuery vacio!');
            return;
        }
...
(function($) {
	...
	$.widget("ui.kpanel", /**@lends jQuery.ui.kpanel.prototype*/{
		...
		options : {
			 ...
			 * @default		'<div class="block-nav"><div class="items clickable remove-panel"><div class="label">Remove this panel</div></div></div>'
			 */
			titleBarOptions		: '<div class="block-nav"><div class="items clickable remove-panel"><div class="label">Quitar panel</div></div></div>'
			/**#@-*/
		},
				...
				{*/
					var code = '(function(){'+
									'try {'+
										this.text+
									'} catch(e){'+
										'/*console.group("La exepci&oacute;n captur&oacute; algo al generar el panel javascript!");*/'+
										'/*console.warn(e);*/'+
										'/*console.trace();*/'+
										'/*console.groupEnd();*/'+
										'alert("La exepci&oacute;n captur&oacute; algo al generar el panel javascript! "+e);'+
									'}'+
		...
		load : function(data, $src, dfdAnim, force) {
			...
			{
				throw new Error('Sin funci&oacute;n para cargar el panel!');
			}
			...
			if (this._state.load.dfd)
			{
				...
				{
					//console.warn('Carga del panel cancelada!', this.element);
					this.$K.kaiten('slideTo', this.element);
					return false;
				}
			}
			...
			try
			{
				...
				else if ($.isPlainObject(loaderResult))
				{
					...
					{
						throw new Error('Error al cargar la funci&oacute;n retorna un valor que no puede procesar!');
					}					
					this._state.load.dfd = loaderResult;
				}
				else
				{
					throw new Error('Error al cargar la funci&oacute;n retorna un valor que no puede procesar! ('+$.type(loaderResult)+')');
				}				
			}
			catch(e)
			{
				...
				if (textStatus !== 'abort')
				{
					...
					if ($.isPlainObject(jqXHR)) // assume it's an AJAX error
					{
						if ($.type(errorThrown) === 'object')
						{
							...
							statusMsg += '<br /><br />Archivo: '+errorThrown.fileName;
						...
						else if (jqXHR.status === 0)
						{
							...
							{
								statusMsg = 'Error AJAX: Sin conexi&oacute;!';
							}
						}
						else
						{
							statusMsg = 'Algo inesperado ha pasado: <strong>'+textStatus+'</strong>!';
						}
						statusMsg += '<br /><br /><u>Cargando opciones</u>:<br />';
						...
						{
							statusMsg += '<br /><br/><u>Respuesta</u>:<br />'+jqXHR.responseText;
						}
					}
					else
					{
		...
		switch (templateName) 
		{
			...
			case 'line.navigation':
				...
				if (type === 'html')
				{
					if (config.data)
					{
						throw new Error('No es posible usar HTML! Por favor use kTemplater.jQuery() en su lugar.');
					}
				}
				else
				{
