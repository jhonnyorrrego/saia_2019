 ...
 * Copyright (c) 2016 Jörn Zaefferer; Licensed MIT */! function(a) {
	...
	return function() {
		...
		f = "JABCDEFGHI".substr(e, 1).toString(), h.match(/[ABEH]/) ? j === e : h.match(/[KPQS]/) ? j === f : j === e || j === f
		...
		for ( c = a.length - 1; c >= 0; c--)
			d = a.charAt(c),
			f = parseInt(d, 10), g && (f *= 2) > 9 && (f -= 9), e += f,
			g = !g;
		return e % 10 === 0
		...
		h = g.getUTCFullYear() === f && g.getUTCMonth() === e - 1 && g.getUTCDate() === d) : h = !1, this.optional(b) || h
		...
		return a = a.toString().toUpperCase(), !(a.length > 10 || a.length < 9 || !c.test(a)) && ( a = a.replace(/^[X]/, "0").replace(/^[Y]/, "1").replace(/^[Z]/, "2"),
		b = 9 === a.length ? a.substr(0, 8) : a.substr(0, 9), d.charAt(parseInt(b, 10) % 23) === e)
		...
		for ( b = 0; b < 17; b++) {
			if ( e = j[b],
			d = a.slice(b, b + 1), 8 === b && ( g = d), isNaN(d)) {
