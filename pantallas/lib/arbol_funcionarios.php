...
if($codigo == 0)
{
  $prof=busca_filtro_tabla("","dependencia","(cod_padre is null OR cod_padre='')AND estado=1","",$conn);
  if($prof["numcampos"]){
    $cadena.=("<item style=\"font-family:verdana; font-size:7pt;\" ");
...
//      $llenado=llena_dependencia($hijos[$i]["iddependencia"],$ruta);
     ...
     {$cadena.=("<item style=\"font-family:verdana; font-size:7pt;\" $adicional ");
      $cadena.=("text=\"".htmlentities(formato_cargo($hijos[$i]["nombre"]))."\" id=\"".$hijos[$i]["iddependencia"]."#\"");
      if($no_padre || $tipo_llenado==1)
        $cadena .= " nocheckbox=\"1\" ";  
      $cadena .= " >\n";
...
if($codigo == 0){
  ...
  for($i=0;$i<$prof["numcampos"];$i++){
    $cadena.=("<item style=\"font-family:verdana; font-size:7pt;\" ");
    $cadena.=("text=\"".ucwords(htmlentities($prof[$i]["nombre"]))."\" id=\"".$prof[$i]["idcargo"]."*\" ");
    $cadena .= " >\n";
...
//      $llenado=llena_dependencia($hijos[$i]["iddependencia"],$ruta);
     ...
     if($codigo_hijos<>"")
     {$cadena.=("<item style=\"font-family:verdana; font-size:7pt;\" $adicional ");
      $cadena.=("text=\"".htmlentities(formato_cargo($hijos[$i]["nombre"]))."\" id=\"".$hijos[$i]["idcargo"]."*\"");
      if($no_padre)
        $cadena .= " nocheckbox=\"1\" ";
      $cadena .= " >\n";
      $cadena.=$codigo_hijos."</item>\n";
...
{ $sistema = "";
  ...
  if($usuarios[$j]["nombres_ord"])
    $func.=("text=\"".ucwords(htmlentities(codifica_caracteres($usuarios[$j]["nombres_ord"]." ".$usuarios[$j]["apellidos"])))."-".htmlentities(formato_cargo($usuarios[$j]["cargo"]))."  $sistema\" id=\"".$usuarios[$j][$tipo_id]."$ruta\" ruta=\"$ruta\" >");
   else  
    $func.=("text=\"".htmlentities($usuarios[$j]["login"])."\" id=\"".$usuarios[$j]["funcionario_codigo"]."$ruta\" ruta=\"$ruta\" >");
   $func.=("</item>\n"); 
}
...
if($usuarios["numcampos"]){
  ...
  if($usuarios[0]["nombres_ord"])
    echo("text=\"".ucwords(htmlentities(codifica_caracteres($usuarios[0]["nombres_ord"]." ".$usuarios[0]["apellidos"])))." (".$usuarios[0]["funcionario_codigo"].") $sistema\" id=\"".$usuarios[0]["funcionario_codigo"]."%$ruta\" ruta=\"$ruta\">");
  else  
    echo("text=\"".htmlentities($usuarios[0]["login"]."\" id=\"".$usuarios[0]["funcionario_codigo"])."\" ruta=\"$ruta\">");
  echo("</item>\n");
  return(TRUE);    
}
return(FALSE);
...
{ $func.=("<item style=\"font-family:verdana; font-size:7pt;\" ");
  if($usuarios[$j]["nombres_ord"])
    $func.=("text=\"".ucwords(htmlentities(codifica_caracteres($usuarios[$j]["nombres_ord"]." ".$usuarios[$j]["apellidos"])))."\" id=\"".$usuarios[$j]["funcionario_codigo"]."\" >");
   else  
    $func.=("text=\"".ucwords(htmlentities(codifica_caracteres($usuarios[$j]["login"])))."\" id=\"".$usuarios[$j]["funcionario_codigo"]."\" >");
   $func.=("</item>\n"); 
}
if($usuarios["numcampos"])
  return($func);
...
{ $func.=("<item style=\"font-family:verdana; font-size:7pt;\" ");
  if($usuarios[$j]["nombres_ord"])
    $func.=("text=\"".ucwords(htmlentities($usuarios[$j]["nombres_ord"]." ".$usuarios[$j]["apellidos"]))."\" id=\"".$usuarios[$j]["funcionario_codigo"]."\" ");
   else  
    $func.=("text=\"".ucwords(htmlentities(codifica_caracteres($usuarios[$j]["login"])))."\" id=\"".$usuarios[$j]["funcionario_codigo"]."\" ");
