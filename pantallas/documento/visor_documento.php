...
if (isset($_REQUEST['pdf_word'])) {
	if ($datos[0]["pdf"] != "" && !isset($_REQUEST["actualizar_pdf"])) {
		$export = "views/visor/viewer.php?tipo_visor=1&actualizar_pdf=1&iddocumento=" . $iddoc . "&ruta=" . base64_encode($datos[0]["pdf"]);
	} else {
		if (!isset($_REQUEST["error_pdf_word"])) {
			...
			if ($datos_word[0]["pdf"] != "") {
				$export = "views/visor/viewer.php?tipo_visor=1&actualizar_pdf=1&iddocumento=" . $iddoc . "&ruta=" . base64_encode($datos_word[0]["pdf"]);
			}
		}
	}
} else {
	if ($iddoc && $datos[0]['mostrar_pdf'] != 2) {
		...
		$export = "";
		if ($datos[0]["pdf"] != "" && !isset($_REQUEST["actualizar_pdf"])) {
			$export = "views/visor/viewer.php?tipo_visor=1&iddocumento=" . $iddoc . "&ruta=" . base64_encode($datos[0]["pdf"]);
		} else {
