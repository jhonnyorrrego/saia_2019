...
if($datos[0]["pdf"] && is_file($ruta_db_superior.$datos[0]["pdf"]) && !@$_REQUEST["vista"] ){
	...
	//print_r($anexos_documento_word);die("KAPUT");
	  if($anexos_documento_word['numcampos']){
		...
		$arr_ruta_pdf = array("servidor" => $arr_alm["servidor"], "ruta" => $pdf);
	...
	if ($almacenamiento && !$almacenamiento->get_filesystem()->has($pdf)) {
		...
		$ruta_mostrar=$ruta_db_superior.FORMATOS_CLIENTE.$documento[0]['nombre'].'/'.$documento[0]['ruta_mostrar'].'?idformato='.$documento[0]['idformato'].'&iddoc='.$iddoc.'&error_pdf_word=1';
...
<iframe id="detalles" width="100%" frameborder="0" name="detalles"
	src="<?php echo($ruta_db_superior . "filesystem/mostrar_binario.php?ruta=" . $ruta_pdf); ?>"></iframe>
<?php
} else {?>
	<iframe id="detalles" width="100%" frameborder="0" name="detalles"
			src="<?php echo($pdf); ?>"></iframe>
<?php
}
?>
