...
function mostrar_numero_enlace($numero, $iddoc, $plantilla) {
    $titulo = "No. " . $numero;
    if (empty($numero) || $numero == 'numero') {
        $numero = "Ver";
        $titulo = nombre_plantilla(null, $iddoc);
    }
    $documentRoute = 'views/documento/index_acordeon.php?';
    $documentRoute .= http_build_query([
        'documentId' => $iddoc,
        'format' => strtolower($plantilla)
    ]);
    $campo = '<div class="link kenlace_saia"'
            . 'enlace="' . $documentRoute . '" '
            . 'conector="iframe" titulo="' . $titulo . '">'
            . '<span class="badge cursor btn">' . $numero . '</span></div>';
    return $campo;
}
