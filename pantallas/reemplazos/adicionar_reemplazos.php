...
<form name="formulario_reemplazo" id="formulario_reemplazo" method="post" action="#">
    <table class="table table-bordered" style="width:80%;" align="center">
        ...
        <tr>
            ...
            <td>
            	<input type="hidden" id="hoy" name="hoy" value="<?php echo(date('Y-m-d'));?>">
        ...
        <tr id="fila_fecha_fin">
            ...
            <td>
              <div id="fecha_fin_label" class="input-append date">
          		...
          		Tener en cuenta que la fecha que se indique como fecha fin de reemplazo es la fecha hasta cuando el usuario va a estar realizando el reemplazo,<br/> es decir, que un d&iacute;a despues se inactivar&aacute; autom&aacute;ticamente el reemplazo por el sistema
            </td>
        </tr>
        <tr>
	...
	$(document).ready(function() {
		...
		function fin_cargando_antiguo() {
			if (browserType == "gecko")
				document.poppedLayer = eval('document.getElementById("esperando_antiguo")');
			else if (browserType == "ie")
				document.poppedLayer = eval('document.getElementById("esperando_antiguo")');
			else
				document.poppedLayer = eval('document.layers["esperando_antiguo"]');
		...
		function fin_cargando_nuevo() {
			if (browserType == "gecko")
				document.poppedLayer = eval('document.getElementById("esperando_nuevo")');
			else if (browserType == "ie")
				document.poppedLayer = eval('document.getElementById("esperando_nuevo")');
			else
		...
		$("#submit_formulario_reemplazo").click(function() {
			...
			var hoy = new Date($("#hoy").val());
			var inDate = new Date($("#fecha_inicio").val());
			if (inDate<hoy) {
					alert("La fecha inicial no pueden ser inferior a hoy");
					return (false);
			}
			if ($("input[name='tipo_reemplazo']").val() == 1) {
				var eDate = new Date($("#fecha_fin").val());
				if ($("#fecha_inicio").val() == $("#fecha_fin").val()) {
					alert("Las fechas no pueden ser iguales");
					return (false);
				} else if (inDate > eDate) {
					alert("La fecha final no pueden ser anterior a la fecha inicial");
					return (false);
				}
			} else {
...
function adicionar_reemplazo() {
	...
	$valida = busca_filtro_tabla("idreemplazo_saia", "reemplazo_saia", "estado=1 and antiguo='" . @$_REQUEST["nuevo"] . "'", "", $conn);
	if ($valida["numcampos"]) {
		$ok=0;
		alerta("El funcionario de reemplazo esta vinculado a un reemplazado que se encuentra activo");
		return (0);
	}

	$valida2 = busca_filtro_tabla("idreemplazo_saia", "reemplazo_saia", "estado=1 and antiguo='" . @$_REQUEST["antiguo"] . "'", "", $conn);
	if ($valida2["numcampos"]) {
		$ok=0;
		alerta("El funcionario actual esta vinculado a un reemplazado que se encuentra activo");
		return (0);
	}
	
	if($ok){
		$sql2 = "INSERT INTO reemplazo_saia(antiguo,nuevo,fecha_inicio,fecha_fin,observaciones,tipo_reemplazo) VALUES('" . @$_REQUEST["antiguo"] . "','" . @$_REQUEST["nuevo"] . "'," . fecha_db_almacenar(@$_REQUEST["fecha_inicio"], "Y-m-d") . "," . $fecha_fin . ",'" . htmlentities(strip_tags(@$_REQUEST["observaciones"])) . "','" . $_REQUEST['tipo_reemplazo'] . "')";
		phpmkr_query($sql2);
