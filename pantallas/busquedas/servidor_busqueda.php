...
if ($result["numcampos"]) {
        ...
        if (@$_REQUEST["ruta_exportar_saia"]) {
            ...
            if (file_exists($ruta_db_superior . $_REQUEST["ruta_exportar_saia"]) === false || $page == 1) {
                ...
                if ($_REQUEST["exportar_saia"] == "excel") {
                    ...
                    $speadsheet->getProperties()->setCreator($nombre)->setLastModifiedBy($nombre)->setTitle($titulo)->setSubject($titulo)->setKeywords("cerok SAIA reporte");
                    $writer = new Xls($speadsheet);

                    # Le pasamos la ruta de guardado

                    $writer->save($ruta_db_superior . $_REQUEST["ruta_exportar_saia"]);
                    $highestRow = 0;
                }
                ...
                $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load($ruta_db_superior . $_REQUEST["ruta_exportar_saia"]);
    ...
    if ($_REQUEST["exportar_saia"] == "csv" && $page == 1) {
        ...
        $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load($ruta_db_superior . $_REQUEST["ruta_exportar_saia"]);
        $spreadsheet->getActiveSheet()->fromArray($array_export, NULL, 'A' . ($highestRow));
