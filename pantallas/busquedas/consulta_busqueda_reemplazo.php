...
<script>
  ...
  $("#panel_body").scroll(function(){
    if(($("#panel_body").scrollTop() >= $("#resultado_busqueda_principal<?php echo($datos_componente);?>").height() - $("#panel_body").height()) && !carga_final){
      cargar_datos_scroll();
    }
  });
  ...
  function cargar_datos_scroll2(){
    if(carga_final!=2){
      $.ajax({
        type:'POST',
        url: "servidor_busqueda.php",
        data: "idbusqueda_componente=<?php echo($busqueda_documento_reemplazo[0]['idbusqueda_componente']);?>&page="+$("#busqueda_pagina_doc").val()+"&rows=todos&actual_row="+$("#fila_actual_doc").val()+"&idreemplazo=<?php echo($idreemplazo);?>",
        success: function(html){
          if(html){
            var objeto=jQuery.parseJSON(html);
            if(objeto.exito){
