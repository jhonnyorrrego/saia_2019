...
function calcular_fecha_extrema_inicial($idcaja) {
	global $conn;
	$expedientes = busca_filtro_tabla("MIN(fecha_extrema_i) as fecha", "expediente", "fk_idcaja = " . $idcaja, "", $conn);
	return $expedientes[0]["fecha"];
}

function calcular_fecha_extrema_final($idcaja) {
	global $conn;
	$expedientes = busca_filtro_tabla("MAX(fecha_extrema_f) as fecha", "expediente", "fk_idcaja = " . $idcaja, "", $conn);
	return $expedientes[0]["fecha"];
}
