			...
			if ($evaluador["numcampos"]) {
				if ($evaluador[0]['evaluador'] != '') {
					...
					if ($funcionario["numcampos"]) {
						...
						enviar_mensaje("", array("para" => "funcionario_codigo"), array("para" => array($fun_cod)), "Tarea para Evaluar", $mensaje);
					}
				}
			}
			...
			if ($ok && $progreso > 24) {
				...
				if ($seguidores["numcampos"]) {
					...
					if ($funcionarios["numcampos"]) {
						...
						enviar_mensaje("", array("para" => "funcionario_codigo"), array("para" => $fun_cod), "Nuevo Progreso Registrado", $mensaje);
					}
				}
			}
		}
			...
			if ($_REQUEST["enviar_correo"] == 1) {
				...
				if ($responsable["numcampos"]) {
					...
					NOTA: " . $_REQUEST["descripcion"] . "<br/><br/>" . $link;
						enviar_mensaje("", array("para" => "funcionario_codigo"), array("para" => $funcionarios_enviar_vector), "Nueva Nota", $mensaje);
					}
				}
			}
			$retorno -> exito = 1;
...
function actualizar_calificacion() {
	...
	if (isset($_REQUEST["calificacion"]) && @$_REQUEST["idtareas_listado"]) {
        ...
        ' . $link . '
      ';
		enviar_mensaje("", array("para" => "funcionario_codigo"), array("para" => $fun_cods), "Nueva Tarea Calificada", $mensaje);
...
function calcula_ejecuta_proxima($idtareas_listado) {
	...
	$tareas_listado = busca_filtro_tabla(fecha_db_obtener("ejecuta_proxima", "Y-m-d") . " AS ejecuta_proxima,recurrencia,repetir_cada,dias_semana,repetir_mes,finaliza_el," . fecha_db_obtener("finaliza_el_fecha", "Y-m-d") . " AS finaliza_el_fecha,idtareas_listado", "tareas_listado A, tareas_listado_recur B", "A.idtareas_listado=B.fk_tareas_listado AND A.idtareas_listado=" . $idtareas_listado, "", $conn);
		...
		switch($tareas_listado[0]["recurrencia"]) {
			...
			case "4" :
				if ($tareas_listado[0]['repetir_mes'] == 2) {
					...
					$fecha_inicial = explode("-", $tareas_listado[0]["ejecuta_proxima"]);
					$nuevo_mes = intval($fecha_inicial[1]) + intval($tareas_listado[0]["repetir_cada"]);
					if ($nuevo_mes < 10) {
						$nuevo_mes = "0" . $nuevo_mes;
					}
					$nueva_fecha = intval($fecha_inicial[0]) . "-" . intval($nuevo_mes) . "-" . intval($fecha_inicial[2]);
					echo($nueva_fecha);
					echo("<hr>");
					$nuevo_dia = DateTime::createFromFormat('Y-m-d', $nueva_fecha);
				}
				$ejecuta_proxima = info_finaliza($nuevo_dia, $tareas_listado);
				break;
		}
...
function generar_tarea_recurrencia($idtareas_listado, $respuesta = NULL) {
	global $conn, $ruta_db_superior;
	...
	$datos_tarea = busca_filtro_tabla("idtareas_listado, listado_tareas_fk, estado_tarea," . fecha_db_obtener('a.fecha_creacion', 'Y-m-d') . " as fecha_creacion, nombre_tarea, tipo_tarea, responsable_tarea, co_participantes, seguidores, descripcion_tarea, " . fecha_db_obtener('a.fecha_inicio', 'Y-m-d') . " as fecha_inicio, " . fecha_db_obtener('a.fecha_limite', 'Y-m-d') . " as fecha_limite, prioridad," . fecha_db_obtener('a.tiempo_estimado', 'H:i') . " as tiempo_estimado, enviar_email, progreso, calificacion, creador_tarea, cod_padre, fk_tarea_recurrencia, evaluador, " . fecha_db_obtener('a.fecha_planeada', 'Y-m-d') . " as fecha_planeada, generica, from_generica, info_recurrencia", "tareas_listado a", "a.idtareas_listado=" . $idtareas_listado, "", $conn);
	$datos_recurrencia = busca_filtro_tabla("*," . fecha_db_obtener('ejecuta_proxima', 'Y-m-d') . " as ejecuta_proxima", "tareas_listado_recur", "ejecuta_proxima IS NOT NULL AND fk_tareas_listado=" . $idtareas_listado, "", $conn);
	...
	if ($datos_recurrencia['numcampos']) {
		...
		if ($proxima_ejecucion) {
			...
			phpmkr_query($sql);
			$id_tareas_listado_nueva = phpmkr_insert_id();
		} else {
		...
		$retorno = 1;
		if ($respuesta) {
			$retorno = $id_tareas_listado_nueva;
		}
	}
