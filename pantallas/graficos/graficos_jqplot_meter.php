...
<script type="text/javascript" src="<?php echo($ruta_db_superior);?>js/jqplot_plugins/jqplot.meterGaugeRenderer.min.js"></script>
...
$(document).ready(function(){
    $.ajax({      
      async: false,
      url: "<?php echo($ruta_db_superior)?>pantallas/graficos/datos_graficos_meter.php?idbusqueda_grafico=<?php echo(1);?>",
      dataType:"json",
      success: function(data) {
		var cant=Math.round((data.max-data.min)/3);
		var info=data.min;
		var ticks = new Array();
		var intervalos = new Array();
		//intervalos.push(0);
		 for(i=0;i<3;i++){
		 	intervalos.push(info);
		 	info=info+cant;
		 }
	     intervalos.push(data.max);
		  $.jqplot('chart2',[[6]],{
