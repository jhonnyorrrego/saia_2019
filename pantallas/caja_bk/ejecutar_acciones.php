...
function set_caja() {
    ...
    if ($idcaja) {
        if (asignar_caja($idcaja, 1, $_SESSION["idfuncionario"])) {
            $exito = 1;
        }
    }
    if ($exito) {
        if (isset($_REQUEST["obtener_idbusqueda_filtro_temp"]) && $_REQUEST["idbusqueda_componente"]) {
            $insert = "INSERT INTO busqueda_filtro_temp (fk_busqueda_componente, funcionario_idfuncionario, fecha, detalle) VALUES(" . $_REQUEST["idbusqueda_componente"] . ", " . $_SESSION["idfuncionario"] . ", " . fecha_db_almacenar(date("Y-m-d"), "Y-m-d") . ", 'idcaja|=|" . $idcaja . "');";
