...
if($idpantalla[0]["valor"]){
	...
	$campo_tarea=busca_filtro_tabla("","pantalla_campos a","a.pantalla_idpantalla=".$pantalla_campos[0]["pantalla_idpantalla"]." and nombre like 'acciones_tarea_bpmni%'","",$conn);
	...
	$campo_idtabla=busca_filtro_tabla("","pantalla_campos a","a.pantalla_idpantalla=".$pantalla_campos[0]["pantalla_idpantalla"]." and nombre='id".$pantalla_campos[0]["pantalla"]."'","",$conn);
	
	$datos=1;
	$llenar_parametros="";
	$llenar_parametros.="$('#div3_bpmni').click();";
	$llenar_parametros.="$('#div1_idbpmn_tarea').click();";
	$llenar_parametros.="$('#div1_registro').click();";
	$llenar_parametros.="$('#div1_iddocumento').click();";
	
	$llenar_parametros.="$('#bpmni_dato').val('bpmni');";
	$llenar_parametros.="$('#idbpmn_tarea_dato option[value=\'".$campo_tarea[0]["idpantalla_campos"]."\']').attr('selected',true);";
	$llenar_parametros.="$('#registro_dato option[value=\'".$campo_idtabla[0]["idpantalla_campos"]."\']').attr('selected',true);";
	if($pantalla_campos[0]["tipo_pantalla"]==2){
		$llenar_parametros.="$('#iddocumento_dato option[value=\'".$campo_doc[0]["idpantalla_campos"]."\']').attr('selected',true);";
	}
}
?>
...
<input type="hidden" name="ruta" id="ruta" value="bpmn/bpmni_tarea/funciones.php">
</form>
<?php
...
$.ajax({
  ...
  data: "funcion=instanciar_tarea_bpmni($bpmni,$idbpmn_tarea,$registro,$iddocumento,$estado)&idpantalla=<?php echo($idpantalla[0]["pantalla_idpantalla"]);?>&ruta=<?php echo($ruta_db_superior);?>bpmn/bpmni_tarea/funciones.php&rand="+Math.round(Math.random()*100000),
  ...
  $(".seleccion_variable_funcion_pantalla").live("change",function(){    
    ...
    $("#enviar_formulario_saia").live("click",function(){
      var nombre_form = "editar_acciones_pantalla";
      if(formulario.valid()){
