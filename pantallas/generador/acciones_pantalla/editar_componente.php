...
if($idpantalla[0]["valor"]){
	...
	else if($datos[0]["momento"]==4)$momento_checked4="checked";
	...
	$("#funcion").change();$("#funcion").trigger("change");';
...
<form method="POST" action="" class="form-horizontal" name="editar_acciones_pantalla" id="editar_acciones_pantalla">
  ...
  <div class="control-group">
    ...
    <div class="controls">
	  ...
	  <label class="radio inline" for="momento">        
        <input type="radio" name="momento" id="momento_4" value="4" <?php echo $momento_checked4; ?>>
        Ingresar
      </label>
    </div>
  </div> 
...
$(document).ready(function(){
  ...
  tree_acciones.loadXML("<?php echo($ruta_db_superior);?>pantallas/lib/test_archivos_carpeta.php?carpeta_inicial=<?php echo($carpeta_inicial[0]["valor"]); ?>&extensiones_permitidas=php&idpantalla=<?php echo($idpantalla[0]["pantalla_idpantalla"]); ?>");
...
var nombre_funcion='';
  $(".seleccionar_pantalla_funcion").live("change",function(){
    ...
    $.ajax({
      ...
      data: "funcion="+funcion+"&idpantalla=<?php echo($idpantalla[0]["pantalla_idpantalla"]);?>&ruta=<?php echo($_REQUEST['ruta']);?>&rand="+Math.round(Math.random()*100000),
      success: function(html){                
        if(html){
      ...
      if(formulario.valid()){
        ...
        if (formulario.data('locked') == undefined && !formulario.data('locked')){
          ...
          $.ajax({
            ...
            data: "ejecutar_pantalla_campo=guardar_configurar_pantalla_libreria&lugar_incluir="+lugar_incluir+"&tipo_retorno=1&ruta="+ruta+"&pantalla_idpantalla=<?php echo $idpantalla[0]["pantalla_idpantalla"].$adicional_exe; ?>&rand="+Math.round(Math.random()*100000)+"&"+formulario.serialize(),
