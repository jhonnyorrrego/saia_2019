...
$datos_pantalla=busca_filtro_tabla("","pantalla a","a.idpantalla='".$idpantalla[0]["pantalla_idpantalla"]."'","",$conn);
...
if($idpantalla[0]["valor"]){
	...
	$datos=1;
	$llenar_parametros="";
	$llenar_parametros.="$('#div2_ruta').click();";
	$llenar_parametros.="$('#ruta_dato').val('ordenar.php');";
	
	$llenar_parametros.="$('#div2_target').click();";
	$llenar_parametros.="$('#target_dato').val('_self');";
	
	$llenar_parametros.="$('#div2_nombre_pantalla').click();";
	$llenar_parametros.="$('#nombre_pantalla_dato').val('".$datos_pantalla[0]["nombre"]."');";
}
?>
      ...
      <select name="accion" id="accion">
        <?php
          for($i=0;$i<$acciones["numcampos"];$i++){
          	$adicional="";
          	if($acciones[$i]["nombre"]=="enviar_exito")$adicional="selected";
            echo('<option value="'.$acciones[$i]["nombre"].'" '.$adicional.'>'.$acciones[$i]["etiqueta"].'</option>');
          }
        ?>
      </select>
    </div>
...
<input type="hidden" name="ruta" id="ruta" value="pantallas/lib/librerias_comunicacion_saia.php">
</form>
<?php
...
$.ajax({
  ...
  data: "funcion=redirecciona_mostrar_formato($ruta,$target,$nombre_pantalla)&idpantalla=<?php echo($idpantalla[0]["pantalla_idpantalla"]);?>&ruta=<?php echo($ruta_db_superior);?>pantallas/lib/librerias_comunicacion_saia.php&rand="+Math.round(Math.random()*100000),
