...
<form method="POST" action="" class="form-horizontal" name="editar_pantalla_campo" id="editar_pantalla_campo">
  ...
  <div class="control-group">
    <label class="control-label" for="listado_saia">Listado SAIA</label>
    <div class="controls">
      <select name="fs_valor" id="fs_valor"><option value="">Por favor seleccione...</option>
<?php
$listados=busca_filtro_tabla("","busqueda a","","a.etiqueta asc",$conn);
for($i=0;$i<$listados["numcampos"];$i++){
	$selected="";
	if($listados[$i]["idbusqueda"]==$pantalla_campos[0]["valor"])$selected="selected";
	echo "<option value='".$listados[$i]["idbusqueda"]."' ".$selected.">".$listados[$i]["etiqueta"]."</option>";
}
?>
      </select>
    </div>
