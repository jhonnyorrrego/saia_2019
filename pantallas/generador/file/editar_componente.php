...
if(@$_REQUEST["idpantalla_campos"]){    
  $pantalla_campos=get_pantalla_campos($_REQUEST["idpantalla_campos"],0);
	...
	$pantalla_campos[0]["valor"]=$datos[0];
  $obligatoriedad_si='';
  $obligatoriedad_no='';
...
<form method="POST" action="" class="form-horizontal" name="editar_pantalla_campo" id="editar_pantalla_campo">
  ...
  <div class="control-group">
    ...
    <div class="controls">
      <label class="control-label" for="obligatorio">
        <input type="radio" name="obligatorio" id="obligatorio1" value="1" <?php echo($obligatoriedad_si);?> required>
        Obligatorio
      </label>
      <label class="control-label" for="radios-1">
        <input type="radio" name="obligatorio" id="obligatorio0" value="0" <?php echo($obligatoriedad_no);?>>
        Opcional
      </label>
    </div>
  </div>
  ...
  <div class="control-group">
    <label class="control-label" for="obligatoriedad">Mostrar</label>
    <div class="controls">
      <label class="control-label" for="obligatorio">
        <input type="radio" name="tipo_mostrar" id="tipo_mostrar1" value="1" <?php echo($tipo_mostrar1);?> required>
        Enlaces
      </label>
      <label class="control-label" for="radios-1">
        <input type="radio" name="tipo_mostrar" id="tipo_mostrar2" value="2" <?php echo($tipo_mostrar2);?>>
        Imagenes
      </label>
    </div>
  </div>
  ...
  <div class="form-actions">
  	...
  	<input type="hidden" name="fs_valor" id="valor" value="">
  	<input type="hidden" name="fs_obligatoriedad" id="obligatoriedad" value="0">
        ...
        success: function(html){
          if(html){          
            ...
            if(objeto.exito){
              ...
              $.ajax({
				       	type:'GET',
				        url: "<?php echo($ruta_db_superior);?>pantallas/generador/file/librerias.php",
				        data: "ejecutar_pantalla_campo=vincular_funcion&idpantalla=<?php echo($pantalla_campos[0]['pantalla_idpantalla']); ?>&rand="+Math.round(Math.random()*100000),
				        success: function(html){
...
function parsear_items(){
	var opcion_obligatorio=$("input[name=obligatorio]:checked").val();
	var opcion_tipo=$("input[name=tipo_mostrar]:checked").val();
	$("#valor").val(opcion_obligatorio+","+opcion_tipo);
}
</script>