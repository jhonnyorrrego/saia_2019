...
function footer_pantalla($pantalla, $accion) {
	...
	if ($pantalla[0]["submit_formulario"] && in_array($accion, array("adicionar","editar","buscar","eliminar" ))) {
...
function generar_archivos_ignorados($idpantalla, $tipo_retorno){
	...
	$vector_ignorar=array("adicionar","buscar","class","editar","eliminar","encabezado","librerias","mostrar","pie");
	$cadena_ignorados="";
					...
					if ($resultado) {
						...
						if ($tipo_almacenamiento == "archivos") {// Los anexos estan guardados en archivos
							...
							if ($idanexo) {
								...
								$retorno["permiso"]=$sql_permiso["sql"];
								$ok=1;
							}
						} else {
