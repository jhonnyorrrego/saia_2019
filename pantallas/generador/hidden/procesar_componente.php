...
function procesar_hidden($idcampo='',$seleccionado='',$accion='',$campo=''){	
  ...
  if($seleccionado==''){
    ...
    $encabezado_hidden='<div class="control-group element" idpantalla_componente="'.$campo["idpantalla_componente"].'" idpantalla_campo="'.$idcampo.'" id="pc_'.$idcampo.'" nombre="'.$campo["etiqueta_html"].'">'.clase_eliminar_pantalla_componente($idcampo).'<label class="control-label" for="'.$campo["nombre"].'">'.$campo["etiqueta"];
    if($campo["obligatoriedad"]){
      $encabezado_hidden.='*';
    }
    $encabezado_hidden.='</label><div class="controls">';
    $pie_hidden='</div></div>';
  }
