...
if(@$_REQUEST['recortar']){
	...
	if($tipo_almacenamiento2->get_filesystem()->has($arr_alm["ruta"])){
		...
		$extension='jpg';
		$nombre_nuevo = (rand());
		$ruta_recortada.=$nombre_nuevo.'.'.$extension;		
		
		imagejpeg($dst_r, $ruta_temporal.$nombre_nuevo.'.'.$extension, $jpeg_quality);
		imagedestroy($dst_r);
		$binario = file_get_contents($ruta_temporal.$nombre_nuevo.'.'.$extension);	
		$tipo_almacenamiento = new SaiaStorage("imagenes");
		$ruta_final = RUTA_FOTOGRAFIA_FUNCIONARIO.'recorte/';
		$resultado = $tipo_almacenamiento->almacenar_contenido($ruta_final.$nombre_nuevo.'.'.$extension, $binario);
		$ruta_anexos = array("servidor" => $tipo_almacenamiento->get_ruta_servidor(), "ruta" => $ruta_final.$nombre_nuevo.'.'.$extension);	
