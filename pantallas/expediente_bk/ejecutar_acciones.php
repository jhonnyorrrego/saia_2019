...
function set_expediente() {
    ...
    if ($exito) {
        if (isset($_REQUEST["obtener_idbusqueda_filtro_temp"]) && $_REQUEST["idbusqueda_componente"]) {
            $insert = "INSERT INTO busqueda_filtro_temp (fk_busqueda_componente, funcionario_idfuncionario, fecha, detalle) VALUES(" . $_REQUEST["idbusqueda_componente"] . ", " . $_SESSION["idfuncionario"] . ", " . fecha_db_almacenar(date("Y-m-d"), "Y-m-d") . ", 'idexpediente|=|" . $idexpediente . "');";
            phpmkr_query($insert) or die("Error al obtener el idbusqueda_filtro_temp");
            $idtemp = phpmkr_insert_id();
            $retorno -> idbusqueda_filtro_temp = $idtemp;
        }
...
function update_expediente() {
    ...
    if ($exito) {
        $retorno -> cod_padre = $_REQUEST["cod_padre"];
        $retorno -> idexpediente = $idexpediente;
        $retorno -> exito = 1;
