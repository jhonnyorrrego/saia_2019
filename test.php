...
function llena_dependencia($codigo, $ruta, $tipo_llenado = "")
{
	...
	if ($codigo == 0) {
		...
		if ($prof["numcampos"]) {
			$cadena .= ("<item style=\"font-family:verdana; font-size:7pt;\" ");
			$cadena .= ("text=\"" . ucwords(codifica_encabezado(htmlspecialchars($prof[0]["nombre"]))) . "\" id=\"" . $prof[0]["iddependencia"] . "#\"");
		...
		if ($prof["numcampos"]) {
			...
			$item_agrupar_fin = '';
			if (@$_REQUEST['agrupar']) {
				$item_agrupar .= "<item style=\"font-family:verdana; font-size:7pt;\" nocheckbox=\"1\" id=\"agrupador_" . $codigo . "\" text=\"" . codifica_encabezado(htmlspecialchars($prof[0]['nombre'])) . "\" child=\"1\"> ";
...
function llena_cargos($codigo, $ruta, $tipo_llenado = "")
{
	...
	if ($codigo == 0) {
		...
		for ($i = 0; $i < $prof["numcampos"]; $i++) {
			$cadena .= ("<item style=\"font-family:verdana; font-size:7pt;\" ");
			$cadena .= ("text=\"" . ucwords(codifica_encabezado(htmlspecialchars(strtolower($prof[$i]["nombre"])))) . "\" id=\"" . $prof[$i]["idcargo"] . "*\" ");
			$cadena .= " >\n";
...
function llena_funcionarios($codigo, $ruta, $tipo_llenado)
{
	...
	for ($j = 0; $j < $usuarios["numcampos"]; $j++) {
		...
		if ($usuarios[$j]["nombres_ord"])
			$func .= ("text=\"" . ucwords(codifica_encabezado(htmlspecialchars(strtolower($usuarios[$j]["nombres_ord"] . " " . $usuarios[$j]["apellidos"])))) . "-" . codifica_encabezado(htmlspecialchars(formato_cargo(strtolower($usuarios[$j]["cargo"])))) . "  $sistema\" id=\"" . $usuarios[$j][$tipo_id] . "$ruta\" ruta=\"$ruta\" >");
		else
			$func .= ("text=\"" . codifica_encabezado(htmlspecialchars($usuarios[$j]["login"])) . "\" id=\"" . $usuarios[$j]["funcionario_codigo"] . "$ruta\" ruta=\"$ruta\" >");
		$func .= ("</item>\n");
	}
...
function funcionarios_inactivos()
{
	...
	for ($j = 0; $j < $usuarios["numcampos"]; $j++) {
		...
		if ($usuarios[$j]["nombres_ord"])
			...
			$func .= ("text=\"" . ucwords(codifica_encabezado(htmlspecialchars($usuarios[$j]["login"]))) . "\" id=\"" . $usuarios[$j]["funcionario_codigo"] . "\" >");
		$func .= ("</item>\n");
	}
	if ($usuarios["numcampos"])
		return ($func);
