...
elseif($identidad_serie!=0){
	...
	if($papas["numcampos"]) {
		for($i = 0; $i < $papas["numcampos"]; $i++) {
			...
			//print_r($idfuncionario." - ".$papas[$i]["idfuncionario"]." -".$papas[$i]["permiso"]." - ".$papas[$i]["tipo"]);
			...
			if(!$partes){
				...
				if($hijos[0]["cant"]) {
					$item["folder"] = 1;
					//print_r($papas[$i]["idserie"]." ".$papas[$i]["identidad_serie"]." ".$iddep." ". $tipo." ".$nombre_dependencia." ".$dependencia_codigo);
