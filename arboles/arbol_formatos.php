...
if (!empty($id_x)) {
    $objetoJson = llena_formato($id_x, 0, $seleccionados, $filtrar,$cargar_seleccionado);
} else {
    $objetoJson = llena_formato(null, 0, $seleccionados, $filtrar,$cargar_seleccionado);
}
...
function llena_formato($id, $nivel=0, $seleccionados=array(), $filtrar=null, $cargar_seleccionado=null) {
    global $conn;
    ...
    if ($papas["numcampos"]) {
        for ($i = 0; $i < $papas["numcampos"]; $i++) {
			...
			$item["data"] = array('descripcion' => $papas[$i]["descripcion_formato"],'version' => $papas[$i]["version"]);
