...
<?php
listado_controles_asignaciones();
...
/*	$sSql = "Delete FROM asignacion";
	...
	$sql="DELETE FROM control_asignacion WHERE asignacion_idasignacion=".$@$_REQUEST["key"];
	die($sql);
	phpmkr_query($sSql,$conn) or die("Falló la búsqueda" . phpmkr_error() . ' SQL:' . $sSql);*/
	return(elimina_asignacion($sqlKey));
}
...
if($funciones["numcampos"]){
  echo('Acciones o Controles vinculados Con la Asignacion:<br /><br /><table border="0" cellspacing="1" cellpadding="4" bgcolor="#CCCCCC">
  	<tr class="encabezado_list">
