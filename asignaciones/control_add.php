...
switch ($sAction)
{
	...
	case "A": // Add
		if (AddData($conn)) { // Add New Record
			$HTTP_SESSION_VARS["ewmsg"] = "Adición exitosa del registro.";
		//phpmkr_db_close($conn);
			ob_end_clean();
			header("Location: control_tarealist.php?key=".@$HTTP_POST_VARS["x_tarea_idtarea"]);
			exit();
		}
		break;
}
...
<form name="control_add" id="control_add" action="control_tareaadd.php" method="post" onSubmit="return EW_checkMyForm(this);">
<p>
...
<textarea cols="35" rows="4" id="x_accion" name="x_accion"><?php echo @$x_accion; ?></textarea>
</span></td>
	</tr>
	<tr>
...
<input type="hidden" name="x_tarea_idtarea" id="x_tarea_idtarea" size="30" value="<?php echo $_REQUEST['idtarea'];?>">
  ...
  <td> 
  <input type="text" name="x_fecha_actualizacion" id="x_fecha_actualizacion" value="<?php echo date("Y/m/d H:i:s"); ?>"> <?php selector_fecha("x_fecha_actualizacion","control_tareaadd","Y/m/d H:i:s",date("m"),date("Y"),"ceramique.css","../","AD:VALOR","VENTANA"); ?></td><tr>
