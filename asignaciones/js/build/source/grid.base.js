...
$.fn.jqGrid = function( p ) {
		...
		dragMove: function(x) {
			if(this.resizing) {
				...
				if(newWidth > 30) {
					...
					$('table',this.hDiv).css("width",this.newWidth +"px");
	...
	$.fn.getRowData = function( rowid ) {
		...
		if (rowid){
			this.each(function(){
				var $t = this,nm;
				$('table tbody #'+rowid+' td',$t.grid.bDiv).each( function(i) {
	...
	$.fn.delRowData = function(rowid) {
		...
		if(rowid) {
			this.each(function() {
				var t = this;
				$('#'+this.id+' > tbody > #'+rowid,this.grid.bDiv).each(function(){
	...
	$.fn.setRowData = function(rowid, data) {
		...
		this.each(function(){
			var t = this;
			if( $('#'+this.id+' > tbody > #'+rowid,t.grid.bDiv).attr('id')==rowid &&  data ) {
				success=true;
				$(this.p.colModel).each(function(i){
					nm = this.name;
					if(data[nm] != 'undefined') {
						$("#"+rowid,t.grid.bDiv).find("td:eq("+i+")").html(data[nm]);
						success = true;
					}
				});
			}
		...
		return this.each(function() {
			...
			$(this.p.colModel).each(function(i) {
				if (colname.indexOf(this.name) != -1 && !this.hidden) {
					var w = parseInt($("tr th:eq("+i+")",$t.grid.hDiv).css("width"));
			...
			if(fndh==true) {
				...
				$("table",$t.grid.hDiv).width(gtw);
				$("table",$t.grid.bDiv).width(gtw);
			...
			$($t.p.colModel).each(function(i) {
				if (colname.indexOf(this.name) != -1 && this.hidden) {
					...
					$("tr",$t.grid.bDiv).each(function(){
						$("td:eq("+i+")",this).css("display","").width(w);
					});
					this.hidden=false;
	...
	$.fn.setLabel = function(colname, nData, prop ){
		return this.each(function(){
			...
			} else {pos = parseInt(colname)};
			if(pos>=0) {
				var thecol = $("thead th:eq("+pos+")",$t.grid.hDiv);
				if (nData){
					$("div",thecol).html(nData);
				};
				if (prop)
					typeof prop == 'string' ? $(thecol).addClass(prop) : $(thecol).css(prop);
			};
		});
	};
	$.fn.setCell = function(rowid,colname,nData,prop) {
		return this.each(function(){
			...
			if(pos>=0) {
				var tcell = $("table #"+rowid+":first td:eq("+pos+")",$t.grid.bDiv);
				if(nData) $(tcell).html(nData);
				if (prop)
					typeof prop == 'string' ? $(tcell).addClass(prop) : $(tcell).css(prop)
	...
	return this.each( function() {
		...
		var addXmlData = function addXmlData (xml,t) {
			...
			var row,gi=0,si=0,cbid,idn, f=[],cn=(ts.p.altRows === true) ? 'alt':'',rd=[];
		...
		var addJSONData = function(data,t) {
			...
			var row,cur,gi=0,si=0,drows,idn,cn=(ts.p.altRows==true) ? 'alt':'',rd=[];
		...
		var setGridWidth = function () {
			...
			for(l=0;l<ts.p.colModel.length;l++) {
				if(!ts.p.shrinkToFit)
					ts.p.colModel[l].owidth = ts.p.colModel[l].width || 150;
		...
		if(this.p.multiselect) {
			...
			$('#cb_jqg',trow).click(function(){
				var chk;
				if (this.checked) {
					$("[@id^=jqg_]",ts.grid.bDiv).attr("checked",true);
					...
					$("[@id^=jqg_]",ts.grid.bDiv).attr("checked",false);
					$("tr",ts.grid.bDiv).removeClass("selected");
					ts.p.selarrrow = [];
					chk=false
				}
		...
		if(ts.p.pager){
			...
			if( $(ts.p.pager).attr("class") === "scroll") { $(ts.p.pager).css({ width: (grid.width)+"px", overflow: "hidden"}).show(); ts.p._height= parseInt($(ts.p.pager).height()); if(hg) $(ts.p.pager).hide();}
			setPager();
		};
		...
		$(grid.bDiv)
			...
			.css({ height: ts.p.height+(isNaN(ts.p.height)?"":"px"), padding: "0px", margin: "0px", overflow: "auto",width: (grid.width)+"px"} ).css("overflow-x","hidden")
			.append(this);
		$("table",grid.bDiv).css({width:grid.width+"px",marginRight:"20px"});
		if( isMSIE ) {
		...
		if( ts.p.toolbar[0] ) {
			...
			ts.p._height += parseInt($(grid.uDiv,ts).height());
			if(hg) $(grid.uDiv,ts).hide();
		};
		if(ts.p.caption) {
			$(grid.cDiv,ts).show().width(grid.width).css("text-align","center");
			ts.p._height += parseInt($(grid.cDiv,ts).height());
			if(ts.p.hidegrid==true) {
				...
				if(hg) { $(".HeaderButton",grid.cDiv).trigger("click"); var tdt = ts.p.datatype; ts.p.datatype="local";}
			};
		};
		ts.p._height += parseInt($(grid.hDiv,ts).height());
		...
		if (!ts.p.shrinkToFit) {
			$("tr:first th", thead).each(function(j){
				...
				if (diff > 0) {
					...
					$('table',grid.bDiv).add(grid.hTable).width(ts.grid.width);
					ts.grid.width += diff;
