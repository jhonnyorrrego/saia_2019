...
if (@$_REQUEST["id"] != '')
    $padre_dependencia = str_replace("#", "", ($_REQUEST["id"]));
...
function llena_dependencia($codigo, $ruta, $tipo_llenado = "") {
    ...
    if ($codigo == "NULL" || $codigo == 0) {
        $papas = busca_filtro_tabla("iddependencia,cod_padre,nombre", "dependencia", "(cod_padre IS NULL OR cod_padre=0) AND estado=1", $orden . " ASC", $conn);
    } else {
        $papas = busca_filtro_tabla("iddependencia,cod_padre,nombre", "dependencia", "estado=1 AND cod_padre=" . $codigo, $orden . " ASC", $conn);
    }

    if ($papas["numcampos"]) {
        for ($i = 0; $i < $papas["numcampos"]; $i++) {
            $hijos = busca_filtro_tabla("count(*) AS cant", "dependencia", "estado=1 AND cod_padre=" . $papas[$i]["iddependencia"], "", $conn);
            echo "<item style=\"font-family:verdana; font-size:7pt;\" ";
            echo "text=\"" . ucwords(codifica_encabezado(html_entity_decode($papas[$i]["nombre"]))) . "\" id=\"" . $papas[$i]["iddependencia"] . "#\"";

            if ($no_padre || $tipo_llenado == 1) {
                echo " nocheckbox=\"1\" ";
            }

            if (in_array($papas[$i]["iddependencia"], $seleccionado))
                echo " checked=\"1\" ";
            if ($hijos['numcampos'])
                echo " child=\"1\">\n";
            else
    ...
    echo llena_funcionarios($codigo, $ruta, $tipo_llenado);
...
function llena_funcionarios($codigo, $ruta, $tipo_llenado) {
    ...
    if (($tipo_llenado && $ingreso) || $verifica_flujo == 0) {
        ...
        // file_put_contents("log_test.txt", date('Y-m-d H:i:s')." | ".json_encode($usuarios)."\n");
    } else
        return ("");
    // print_r($usuarios);
