...
DHTMLSuite.config.prototype = {
	 ...
	 */
	init : function()
	{	prueba = location.pathname;
	  if(prueba.search("visor_imagenes.php")>0)
     this.cssPath =  'css/';	// Path to images
    else 
      this.cssPath =  '../../css/';	// Path to images 
		this.imagePath = '../imgs/';	// Path to images		
...
DHTMLSuite.common.prototype = {
	...
	loadCSS : function(cssFile,prefixConfigPath)
	{
		...
		if(!this.loadedCSSFiles[cssFile]){
			...
			if(prefixConfigPath){
				linkTag.href = DHTMLSuite.configObj.cssPath + cssFile;
			}else{
...
DHTMLSuite.tableWidgetPageHandler.prototype = {
	...
	__createPageLinks : function()
	{
		...
		if(this.maximumNumberOfPageLinks && this.maximumNumberOfPageLinks<numberOfPages){
			...
			firstLink.onclick = function(e){ return DHTMLSuite.variableStorage.arrayDSObjects[ind].__navigate(e); }
		...
		if(this.maximumNumberOfPageLinks && this.maximumNumberOfPageLinks<numberOfPages){
			...
			lastLink.onclick = function(e){ return DHTMLSuite.variableStorage.arrayDSObjects[ind].__navigate(e); }
	...
	displayTooltip : function(externalFile,inputObj)
	{
		...
		if(!this.ajax_tooltipObj)	/* Tooltip div not created yet ? */
		{
			...
			if(DHTMLSuite.clientInfoObj.isMSIE){	/* Create iframe object for MSIE in order to make the tooltip cover select boxes */
				...
				this.ajax_tooltipObj_iframe.id='DHTMLSuite_ajax_tooltipObjIframe';
	...
	__createMenuItems : function()
	{
		...
		for(var no=0;no<this.menuItemObj.menuItemsOrder.length;no++){	// Looping through menu items		
			...
			if(!this.menuItems[indexThis].separator){
				...
				//DHTMLSuite.commonObj.addEvent(ref,"mouseover",function(){ DHTMLSuite.variableStorage.arrayDSObjects[index].__activateMenuElements(this,DHTMLSuite.variableStorage.arrayDSObjects[index],true); });	
			}
...
DHTMLSuite.paneSplitterPane.prototype =
{
	...
	*/		
	__deleteContentByIndex : function(contentIndex)
	{
		...
		var htmlId = this.paneModel.contents[contentIndex].htmlId;
		if(htmlId){
			try{
		...
		for(var no in this.menuItems){	// Looping through menu items		
			...
			if(this.menuItems[no].depth==1){
				...
				if(this.menuItems[no].separator){
					...
					DHTMLSuite.commonObj.__addEventEl(li);
				}				
...
DHTMLSuite.imageEnlarger.prototype = {
	...
	__createHTMLElements : function()
	{
		...
		DHTMLSuite.commonObj.addEvent(closeButton,'click',function(){ DHTMLSuite.variableStorage.arrayDSObjects[ind].hide(); });
...
DHTMLSuite.formValidator = function(propArray)
{
	...
	this.masks.email = /^[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/gi;
	this.masks.numeric = /^[0-9]+$/gi;	// Numeric
	this.masks.letter = /^[A-Zæøå]+$/gi;	// letteric
	this.masks.domain = /^(https?\:\/\/)?[a-zA-Z0-9]+([a-zA-Z0-9\-\.]+)?\.(com|org|net|mil|edu|info|a[cdfgilmnoqrstuwxz]|b[abdefghijmnorstwyz]|c[acdfghiklmnoruvxyz]|d[ejkmoz]|e[cegrstu]|f[ijkmor]|g[adefghilmnpqrstuwy]|h[kmnrtu]|i[delmnoqrst]|j[emop]|k[eghimnrwyz]|l[abcikrstuvy]|m[acdghklmnopqrstuvwxyz]|n[acefgilopruz]|om|p[aefghklmnrstwy]|qa|r[eouw]|s[abcdeghiklmnrtvyz]|t[cdfghjklmnortvwz]|u[agksyz]|v[aceginu]|w[fs]|y[etu]|z[amw])$/gi;
