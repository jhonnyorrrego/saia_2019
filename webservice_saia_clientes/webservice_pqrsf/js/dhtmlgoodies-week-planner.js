...
function parseItemsFromServer(ajaxIndex)
{
	...
	//Este es el ciclo para cada uno de los items que se van a crear.
	for(var no=1;no<items.length;no++){  
		var lines = items[no].split(/\n/g);
		itemsToBeCreated[no] = new Array();
		//Ciclo con cada uno de los tags, es decir, divide id, description, eventstartdate, eventenddate.
      ...
      // Arreglo con cada uno de los parametros especificados en el archivo xml que llega
			itemsToBeCreated[no][key] = value;
		}	
		
		if(itemsToBeCreated[no]['id']){
		  // Diferencia entre la fecha y hora en que se inicia el evento y el principio de la semana que se está mostrando
			var dayDiff = itemsToBeCreated[no]['eventStartDate'].getTime() - dateStartOfWeek.getTime();
			//como el tiempo se obtiene en milisegundos, debe ser 1000:ms, 60:seg, 60:min, 24:hor. para pasarlo a dias
			dayDiff = Math.floor(dayDiff / (1000*60*60*24));
			//Segun dicha diferencia obtiene el x en el cual tiene que empezar
			el_x = dayPositionArray[dayDiff];
			
			// Donde empieza el cuadro con relacion a la y, se obtiene con las horas y minutos del principio.
			topPos = getYPositionFromTime(itemsToBeCreated[no]['eventStartDate'].getHours(),itemsToBeCreated[no]['eventStartDate'].getMinutes());
			
			//Altura total que tendrá el cuadro que representa el evento. Calcula el tiempo y lo trasforma a minutos, para redondearlo con la altura de la fila 
      var elHeight = (itemsToBeCreated[no]['eventEndDate'].getTime() - itemsToBeCreated[no]['eventStartDate'].getTime()) / (60 * 60*1000);
			...
			// Invoca a la funcion para crear el cuadro que representa el evento
      ...
      currentAppointmentDiv.id = itemsToBeCreated[no]['id'];
			...
			//Obtiene el div que tiene las horas
			currentTimeDiv = getCurrentTimeDiv(currentAppointmentDiv);
			currentTimeDiv.style.display='block';
			currentTimeDiv.style.fontFamily = 'Verdana';
			currentTimeDiv.style.fontSize = '9px';
      ...
      //Obtiene el div que contiene el texto que esta escrito dentro del evento para asignar la altura
			currentAppointmentContentDiv  = getCurrentAppointmentContentDiv(currentAppointmentDiv);
			currentAppointmentContentDiv.style.height = '60px';
			//currentAppointmentContentDiv.style.height = (elHeight-20) + 'px';
			currentTimeDiv.innerHTML = '<span>' + obtenerHoras(itemsToBeCreated[no]['eventStartDate'], itemsToBeCreated[no]['eventEndDate']) + '</span>';
...
function obtenerHoras(empieza, termina)
{
  var hi = empieza.getHours();
  var hiopc = '';
  if(hi<10)
    var hiopc = '0';
    
  var mi = empieza.getMinutes();
  var miopc = '';
  if(mi<10)
    var miopc = '0';

  var hf = termina.getHours();
  var hfopc = '';
  if(hf<10)
    var hfopc = '0';

  var mf = termina.getMinutes();
  var mfopc = '';
  if(mf<10)
    var mfopc = '0';
    
  return hiopc + hi + ':' + miopc + mi + '-' + hfopc + hf + ':' + mfopc + mf; 
}
...
function getYPositionFromTime(hour,minute){
  //Calculo para obtener la posicion en y
...
function getItemsFromServer()
{
	...
	var documento = document.getElementById('docactual').value;
	weekSchedule_ajaxObjects[ajaxIndex] = new sack();	
	weekSchedule_ajaxObjects[ajaxIndex].requestFile = externalSourceFile_items  + '?year=' + dateStartOfWeek.getFullYear() + '&month=' + (dateStartOfWeek.getMonth()/1+1) + '&day=' + dateStartOfWeek.getDate() + '&documento=' + documento;	// Specifying which file to get
...
{
	//Obtiene el espacio de diferencia entre los div del calendario y el div del evento, haciendole la conversion a tiempo
  var startTime = (getTopPos(inputObj) - appointmentsOffsetTop) / (itemRowHeight+1) + weekplannerStartHour;

	// Si la diferencia es mayor a 23 es porque es mas de un dia, entonces le resta las 24 horas, creo!!!
  if(startTime>23)startTime = startTime - 24;
  // Redondea para obtener la hora en que debe empezar
	var startHour = Math.floor(startTime);
	var hourPrefix = "";
	// para colocarle el cero a la hora en caso de ser solo de un digito
	if(startHour<10)hourPrefix = "0";
	// lo que sobra convirtiendolo a minutos
	var startMinute = Math.floor((startTime - startHour) *60);
	//Si los minutos son de un digito le coloca le cero al principio
  var startMinutePrefix = "";
	if(startMinute<10)startMinutePrefix="0";	
	
	// Exactamente el mismo proceso pero para la hora de finalizacion
...
function createNewAppointmentDiv(leftPos,topPos,width,contentHTML,height)
{
	...
	if(height>30)div.style.height = height + 'px';
	else div.style.height = '30px';
	...
	innerSpan.innerHTML = '<div style="font-family:Verdana; font-size:9px;"><span style="float:right;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;</span>' + contentHTML + "</div>";
