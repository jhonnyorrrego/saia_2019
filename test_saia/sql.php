...
*/    
  ...
  function Busca_tabla_SqlServer($tabla, $conn){
    global $conn;
    //TODO: Verificar y estandarizar lo que retorna
...
*/    
  ...
  function Ejecutar_Limit_SqlServer($sql,$inicio,$fin,$conn)
    ...
    function Ejecutar_Limit_MSSql($sql,$inicio,$fin,$conn,$orden)
    {
     if($inicio<0)
      $inicio = 0;  
     $consulta=trim($sql);  
     $complemento = substr($consulta,strpos($consulta,' '));
     $select1="SELECT distinct TOP ".($fin+1).$complemento;
     $select2="SELECT distinct TOP ".$inicio.$complemento;                                 
     $consulta="select * from (".$select1." ) as s1 except select * from ( ".$select2.") as s2";  
		
		 $existe_group=strpos($orden, "order by");
		if($existe_group){
		 $orden=str_replace("order by", "%", $orden);
		 $length_group=strlen($orden);
		 $pos_group=strpos($orden, "%");
		 $orden=substr($orden,($pos_group+1),$length_group);
		}
		 if($orden!=""){
			 $vector=explode(",", str_replace("order by ","",$orden)); 							
			 for($i=0;$i<count($vector);$i++){
					$length=strlen($vector[$i]);
					$pos=strpos($vector[$i], ".");
					if($pos){
						$parte=substr($vector[$i],($pos+1),$length);
					}else{
						$parte=$vector[$i];
					}
					if($i==0){
						$parte=" order by ".$parte;
					}
					$ordenar[]=$parte;
				}
			 
		 		$consulta.=implode(",", $ordenar);		 	
		 }
