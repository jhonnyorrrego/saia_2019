		...
		if(!is_dir($ruta_db_superior.$ruta)){
			...
			foreach($documentos["anexos"] as $anexo){
				...
				if(!copy($ruta_origen, $ruta_db_superior.$ruta_destino)){					
					...
					$insert_anexo = "INSERT INTO anexos_version(documento_iddocumento,version_numero,ruta,etiqueta,tipo) VALUES(".$datos_documento["iddocumento"].",".$datos_documento["version"].",'".$ruta_destino."','".$anexo["etiqueta"]."','".$anexo["tipo"]."')";				
		...
		if(!is_dir($ruta_db_superior.$ruta)){
			...
			foreach($documentos["paginas"] as $pagina){
				...
				if(!copy($ruta_origen, $ruta_db_superior.$ruta_destino)){
					...
					$insert_pagina = "INSERT INTO anexos_version(documento_iddocumento,version_numero,ruta,etiqueta,tipo) VALUES(".$datos_documento["iddocumento"].",".$datos_documento["version"].",'".$ruta_destino."','".$pagina["pagina"]."','jpg')";
	...
	if($datos_documento["pdf"]){
		...
		if(!is_dir($ruta_db_superior.$ruta)){
			...
			if(!copy($ruta_db_superior.$ruta_origen, $ruta_db_superior.$ruta_destino)){				
				...
				$insert_pdf = "INSERT INTO anexos_version(documento_iddocumento,version_numero,ruta,etiqueta,tipo) VALUES(".$datos_documento["iddocumento"].",".$datos_documento["version"].",'".$ruta_destino."','pdf','pdf')";
...
function adicionar_registro_nuevo_anexo($datos_documento,$anexo){
	...
	$ruta_destino	= $ruta_db_superior.$ruta_anexo;	
...
function poner_documento_estado_eliminado($datos_documento){
	...
	$update_documento = "UPDATE documento SET estado='ELIMINADO' WHERE iddocumento=".$datos_documento["iddocumento"];
	print_r($update_documento);
	//phpmkr_query($update_documento,"",$datos_documento["funcionario_codigo"]);	
