...
{$.ajax({
   ...
   data: 'plantilla='+plantilla+"&iddoc="+doc+"&versionamiento=1",
   async: false ,
...
{$doc=busca_filtro_tabla("lower(plantilla) as plantilla","documento","iddocumento=".$_REQUEST["key"],"",$conn);
 ...
 echo "<script>
       crear_pdf('".$_REQUEST["key"]."','".$doc[0]["plantilla"]."','".$version."');
       </script>";
}
     ...
     {// echo $archivo;
      ...
      if(copy($archivo,$ruta."/doc".$_REQUEST["doc"].".pdf"))
        ...
        // print_r($paginas);
         ...
         $sql="insert into documento_version(funcionario,documento_iddocumento,version,fecha) values('".usuario_actual("funcionario_codigo")."','".$_REQUEST["doc"]."','".$_REQUEST["version"]."',".fecha_db_almacenar("","Y-m-d H:i:s").")";
         phpmkr_query($sql,$conn);
         comprimir("version".$_REQUEST["version"],$ruta);
         alerta(utf8_encode("Creada versi�n ".$_REQUEST["version"]));
