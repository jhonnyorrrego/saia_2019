  ...
  function Ejecutar_Sql_MySql($sql)
    {
     ...
     if($sql && $sql<>"" && $this->Conn->conn){
         $this->res=mysql_query($this->Conn->conn,$sql) or die("ERROR SQL ".mysqli_error($this->Conn->conn)." en ".$_SERVER["PHP_SELF"]." ->".$sql);// or error//("Error al Ejecutar:  $sql --- ".mysql_error());
        if($this->res){
