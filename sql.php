...
class SQL {
	 ...
	 */
	function Ejecutar_Sql_Noresult($sql) { $sql = html_entity_decode(htmlentities(utf8_decode($sql)));
		switch ($this->motor) {
			case "MySql" :
	...
	function Ejecutar_Sql_MySql($sql) {
		...
		if ($sql && $sql <> "" && $this -> Conn -> conn) {
			...
			//or die("ERROR SQL ".mysqli_error($this->Conn->conn)." en ".$_SERVER["PHP_SELF"]." ->".$sql);// or error//("Error al Ejecutar:  $sql --- ".mysql_error());

			if ($this -> res) {
				if (strpos(strtolower($sql), "insert") !== false)
					$this -> ultimo_insert = $this -> Ultimo_Insert_Mysql();
				else if (strpos(strtolower($sql), "select") !== false) {$this -> ultimo_insert = 0;
					$this -> filas = $this -> res -> num_rows;
				} else {$this -> ultimo_insert = 0;
				}
	...
	function Ejecutar_Sql_SqlServer($sql) { $this -> filas = 0;
		if ($sql && $sql <> "") {
			sqlsrv_query($this -> Conn -> conn, "USE " . $this -> Conn -> Db);
	...
	function Ejecutar_Sql_MSSql($sql) { $this -> filas = 0;
		if ($sql && $sql <> "") {
			//mssql_query("USE ".$this->Conn->Db,$this->Conn->conn);
	...
	function Ejecutar_Sql_Oracle($sql) {
		...
		if ($rs) {
			if (@OCIExecute($rs, OCI_COMMIT_ON_SUCCESS)) {$this -> res = $rs;

				if (strpos(strtolower($sql), "insert") !== false) {$this -> ultimo_insert = $this -> Ultimo_Insert_Oracle();
				}
	...
	/*
	 ...
	 <Post-condiciones>
	 */
	function Insertar($campos, $tabla, $valores) { $valores = html_entity_decode(htmlentities(utf8_decode($valores)));
		switch ($this->motor) {
			case "MySql" :
	...
	function Ejecutar_Sql_Tipo($sql) { $sql = html_entity_decode(htmlentities(utf8_decode($sql)));
		switch ($this->motor) {
			case "MySql" :
	...
	/*
	 ...
	 <Post-condiciones>
	 */
	function Ejecutar_Sql_Tipo_MySql($sql) {$this -> consulta = $sql;
		$this -> res = mysqli_query($this -> Conn -> conn, $this -> consulta);
	...
	/*
	 ...
	 <Post-condiciones>
	 */
	function Ejecutar_Sql_Tipo_SqlServer($sql) {$this -> consulta = $sql;
		sqlsrv_query($this -> Conn -> conn, "USE " . $this -> Conn -> Db);
		$this -> res = sqlsrv_query($this -> Conn -> conn, $this -> consulta);
		$this -> Guardar_log($sql);
	...
	function Ejecutar_Sql_Tipo_MSSql($sql) {$this -> consulta = $sql;
		mssql_query("USE " . $this -> Conn -> Db, $this -> Conn -> conn);
		$this -> res = mssql_query($this -> consulta, $this -> Conn -> conn);
		$this -> Guardar_log($sql);
	...
	/*
	 ...
	 <Post-condiciones>
	 */
	function Resultado_MySql() {$resultado["sql"] = $this -> consulta;
		$resultado["numcampos"] = $this -> Numero_Filas();
		if ($this -> Numero_Filas() > 0) {
			for ($i = 0; $i < $this -> Numero_Filas(); $i++) {$resultado[$i] = mysqli_fetch_array($this -> res, MYSQLI_ASSOC);
				$j = 0;
				foreach ($resultado[$i] as $key => $valor) {$resultado[$i][$j] = $resultado[$i][$key];
					$j++;
				}
			}
	...
	function Resultado_SqlServer() {$resultado["sql"] = $this -> consulta;
		$resultado["numcampos"] = $this -> Numero_Filas();
		if ($this -> Numero_Filas() > 0) {
			for ($i = 0; $i < $this -> Numero_Filas(); $i++) {$resultado[$i] = sqlsrv_fetch_array($this -> res, SQLSRV_FETCH_ASSOC);
				$j = 0;
				foreach ($resultado[$i] as $key => $valor) {$resultado[$i][$j] = $resultado[$i][$key];
					$j++;
				}
	...
	function Resultado_MSSql() {$resultado["sql"] = $this -> consulta;
		$resultado["numcampos"] = $this -> Numero_Filas();
		if ($this -> Numero_Filas() > 0) {
			for ($i = 0; $i < $this -> Numero_Filas(); $i++) {$resultado[$i] = mssql_fetch_array($this -> res, MSSQL_ASSO);
				$j = 0;
				foreach ($resultado[$i] as $key => $valor) {$resultado[$i][$j] = $resultado[$i][$key];
					$j++;
				}
	...
	/*
	 ...
	 <Post-condiciones>
	 */
	function Rows_Count() {mysqli_affected_rows($this -> res);
	}
	...
	function Ejecutar_Limit_MySql($sql, $inicio, $fin, $conn) {
		...
		//echo $consulta;
		$res = mysqli_query($conn -> Conn -> conn, $consulta);
		// or die("consulta fallida ".mysqli_error($conn->Conn->conn));
		return ($res);
	}
	...
	function Ejecutar_Limit_Oracle($sql, $inicio, $fin, $conn) {$inicio = $inicio + 1;
		$fin += 1;
		$cuantos = $fin - $inicio;
		$sql = "SELECT *
	...
	/*
	 ...
	 <Post-condiciones>
	 */
	function Total_Registros_Tabla_SqlServer($tabla) {$this -> consulta = "SELECT COUNT( * ) AS TOTAL FROM " . $tabla;
		sqlsrv_query($this -> Conn -> conn, "USE " . $this -> Conn -> Db);
		$this -> res = sqlsrv_query($this -> Conn -> conn, $this -> consulta);
	...
	function Total_Registros_Tabla_MSSql($tabla) {$this -> consulta = "SELECT COUNT( * ) AS TOTAL FROM " . $tabla;
		mssql_query("USE " . $this -> Conn -> Db, $this -> Conn -> conn);
		$this -> res = mssql_query($this -> consulta, $this -> Conn -> conn);
