  ...
  function Ejecutar_Sql_MySql($sql)
    {
     ...
     if($sql && $sql<>"" && $this->Conn->conn){
         $this->res=mysql_query($sql,$this->Conn->conn) or die("ERROR SQL ".mysql_error($this->Conn->conn)." en ".$_SERVER["PHP_SELF"]." ->".$sql);// or error//("Error al Ejecutar:  $sql --- ".mysql_error());
        if($this->res){
