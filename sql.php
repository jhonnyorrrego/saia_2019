  ...
  function Ejecutar_Sql_MySql($sql)
    {
     ...
     if($sql && $sql<>"" && $this->Conn->conn){
         $this->res=mysqli_query($this->Conn->conn,$sql)K; //or die("   ".print_r($this->Conn->conn,true)."<---> ERROR SQL  en ".$_SERVER["PHP_SELF"]." ->".$sql);// or error//("Error al Ejecutar:  $sql --- ".mysql_error());
        if($this->res){
