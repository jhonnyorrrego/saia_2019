...
function llena_expediente($id) {
	global $conn, $objetoXML, $condicion_ad, $seleccionados, $sql_exp_asig;
	if ($id == 0) {
		$papas = busca_filtro_tabla("DISTINCT idexpediente,serie_idserie,nombre,codigo_numero,estado_cierre", "vexpediente_serie a", "(cod_padre=0 or cod_padre is null)" . $condicion_ad, "nombre ASC", $conn);
	} else {
		$papas = busca_filtro_tabla("DISTINCT idexpediente,serie_idserie,nombre,codigo_numero,estado_cierre", "vexpediente_serie a", "cod_padre=" . $id . $condicion_ad, "nombre ASC", $conn);
	}
	if ($papas["numcampos"]) {
		for ($i = 0; $i < $papas["numcampos"]; $i++) {
			...
			$hijos = busca_filtro_tabla("count(*) as cant", "vexpediente_serie a", "cod_padre=" . $papas[$i]["idexpediente"] . $condicion_ad, "", $conn);
...
function llena_tipo_documental($id, $idexp) {
	...
	if ($papas["numcampos"]) {
		for ($i = 0; $i < $papas["numcampos"]; $i++) {
			$permiso=busca_filtro_tabla("count(*) as cant","vpermiso_serie","idfuncionario=".$_SESSION["idfuncionario"]." and idserie=".$papas[$i]["idserie"],"",$conn);
