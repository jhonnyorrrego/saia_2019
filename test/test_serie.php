...
function llena_serie($id) {
	...
	if ($id == 0) {
		$papas = busca_filtro_tabla("", "serie", "(cod_padre=0 or cod_padre is null)" . $condicion_ad, "nombre ASC", $conn);
	} else {
		$papas = busca_filtro_tabla("", "serie", "cod_padre=" . $id . $condicion_ad, "nombre ASC", $conn);
	}
	if ($papas["numcampos"]) {
		for ($i = 0; $i < $papas["numcampos"]; $i++) {
			...
			$hijos = busca_filtro_tabla("count(*) as cant", "serie", "cod_padre=" . $papas[$i]["idserie"] . $condicion_ad, "", $conn);
			if ($hijos[0]["cant"]) {
