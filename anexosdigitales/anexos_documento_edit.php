...
if(isset($_REQUEST["Adicionar"])) // Se procesa el formulario
  {
    ...
    if(isset($_REQUEST["idformato"])&&isset($_REQUEST["idcampo"]))
     {  cargar_archivo($_REQUEST["key"],$permisos,$_REQUEST["idformato"],$_REQUEST["idcampo"]);
     	//abrir_url("anexos_documento_edit.php?key=".$_REQUEST["key"]."&idformato=".$_REQUEST["idformato"]."&idcampo=".$_REQUEST["idcampo"],$_REQUEST["frame"]);
    ...
    { cargar_archivo($_REQUEST["key"],$permisos);
      //abrir_url("anexos_documento_edit.php?key=".$iddocumento,$_REQUEST["frame"]);
...
if(isset($_REQUEST["key"])&&isset($_REQUEST["idformato"])&&isset($_REQUEST["idcampo"]))
 {
   ...
   if($_REQUEST["frame"])
     $frame=$_REQUEST["frame"]; // Si es un frame
   else 
     $frame="centro";
...
elseif(isset($_REQUEST["key"]))
 {
   ...
   echo listar_anexos_documento($iddocumento); // lista anexos documento
...
$adicional="";
if($validaciones[0]["valor"])$adicional='accept="'.$validaciones[0]["valor"].'"';
?>
</br>
...
<tr>
<td class="celda_transparente"><input type="file" name="anexos[]" class="multi" <?php echo($adicional); ?>>
</tr>
<tr><td><input type="submit" value="Adicionar" name="Adicionar"> </td></tr>
<tr><td>&nbsp;</td></tr>
</table>
</form>
<body>
