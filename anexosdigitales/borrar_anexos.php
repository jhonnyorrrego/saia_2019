...
<script>
  ...
  function recargar_centro(iddocumento) {
    ...
    if (parent.frames['arbol_formato']) {
      ...
      parent.parent.parent.frames['arbol_formato'].postMessage({
        iddocumento: iddocumento
      }, "*");
    } else {
...
<form name="borraranexo" id="borraranexo" action="borrar_anexos.php" method="POST">
  ...
  <table aling="center">
    ...
    <tr>
      <td>Archivo : <b> <?php echo $anexo[0]["etiqueta"]; ?> </b> </td>
    </tr>
    <tr>
      <td><input type="hidden" name="idanexo" value="<?php echo $_REQUEST["idanexo"]; ?>"></td>
    </tr>
    <tr>
      <td></td>
    </tr>
    <tr>
      <td><input type="hidden" name="Eliminar" value="Eliminar"><input type="submit" value="Eliminar"></td>
      <td></td>
  </table>
</form>
