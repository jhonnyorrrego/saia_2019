...
require_once($ruta_db_superior.'calendario/activecalendar/source/functions.js');
echo "</script>\n";
...
// asignaciones 
 $asignaciones=busca_filtro_tabla("idasignacion,fecha_inicial,fecha_final","asignacion","asignacion.documento_iddocumento='-1'  AND ".fecha_db_obtener("fecha_inicial",'d-m-Y')."<='$fecha_final'  AND  ".fecha_db_obtener("fecha_final",'d-m-Y').">='$fecha_inicial'","",$conn);
 ...
 if($asignaciones["numcampos"])
   ...
   ?><link rel="stylesheet" type="text/css" href="<?php echo compara_ruta_archivos("/".RUTA_SCRIPT."/calendario/activecalendar/data/css/default2.css"); ?>" /><?php 	
...
// asignaciones 
 $asignaciones=busca_filtro_tabla("idreserva,fecha_inicial,fecha_final","reserva",fecha_db_obtener("fecha_inicial","d-m-Y").">='$fecha_inicial' and ".fecha_db_obtener("fecha_final","d-m-Y")."<='$fecha_final'","",$conn);
...
{  
 $asignaciones=busca_filtro_tabla("idasignacion,fecha_inicial,fecha_final","asignacion","asignacion.documento_iddocumento=\"$id_documento\" AND fecha_inicial<=\"$fin_mes\" AND  fecha_final>=\"$ini_mes\"","",$conn);
  ...
  if($series_doc["numcampos"]) // Encontro la serie asociada al documento  
	{
	 ...
	 $asignaciones_serie=busca_filtro_tabla("idasignacion,fecha_inicial,fecha_final","asignacion","asignacion.serie_idserie=$id_serie AND fecha_inicial<=\"$fin_mes\" AND  fecha_final>=\"$ini_mes\"","",$conn);
	} 
...
function selector_fecha($nombre_campo,$nombre_form,$formato,$mes=NULL,$anio=NULL,$css="default.css",$ruta_relativa=NULL,$parametros_tarea=NULL,$tipo=NULL,$cuadro_texto=NULL,$asigna_tarea=NULL,$hora=NULL,$minuto=NULL,$am_pm=NULL)
{
...
if(!isset($anio)||!isset($mes)) //Obtiene la fecha actual por defecto si no se envia nada
 ...
 if($tipo=="VENTANA") // Verifica el tipo de calendario  
   { 
    if($cuadro_texto)
     { //echo "<script src=\"".compara_ruta_archivos("/".RUTA_SCRIPT."/popcalendar.js")."\" type=\"text/javascript\" language=\"javascript\"></script>";
       //echo "<script src=\"".compara_ruta_archivos("/".RUTA_SCRIPT."/functions.js")."\" type=\"text/javascript\" language=\"javascript\"></script>";
       echo "<input type=\"text\" id=\"".$nombre_campo."\" name=\"".$nombre_campo."\" />&nbsp;&nbsp;&nbsp;<a href=\"javascript:showcalendar('$nombre_cuadro','$nombre_form','$formato','$param',220,225)\" ><img src=\"".$ruta_db_superior."calendario/activecalendar/data/img/calendar.gif"."\" border=\"0\" alt=\"Elija Fecha\" /></a>";
    ...
    else 
    {
       //echo "<script src=\"".compara_ruta_archivos("/".RUTA_SCRIPT."/popcalendar.js")."\" type=\"text/javascript\" language=\"javascript\"></script>";
       //echo "<script src=\"".compara_ruta_archivos("/".RUTA_SCRIPT."/functions.js")."\" type=\"text/javascript\" language=\"javascript\"></script>";
       echo "&nbsp;&nbsp;&nbsp;<a href=\"javascript:showcalendar('$nombre_campo','$nombre_form','$formato','$param',220,225)\" ><img src=\"".$ruta_db_superior."calendario/activecalendar/data/img/calendar.gif"."\" border=\"0\" alt=\"Elija Fecha\" /></a>";
    }    
   echo $asginar_tarea;
   if($asigna_tarea) // Se imprime el control de asignacion de tareas
     { 
      $param_asig=$ruta_db_superior."asignaciones/asignacionadd.php?popup=1&nom_form=".$nombre_form."&nom_campo="."asig_".$nombre_campo; 
       if($cuadro_texto)
        echo "<br /> <input type=\"text\" id=\""."asig_".$nombre_campo."\" name=\""."asig_".$nombre_campo."\" />&nbsp;&nbsp;&nbsp;";     
      
      echo "&nbsp;&nbsp;&nbsp; <a href=\"javascript:showasignatarea('$nombre_cuadro','$nombre_form','$formato','$param_asig',500,500)\" >";
      echo "<img src=\"".$ruta_db_superior."/calendario/activecalendar/data/img/tareas.png"."\" border=\"0\" alt=\"Elija Fecha\" /></a>";
     }  
 ...
 elseif($tipo=="POPUP") // Tipo  popup .. calendario anterior modificado
  { 
     echo "<script src=\"".compara_ruta_archivos("/".RUTA_SCRIPT."/popcalendar.js")."\" type=\"text/javascript\" language=\"javascript\"></script>";
  	 echo  "<input type=\"text\" id=\"".$nombre_campo."\" name=\"".$nombre_campo."\" />&nbsp;&nbsp;&nbsp;<a href=\"javascript:popUpCalendar(this, this.form.fecha_inicial,'dd/mm/yyyy');\" ><img src=\"".compara_ruta_archivos("/".RUTA_SCRIPT."/calendario/activecalendar/data/img/calendar.gif")."\" border=\"0\" alt=\"Elija Fecha\" /></a>";
  }
...
{ global $conn; 
   ...
   $fecha_final=date($formato, mktime( 0, 0, 0,$mesinicial, $diainicial + $dias,$anioinicial));
    $asignaciones=busca_filtro_tabla("idasignacion,fecha_inicial,fecha_final",DB.".asignacion","asignacion.documento_iddocumento='-1'  AND asignacion.fecha_inicial < ".fecha_db_almacenar($fecha_final,$formato)." AND asignacion.fecha_final > ".fecha_db_almacenar($fecha_inicial,$formato),"",$conn); 

  if($asignaciones["numcampos"])
	 {  $no_laborales=$asignaciones["numcampos"]; 
	    $fecha_legal= date($formato, mktime( 0, 0, 0,$mesinicial, $diainicial + $dias,$anioinicial));

 // Verificacion de que en los dias adicionales no existan nuevos festivos !!!!! 
