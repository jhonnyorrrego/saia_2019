...
{ global $conn;
  ...
  $datos= $asignaciones=busca_filtro_tabla("idasignacion","asignacion","asignacion.documento_iddocumento='-1'  AND asignacion.fecha_inicial =to_date('$fecha_inicial','$formato_bd') ","",$conn);
 ...
 if($datos["numcampos"])
  ...
  {   // Busco la tarea de dias festivos para asignara 
   ...
   $datos= $asignaciones=busca_filtro_tabla("idtarea",DB.".tarea","tarea.nombre='FESTIVO' ","",$conn);
   if($datos["numcampos"])
    {
      $idtarea=$datos[0]["idtarea"];
        
      $sql= "INSERT INTO ".DB.".ASIGNACION ( tarea_idtarea , fecha_inicial , fecha_final , documento_iddocumento ) VALUES ('$idtarea',".fecha_db_almacenar($fecha_inicial,$formato).",".fecha_db_almacenar($fecha_final,$formato).",'-1')";
        
      phpmkr_query($sql,$conn) or  die("Falló insercion del dia " . phpmkr_error() . ' SQL:' . $sql);
    }
   //$sql= "INSERT INTO ".DB.".ASIGNACION "   
  }  
return(TRUE); 
}
...
{ global $conn;
  ...
  {  
      $sql= "DELETE FROM ".DB.".ASIGNACION WHERE asignacion.idasignacion='$id_asignacion'";
