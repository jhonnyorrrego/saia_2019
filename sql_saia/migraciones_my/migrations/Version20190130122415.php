    ...
    public function up(Schema $schema): void {
        if($schema->hasTable("wf_elemento")) {
            ...
            if(!$tabla->hasColumn("req_calidad_in")) {
                $tabla->addColumn("req_calidad_in", "text", [
                    "notnull" => false
                ]);
            }
            if(!$tabla->hasColumn("req_calidad_out")) {
                $tabla->addColumn("req_calidad_out", "text", [
                    "notnull" => false
                ]);
            }
        }
        ...
        if($schema->hasTable("wf_tarea_actividad")) {
            ...
            if(!$tabla->hasColumn("obligatorio")) {
                $tabla->addColumn("obligatorio", "integer", [
                    "default" => 0
                ]);
            }
        ...
        if(!$schema->hasTable("wf_formato_actividad")) {
            ...
            $tabla->addColumn("idformato_actividad", "integer", [
                "autoincrement" => true
            ]);
            $tabla->addColumn("fk_actividad", "integer");
        ...
        if(!$schema->hasTable("wf_req_calidad_activ")) {
            ...
            $tabla->addColumn("idrequisito_calidad", "integer", [
                "autoincrement" => true
            ]);
            $tabla->addColumn("fk_actividad", "integer");
            $tabla->addColumn("obligatorio", "integer", [
            ...
            $tabla->addColumn("requisito", "string", [
                "length" => 255
            ]);
        ...
        if(!$schema->hasTable("wf_tipo_decision_activ")) {
            ...
            $tabla->addColumn("idtipo_decision_activ", "integer", [
                "autoincrement" => true
            ]);
            $tabla->addColumn("tipo_decision", "string", [
                "length" => 255
            ]);
            $tabla->setPrimaryKey([
                "idtipo_decision_activ"
            ]);
        }

        if(!$schema->hasTable("wf_tipo_impacto_riesgo")) {
            $tabla = $schema->createTable("wf_tipo_impacto_riesgo");
            $tabla->addColumn("idtipo_impacto", "integer", [
                "autoincrement" => true
            ]);
            $tabla->addColumn("impacto", "string", [
                "length" => 255
            ]);
            $tabla->setPrimaryKey([
                "idtipo_impacto"
        ...
        if(!$schema->hasTable("wf_tipo_prob_riesgo")) {
            ...
            $tabla->addColumn("idtipo_probabilidad", "integer", [
                "autoincrement" => true
            ]);
            $tabla->addColumn("probabilidad", "string", [
                "length" => 255
            ]);
            $tabla->setPrimaryKey([
                "idtipo_probabilidad"
        ...
        if(!$schema->hasTable("wf_riesgo_actividad")) {
            ...
            $tabla->addColumn("idriesgo", "integer", [
                "autoincrement" => true
            ]);
            $tabla->addColumn("fk_actividad", "integer");
            $tabla->addColumn("riesgo", "string", [
                "length" => 255
            ]);
            $tabla->addColumn("descripcion", "string", [
                "length" => 4000
            ]);
        ...
        if(!$schema->hasTable("wf_decision_actividad")) {
            ...
            $tabla->addColumn("iddecision_actividad", "integer", [
                "autoincrement" => true
            ]);
            $tabla->addColumn("decision", "string", [
                "length" => 255
            ]);
            $tabla->addColumn("fk_actividad", "integer");
            $tabla->addColumn("fk_tipo_decision", "integer");
            $tabla->setPrimaryKey([
                "iddecision_actividad"
            ]);
        }
    }
    ...
    public function postUp(Schema $schema): void {
        if($schema->hasTable("wf_tipo_prob_riesgo")) {
            ...
            foreach ($probs as $value) {
                $this->guardar("wf_tipo_prob_riesgo", $value, "probabilidad", "idtipo_probabilidad");
            }
        }
        ...
        if($schema->hasTable("wf_tipo_impacto_riesgo")) {
            $riesgos = [
                ...
                [
                    "impacto" => "Catastr&oacute;fico"
                ]
            ];
            foreach ($riesgos as $value) {
                $this->guardar("wf_tipo_impacto_riesgo", $value, "impacto", "idtipo_impacto");
            }
        ...
        if($schema->hasTable("wf_tipo_decision_activ")) {
            $descis = [
                ["tipo_decision" => "Positiva"],
                ["tipo_decision" => "Negativa"],
                ["tipo_decision" => "Otra"   ]
            ];
            foreach ($descis as $value) {
                $this->guardar("wf_tipo_decision_activ", $value, "tipo_decision", "idtipo_decision_activ");
            }
        }
    }
